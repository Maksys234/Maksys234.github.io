...
(function() { // IIFE для изоляции области видимости
    ...
    PlanApp.generatePlanContentWithGemini = async (testData, topicsData) => {
        const config = PlanApp.config;
        console.log("[Core GeminiGenerate] Starting plan generation...");
        ...
        // --- Промпт для Gemini (Улучшенная версия v23) ---
        const prompt = `
...
`;
        ...
        try {
            ...
            return geminiResponse; // Връщаме пълния отговор (Markdown + JSON)
        } catch (error) {
            console.error('[Core GeminiGenerate] Error generating plan content:', error);
            throw error; // Препредаваме грешката към извикващата функция
        }
    };
