...
(function() { // IIFE для изоляции области видимости
    ...
    PlanApp.ui = {
        ...
        planSection: document.getElementById('planSection'), // Секция для детального просмотра/генерации
        planLoading: document.getElementById('planLoading'),
        planSectionTitle: document.getElementById('plan-section-title'),
        planContent: document.getElementById('planContent'), // Для Markdown
        planActions: document.getElementById('planActions'), // Кнопки под сгенерированным планом
    ...
    PlanApp.setLoadingState = (sectionKey, isLoadingFlag) => {
        ...
        const loaderMap = { current: ui.currentPlanLoader, history: ui.historyLoader, create: ui.createPlanLoader, detail: ui.planLoading, schedule: ui.currentPlanLoader, generation: ui.planLoading, notifications: null, saving: null /* No specific visual loader for saving yet */ };
    ...
    PlanApp.renderMessage = (container, type = 'info', title, message, addButtons = []) => {
        ...
        container.classList.add('content-visible'); // Make sure the message container is shown
    ...
    PlanApp.switchTab = async (tabId) => {
        ...
        const ui = PlanApp.ui;
        if (!state.supabaseClient) { PlanApp.showGlobalError("Aplikace není správně inicializována."); return; }
        console.log(`[UI NAV] Switching to tab: ${tabId}`);
        ...
        // Hide specific content within sections to prevent flashes
        ...
        // Clear generated state if leaving the detail/preview section
        if (state.lastGeneratedMarkdown !== null && tabId !== 'detail' && tabId !== 'generation') { // Assuming 'detail' or 'generation' could be used for preview section ID
            console.log("[UI NAV] Clearing generated plan state as we are leaving the preview/detail section.");
            state.lastGeneratedMarkdown = null;
            state.lastGeneratedActivitiesJson = null;
            state.lastGeneratedTopicsData = null;
        }

        PlanApp.hideGlobalError(); // Clear any previous global errors
        ...
        try {
            ...
            switch(tabId) {
                ...
                default: console.warn(`[UI NAV] Unknown tab ID: ${tabId}`); return;
            }
            ...
            if (targetSectionElement) {
                ...
                if (tabId === 'current') await PlanApp.loadCurrentPlan();
                else if (tabId === 'history') await PlanApp.loadPlanHistory();
                else if (tabId === 'create') await PlanApp.checkPlanCreationAvailability();

            } else {
                console.warn(`[UI NAV] Target section element not found for tab: ${tabId}`);
            }
        } catch (error) {
            console.error(`[UI NAV] Error loading tab ${tabId}:`, error);
            const errorTargetSection = document.getElementById(`${tabId}PlanSection`);
            const errorContentContainer = errorTargetSection?.querySelector('.section-content'); // Generic selector
    ...
    PlanApp.showVerticalSchedule = async (plan) => {
        const ui = PlanApp.ui;
        if (!PlanApp.state.supabaseClient || !plan || !plan.id) {
        ...
        try {
            ...
            const { data: activities, error } = await PlanApp.state.supabaseClient
    ...
    PlanApp.renderLockedPlanSection = (container) => {
     ...
     PlanApp.startPlanTimer = () => {
         const state = PlanApp.state; // Need state
         ...
         state.planTimerInterval = setInterval(() => {
             ...
             if (timerEl && document.body.contains(timerEl) && typeof PlanApp.updateNextPlanTimer === 'function') {
                  PlanApp.updateNextPlanTimer(timerEl);
             } else {
                 clearInterval(state.planTimerInterval); // Stop if element is gone
                 state.planTimerInterval = null;
             }
         }, 1000);
     };
     ...
     PlanApp.updateNextPlanTimer = (el) => {
          ...
          const now = new Date();
          const diff = state.nextPlanCreateTime - now;

          if (diff <= 0) {
              el.textContent = 'Nyní';
              clearInterval(state.planTimerInterval);
              state.planTimerInterval = null;
              state.planCreateAllowed = true;
              // Optionally refresh the create tab if currently active
              if (state.currentTab === 'create' && typeof PlanApp.checkPlanCreationAvailability === 'function') {
                  setTimeout(PlanApp.checkPlanCreationAvailability, 500);
              }
              return;
          }
    ...
    PlanApp.renderPlanCreationForm = (container) => {
     ...
     PlanApp.handleGenerateClick = function() {
         const state = PlanApp.state;
         ...
         // Call the core generation function
          if (typeof PlanApp.generateStudyPlan === 'function') {
              ...
              console.error("PlanApp.generateStudyPlan function not found!");
              this.disabled = false;
              this.innerHTML = '<i class="fas fa-cogs"></i> Vygenerovat nový plán';
              PlanApp.showToast("Chyba", "Funkce pro generování plánu není dostupná.", "error");
          }
     };
    ...
    PlanApp.renderPreviewActions = (isError = false) => {
         ...
         // Attach listeners only if the corresponding core function exists
    ...
    // --- Инициализация Приложения (включает UI) ---
    PlanApp.initializeApp = async () => {
        const state = PlanApp.state; // Доступ к состоянию
        const ui = PlanApp.ui;     // Доступ к UI элементам

        console.log("🚀 [Init Plan UI] Starting...");
        if (!PlanApp.initializeSupabase()) return; // Инициализация Supabase из ядра

        if(ui.initialLoader) { ui.initialLoader.style.display = 'flex'; ui.initialLoader.classList.remove('hidden');}
        if (ui.mainContent) ui.mainContent.style.display = 'none'; // Скрыть основной контент до загрузки
        PlanApp.hideGlobalError(); // Скрыть предыдущие ошибки

        try {
            // Аутентификация (из ядра)
            const { data: { user }, error } = await state.supabaseClient.auth.getUser();
            if (error) throw new Error("Nepodařilo se ověřit uživatele.");
            if (!user) { window.location.href = '/auth/index.html'; return; } // Перенаправление, если не залогинен
            state.currentUser = user;

            // Загрузка профиля (из ядра)
            state.currentProfile = await PlanApp.fetchUserProfile(user.id);
            if (!state.currentProfile) { throw new Error("Nepodařilo se načíst profil uživatele."); }
            PlanApp.updateUserInfoUI(); // Обновление UI с данными пользователя

            // Настройка базовых UI обработчиков
            PlanApp.setupEventListeners();
            PlanApp.initTooltips();
            PlanApp.initMouseFollower();
            PlanApp.initHeaderScrollDetection();
            PlanApp.updateCopyrightYear();
            PlanApp.updateOnlineStatus(); // Check initial status

            // Загрузка уведомлений (из ядра, рендеринг в UI)
             const loadNotificationsPromise = PlanApp.fetchNotifications(user.id, PlanApp.config.NOTIFICATION_FETCH_LIMIT)
                 .then(({ unreadCount, notifications }) => PlanApp.renderNotifications(unreadCount, notifications))
                 .catch(err => { console.error("Failed to load notifications initially:", err); PlanApp.renderNotifications(0, []); });

            // Загрузка данных для начальной вкладки ('current')
            const loadInitialTabPromise = PlanApp.switchTab('current'); // switchTab теперь вызывает loadCurrentPlan

            await Promise.all([loadNotificationsPromise, loadInitialTabPromise]);

            if (ui.mainContent) {
                ui.mainContent.style.display = 'block'; // Показать основной контент
                requestAnimationFrame(() => { ui.mainContent.classList.add('loaded'); PlanApp.initScrollAnimations(); });
            }
            console.log("✅ [Init Plan UI] Page Initialized.");

        } catch (error) {
            console.error("[UI Init] App initialization error:", error);
            PlanApp.showGlobalError(`Chyba inicializace: ${error.message}`);
            if (ui.mainContent) ui.mainContent.style.display = 'block'; // Показать контент, чтобы была видна ошибка
             // Попытка отобразить ошибку в контейнере текущей вкладки
             const errorContainer = ui.currentPlanContent || ui.mainContent;
             if(errorContainer) PlanApp.renderMessage(errorContainer, 'error', 'Chyba inicializace', error.message);
             // Сбросить все состояния загрузки при ошибке
    ...
    // --- Запуск ---
    // Добавляем слушатель, который вызовет инициализацию UI после загрузки DOM
