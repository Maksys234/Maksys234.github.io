...
// Version: 23.2 - Added loadPageData, event listener, fixed initialization flow

(function() {
    'use strict';

    // Use the globally exposed Supabase client
    // It will be assigned after the 'dashboardReady' event
    let supabase = null;
    let currentUser = null;
    let currentProfile = null;
    let allTitles = []; // Store titles fetched by dashboard.js
    ...
    // Assumes elements exist as per main.html structure
    const ui = {
        // Common elements (might be cached by dashboard.js too, harmless redundancy)
        ...
        statsCompleted: document.getElementById('stats-completed'), // Corrected ID
        statsCompletedChange: document.getElementById('stats-completed-change'), // Corrected ID
        statsAvgScore: document.getElementById('stats-avg-score'), // Corrected ID
        statsAvgScoreChange: document.getElementById('stats-avg-score-change'), // Corrected ID
        statsTimeSpent: document.getElementById('stats-time-spent'), // Corrected ID
        statsTimeChange: document.getElementById('stats-time-change'), // Corrected ID
        statsWeakestTopic: document.getElementById('stats-weakest-topic'), // Corrected ID
        statsWeakestTopicFooter: document.querySelector('#stats-weakest-topic')?.closest('.dashboard-card')?.querySelector('.card-footer'), // Corrected selector
        shortcutsGrid: document.getElementById('shortcuts-grid'), // Added for shortcuts
    ...
    function updateOnlineStatus() { /* Assume offline banner doesn't exist here */ if (!navigator.onLine) console.warn("Network offline, banner display is disabled."); }
    ...
    // --- Loading State (Specific to this page's content) ---
    function setLoadingState(section, isLoadingFlag) {
        ...
        const sectionMap = {
            ...
            notifications: { container: ui.notificationsList, empty: ui.noNotificationsMsg } // Handled by dashboard.js mostly
        };

        const config = sectionMap[section];
        if (!config) {
             // If section is not specific to main.js, assume dashboard.js handles it
        ...
        if (isLoadingFlag) {
            ...
            // Optionally render specific skeletons here if needed
            if (section === 'tests' && config.content) renderTestSkeletons(config.content);
            if (section === 'plan' && config.content) renderPlanSkeletons(config.content);
            if (section === 'topics' && config.content) renderTopicSkeletons(config.content);
            ...
            // Stats are handled differently (loading class on cards)
            // Notifications are handled by dashboard.js
        ...
        // Handle loading class for stats cards
    ...
    // --- Data Fetching (Specific to main.html content) ---
    ...
    // --- Event Handlers (Keep existing, ensure they use local ui cache) ---
    ...
    // --- Setup Event Listeners (Adjusted for consistency) ---
    function setupEventListeners() {
        // Use local ui cache
        // Sidebar listeners might be handled by dashboard.js, but keep them for standalone use possibility
        if (ui.mainMobileMenuToggle) ui.mainMobileMenuToggle.addEventListener('click', openMenu); // Assuming openMenu is defined locally or globally
    ...
    async function initializeApp() {
        console.log("[INIT Procvičování] Starting initialization...");

        // Listen for the dashboardReady event before proceeding
        document.addEventListener('dashboardReady', async (event) => {
            ...
            if (!supabase || !currentUser || !currentProfile) {
                console.error("[INIT Procvičování] Critical data missing from dashboardReady event. Redirecting to dashboard.");
                 showError("Chyba načítání základních dat. Zkuste obnovit Nástěnku.", true);
                // Redirect back to dashboard might be safer
                 setTimeout(() => window.location.href = '/dashboard/dashboard.html', 3000);
                 return;
            }

            console.log(`[INIT Procvičování] User authenticated (ID: ${currentUser.id}). Profile and titles received.`);

            // Now that we have the core data, setup this page's specific UI and load its data
            try {
                setupEventListeners(); // Listeners specific to main.html
                updateCopyrightYear();

                // Initial UI update (redundant with dashboard.js? Maybe skip if dashboard.js does it)
                // updateUserInfoUI(currentProfile, allTitles); // Assumes dashboard.js might not have updated sidebar yet

                await loadPageData(); // Load the specific data for this page's tabs
                ...
                setLoadingState('all', false);
                 ...
                 // Hide main loader if it wasn't hidden by dashboard.js (e.g., if dashboard init finished earlier)
        ...
        // Initial setup that doesn't depend on dashboardReady (like hiding main content)
        if (ui.mainContent) ui.mainContent.style.display = 'none';
        // The initialLoader visibility is handled by dashboard.js

    }
    // --- END MODIFIED ---

    // --- Run ---
    initializeApp(); // Start the initialization listener sequence
