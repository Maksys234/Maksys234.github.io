(function() {
    ...
    // Assuming dashboard.js initializes Supabase and makes 'supabase' globally available or uses a shared instance.
    // If not, dashboard.js needs modification. For now, we try to get it from global scope.
    let supabase = window.supabase?.client || null; // Attempt to get client from global scope
    ...
    const ui = {
        ...
        sidebarOverlay: document.getElementById('sidebar-overlay'), // Needed for closeMenu
        mainContent: document.getElementById('main-content'),
        sidebar: document.getElementById('sidebar'), // Needed for closeMenu
        mainMobileMenuToggle: document.getElementById('main-mobile-menu-toggle'), // Keep mobile toggle if needed specifically here
        sidebarCloseToggle: document.getElementById('sidebar-close-toggle'), // Keep mobile close if needed
        ...
        dashboardTitle: document.getElementById('dashboard-title'), // Might not exist on main.html
        ...
        statsProgress: document.getElementById('stats-completed'),
        statsProgressChange: document.getElementById('stats-completed-change'),
        statsTests: document.getElementById('stats-avg-score'),
        statsTestsAvg: document.getElementById('stats-avg-score-change'),
        statsPoints: document.getElementById('stats-time-spent'),
        statsPointsChange: document.getElementById('stats-time-change'),
        statsStreak: document.getElementById('stats-weakest-topic'),
        statsStreakLongest: document.querySelector('#stats-weakest-topic + .card-description + .card-footer'), // Adjusted selector
    ...
    // --- UI Rendering Functions (Keep) ---
    function renderStatsCards(stats) { /* ... */ if (!stats) { ui.statsCardsContainer?.querySelectorAll('.dashboard-card').forEach(c => { c.classList.add('loading'); c.querySelector('.card-value').textContent = 'ERR'; c.querySelector('.card-description').textContent = 'Data nedostupná'; const footer = c.querySelector('.card-footer'); if(footer) footer.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Chyba'; }); if(ui.statsProgress) ui.statsProgress.textContent = 'ERR'; if(ui.statsProgressChange) ui.statsProgressChange.innerHTML = ''; if(ui.statsTests) ui.statsTests.textContent = 'ERR'; if(ui.statsTestsAvg) ui.statsTestsAvg.innerHTML = ''; if(ui.statsPoints) ui.statsPoints.textContent = 'ERR'; if(ui.statsPointsChange) ui.statsPointsChange.innerHTML = ''; if(ui.statsStreak) ui.statsStreak.textContent = 'ERR'; if(ui.statsStreakLongest) ui.statsStreakLongest.innerHTML = ''; return; } ui.statsCardsContainer?.querySelectorAll('.dashboard-card').forEach(c => c.classList.remove('loading')); if (ui.statsProgress) ui.statsProgress.textContent = stats.completed_exercises ?? 0; if (ui.statsProgressChange) ui.statsProgressChange.innerHTML = `<i class="fas fa-check"></i> Celkový počet`; const avgScore = calculateAverageScore(diagnosticResultsData); if (ui.statsTests) ui.statsTests.textContent = `${avgScore}%`; if (ui.statsTestsAvg) ui.statsTestsAvg.innerHTML = `<i class="fas fa-poll"></i> V testech`; const totalSeconds = stats.total_study_seconds ?? 0; const hours = Math.floor(totalSeconds / 3600); const minutes = Math.floor((totalSeconds % 3600) / 60); if (ui.statsPoints) ui.statsPoints.textContent = `${hours}h ${minutes}m`; if (ui.statsPointsChange) ui.statsPointsChange.innerHTML = `<i class="fas fa-stopwatch"></i> Celkem stráveno`; if (ui.statsStreak) ui.statsStreak.textContent = stats.weakest_topic_name || '-'; if (ui.statsStreakLongest) ui.statsStreakLongest.innerHTML = `<i class="fas fa-atom"></i> Poslední analýza`; }
    ...
    // *** UPDATED: updateUserInfoUI now accepts titlesData ***
    ...
    // --- Setup Event Listeners (Keep, but sidebar listener handled by dashboard.js) ---
    function setupEventListeners() {
        // REMOVED: Listener for sidebarToggleBtn, handled by dashboard.js
    ...
    // --- UPDATED: initializeApp (No Supabase init, get client from global scope) ---
    async function initializeApp() {
        // Get Supabase client (assuming it's initialized in dashboard.js and available globally)
        supabase = window.supabase?.client || null;
        if (!supabase) {
            console.error("Supabase client not found. Ensure dashboard.js initializes it correctly and makes it globally available (e.g., window.supabaseClient = supabase).");
        ...
        setupEventListeners(); // Setup listeners specific to this page
        ...
        try {
            ...
            if (session?.user) {
                ...
                updateUserInfoUI(currentProfile, allTitles); // Update sidebar (expects titles)
                updateCopyrightYear();
