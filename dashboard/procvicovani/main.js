...
(function() { // Start IIFE
    ...
    function renderMessage(container, type = 'info', title, message, addButtons = []) {
        if (!container) { console.error("[RenderMessage] Container not found!"); return; }
		console.log(`[RenderMessage] Rendering into: #${container.id || container.tagName}, Type: ${type}, Title: ${title}`);
		const iconMap = { info: 'fa-info-circle', warning: 'fa-exclamation-triangle', error: 'fa-exclamation-circle', empty: 'fa-box-open' };
		let buttonsHTML = '';
		addButtons.forEach(btn => { buttonsHTML += `<button class="btn ${btn.class || 'btn-primary'}" id="${btn.id}" ${btn.disabled ? 'disabled' : ''}>${btn.icon ? `<i class="fas ${btn.icon}"></i> ` : ''}${sanitizeHTML(btn.text)}</button>`; });
		container.innerHTML = `<div class="empty-state ${type}" style="display: flex;"> <i class="fas ${iconMap[type] || iconMap.info} empty-state-icon"></i> <h3>${sanitizeHTML(title)}</h3> <p>${sanitizeHTML(message)}</p> <div class="action-buttons">${buttonsHTML}</div> </div>`;
	...
	function renderTestSkeletons(container) { /* Placeholder */ }
	...
	function renderTopicSkeletons(container) { /* Placeholder */ }
	...
	function setLoadingState(sectionKey, isLoadingFlag) {
		...
		const loaderMap = { stats: ui.statsCards, shortcuts: ui.shortcutsGrid, plan: ui.studyPlanContainer, notifications: ui.notificationsList };
		const contentMap = { plan: ui.studyPlanContent };
		const emptyMap = { plan: ui.studyPlanEmpty, notifications: ui.noNotificationsMsg };
		const skeletonFnMap = { stats: renderStatsSkeletons, shortcuts: renderShortcutSkeletons, plan: renderPlanSkeletons, notifications: renderNotificationSkeletons };
        const displayTypeMap = { stats: 'grid', shortcuts: 'grid', plan: 'block', notifications: 'block' };
        ...
        if (sectionKey === 'practice-tab') {
            setLoadingState('stats', isLoadingFlag); setLoadingState('shortcuts', isLoadingFlag); return;
        } else if (sectionKey === 'study-plan-tab') {
            setLoadingState('plan', isLoadingFlag); return;
        } else if (sectionKey === 'vyuka-tab') {
            console.log("[SetLoadingState v3] Vyuka tab state (static)."); return;
        } else if(sectionKey === 'goalSelection') {
             console.log("[SetLoadingState v3] Goal selection state:", isLoadingFlag); return;
        } else if (loaderMap[sectionKey] !== undefined) {
             container = loaderMap[sectionKey]; skeletonFn = skeletonFnMap[sectionKey]; emptyStateEl = emptyMap[sectionKey]; contentEl = contentMap[sectionKey]; displayType = displayTypeMap[sectionKey] || 'block';
         } else {
             console.warn(`[SetLoadingState v3] Unknown section key or no UI mapping: '${sectionKey}'`); return;
         }
		...
		if (isLoadingFlag) {
            ...
            if (primaryElement) {
                 if (!primaryElement.querySelector('.loading-skeleton')) primaryElement.innerHTML = ''; // Clear only if not already skeleton
                 primaryElement.style.display = 'none';
                 if (skeletonFn) {
                     skeletonFn(primaryElement); // Render skeletons
                     primaryElement.style.display = displayType; // Show skeletons
                 }
            }
            ...
            // Render functions clear skeletons. Check if empty state should be shown by render function.
		}
	}
	...
	function renderStatsCards(stats) {
		...
		if (!container) { console.error("[Render Stats] Stats container (#stats-cards) not found!"); return; }
		container.innerHTML = '';
		const isEmpty = !stats || ((stats.totalPoints === undefined || stats.totalPoints === 0) && (stats.completedExercises === undefined || stats.completedExercises === 0) && (stats.activeStreak === undefined || stats.activeStreak === 0) && (stats.lastTestScore === undefined || stats.lastTestScore === null));
		if (isEmpty) {
			console.log("[Render Stats] No significant stats data, rendering empty state.");
			renderMessage( container, 'empty', 'Žádné statistiky', 'Zatím zde nejsou žádná data ke zobrazení. Začněte procvičovat!', [{ id: 'startPracticeBtn', text: 'Začít procvičovat', onClick: () => switchActiveTabUI('vyuka-tab'), class: 'btn-primary btn-sm' }] );
            container.style.display = 'block'; // Override potential grid display for empty state
			return;
		}
		const html = `
	...
	function renderStudyPlanOverview(plan, activities, goal) {
        ...
        const container = ui.studyPlanContainer; const contentEl = ui.studyPlanContent; const emptyEl = ui.studyPlanEmpty;
        if (!container || !contentEl || !emptyEl) { console.error("[Render Plan] Study plan UI elements missing!"); return; }
        contentEl.innerHTML = ''; emptyEl.innerHTML = ''; contentEl.style.display = 'none'; emptyEl.style.display = 'none'; container.style.display = 'block';
        if (plan) {
            ...
            contentEl.innerHTML = `
                <div class="plan-summary card" style="margin: 1rem; border: 1px solid var(--border-color-medium); background-color: rgba(var(--dark-purple-accent-rgb), 0.3);">
                    ...
                    <a href="plan.html" class="btn btn-primary btn-sm"> <i class="fas fa-tasks"></i> Zobrazit detail plánu </a>
                </div>
                <div id="main-plan-schedule" style="padding: 0 1rem 1rem 1rem; text-align: center; color: var(--text-muted);"> <p><small>Týdenní přehled aktivit je dostupný v detailu plánu.</small></p> </div>`;
            contentEl.style.display = 'block';
        } else {
            console.log("[Render Plan] No active plan found, rendering empty state message.");
             renderMessage( emptyEl, 'empty', 'Žádný aktivní plán', 'Momentálně nemáte aktivní studijní plán. Pokud jste již absolvoval/a diagnostický test, můžete si <a href="plan.html" onclick="event.preventDefault(); showToast(\'Info\', \'Přejděte na detailní stránku plánu pro vytvoření.\', \'info\'); return false;">vytvořit nový</a>.', [{ id: 'goToTestBtnPlan', text: 'K diagnostickému testu', onClick: () => window.location.href = 'test1.html', class: 'btn-secondary btn-sm' }] );
        }
	...
	function renderShortcutsForGoal(goal, container) {
        ...
        let shortcutsHTML = '';
        const shortcuts = { test: `<a href="test1.html" class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-graduation-cap"></i></div><h3 class="shortcut-title">Diagnostický Test</h3><p class="shortcut-desc">Ověřte své znalosti.</p></a>`, plan: `<a href="plan.html" class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-tasks"></i></div><h3 class="shortcut-title">Studijní Plán</h3><p class="shortcut-desc">Zobrazte personalizovaný plán.</p></a>`, tutor: `<a href="vyuka/vyuka.html" class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-book-open"></i></div><h3 class="shortcut-title">AI Tutor (Výuka)</h3><p class="shortcut-desc">Vysvětlení témat s AI.</p></a>`, nextTopic: `<a href="vyuka/vyuka.html" class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-forward"></i></div><h3 class="shortcut-title">Další Téma Osnovy</h3><p class="shortcut-desc">Pokračujte v učení.</p></a>`, curriculum: `<a href="plan.html" class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-stream"></i></div><h3 class="shortcut-title">Přehled Osnovy</h3><p class="shortcut-desc">Zobrazte témata plánu.</p></a>`, weakness: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce analýzy témat je ve vývoji.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-search"></i></div><h3 class="shortcut-title">Moje Slabiny</h3><p class="shortcut-desc">Témata k zlepšení.</p></button>`, review: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce opakování se připravuje.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-history"></i></div><h3 class="shortcut-title">Opakování</h3><p class="shortcut-desc">Procvičte si starší témata.</p></button>`, explore: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce procházení témat je ve vývoji.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-compass"></i></div><h3 class="shortcut-title">Procházet Témata</h3><p class="shortcut-desc">Vyberte si téma k učení.</p></button>`, random: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce náhodného cvičení se připravuje.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-dumbbell"></i></div><h3 class="shortcut-title">Náhodné Cvičení</h3><p class="shortcut-desc">Rychlé procvičení.</p></button>`, progress: `<a href="../pokrok.html" class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-chart-line"></i></div><h3 class="shortcut-title">Můj Pokrok</h3><p class="shortcut-desc">Sledujte své zlepšení.</p></a>` };
        ...
        if (!shortcutsHTML) {
			...
			renderMessage(container, 'empty', 'Žádné rychlé akce', 'Pro tento cíl nejsou definovány žádné rychlé akce.');
            container.style.display = 'block';
        } else {
            container.innerHTML = shortcutsHTML;
            container.querySelectorAll('.shortcut-card').forEach((card, index) => card.style.setProperty('--animation-order', index + 1));
            container.style.display = 'grid';
        }
    ...
    function renderVyukaTabContent() {
        ...
        if (!container) { console.error("[Render Vyuka] Container #vyuka-tab-content not found!"); return; }
        container.style.display = 'block';
        if (!container.querySelector('.vyuka-section')) {
            container.innerHTML = ` <section class="vyuka-section card"> <h2 class="section-title"><i class="fas fa-person-chalkboard"></i>Výuka s AI</h2> <div class="empty-state" style="display:flex;"> <i class="fas fa-robot empty-state-icon"></i> <h3>AI Výukový Modul</h3> <p>Zde můžete procházet výukové lekce s naším AI tutorem Justaxem. Lekce jsou přizpůsobeny vašemu studijnímu plánu a pokroku.</p> <a href="vyuka/vyuka.html" class="btn btn-primary" style="margin-top: 1rem;"> <i class="fas fa-book-open"></i> Spustit výuku </a> </div> </section> `;
            console.log("[Render Vyuka] Static content added.");
        } else { console.log("[Render Vyuka] Content already exists, skipping render."); }
    }
	...
	async function fetchActiveStudyPlan(userId, goal) { console.log("[Fetch Data Stub] fetchActiveStudyPlan called."); await new Promise(resolve => setTimeout(resolve, 800)); console.log("[Fetch Data Stub] fetchActiveStudyPlan finished delay."); return null; } // Returns null to simulate no plan
	async function fetchPlanActivities(planId, goal) { console.log("[Fetch Data Stub] fetchPlanActivities called."); await new Promise(resolve => setTimeout(resolve, 500)); console.log("[Fetch Data Stub] fetchPlanActivities finished delay."); return []; } // Returns empty array
	...
	async function fetchNotifications(userId, limit) { console.log(`[Notifications Stub] fetchNotifications called for user ${userId}, limit ${limit}.`); setLoadingState('notifications', true); await new Promise(resolve => setTimeout(resolve, 600)); console.log("[Notifications Stub] fetchNotifications finished delay."); const fakeNotifications = []; renderNotifications(0, fakeNotifications); return { unreadCount: 0, notifications: fakeNotifications }; }
	...
	async function markAllNotificationsRead() { console.log(`[Notifications Stub] markAllNotificationsRead.`); setLoadingState('notifications', true); await new Promise(resolve => setTimeout(resolve, 300)); renderNotifications(0, []); }
	...
	async function loadTabData(tabId) {
		...
		try {
			...
			switch (tabId) {
				case 'practice-tab':
					const [statsResult, topicResult] = await Promise.allSettled([ fetchDashboardStats(currentUser.id, currentProfile), fetchTopicProgress(currentUser.id, goal) ]);
				...
				case 'study-plan-tab':
					...
					renderStudyPlanOverview(studyPlanData, planActivitiesData, goal); // Renders into content or empty div
					break;
				case 'vyuka-tab': renderVyukaTabContent(); break; // Static content
				default: console.warn(`[Load Tab Data v30] No specific logic for tab: ${tabId}`); renderMessage(targetContentElement, 'info', 'Obsah se připravuje', `Obsah pro záložku '${tabId}' bude brzy dostupný.`); break;
			}
			success = true; console.log(`%c[Load Tab Data v30] Successfully finished for tab: ${tabId}`, "color: lime;");
		} catch (error) {
			console.error(`[Load Tab Data v30] Error processing tab ${tabId}:`, error);
			if (targetContentElement) renderMessage(targetContentElement, 'error', 'Chyba načítání dat', `Nepodařilo se načíst obsah: ${error.message}`); else showError(`Nepodařilo se načíst data pro ${tabId}: ${error.message}`, true);
		} finally {
	...
	async function loadPageData() { console.log("🔄 [Load Page Data] Starting initial data load sequence..."); hideError(); if (!currentProfile || !currentProfile.learning_goal) { console.error("[Load Page Data] Cannot load page data, profile/goal missing."); if (!currentProfile) showGoalSelectionModal(); else if (ui.tabsWrapper && ui.tabContentContainer) { ui.tabsWrapper.style.display = 'none'; ui.tabContentContainer.style.display = 'none'; } return; } let activeTabId = localStorage.getItem(LAST_ACTIVE_TAB_KEY) || 'practice-tab'; let activeTabButton = document.querySelector(`.content-tab[data-tab="${activeTabId}"]`); if (!activeTabButton || getComputedStyle(activeTabButton).display === 'none') { console.warn(`[Load Page Data] Invalid/hidden last active tab '${activeTabId}', defaulting to 'practice-tab'.`); activeTabId = 'practice-tab'; localStorage.setItem(LAST_ACTIVE_TAB_KEY, activeTabId); } console.log(`[Load Page Data] Loading data for initial active tab: ${activeTabId}`); await loadTabData(activeTabId); initTooltips(); console.log("✅ [Load Page Data] Initial page data loading process complete."); isInitialPageLoadComplete = true; }
	function handleTabSwitch(event) { console.log("[Handle Tab Switch] Click detected."); const targetTabButton = event.currentTarget; const tabId = targetTabButton.dataset.tab; if (!tabId) { console.warn("[Handle Tab Switch] No tabId found on clicked element."); return; } if (currentlyLoadingTabId && currentlyLoadingTabId !== tabId) { showToast('Počkejte prosím', `Data pro záložku '${currentlyLoadingTabId}' se stále načítají.`, 'info', 2500); console.warn(`[Handle Tab Switch] Blocked switch to '${tabId}' while '${currentlyLoadingTabId}' is loading.`); return; } const currentActiveTabButton = document.querySelector('.content-tab.active'); if (targetTabButton === currentActiveTabButton && isInitialPageLoadComplete) { console.log(`[Handle Tab Switch] Tab ${tabId} is already active and loaded. Ignoruji.`); return; } console.log(`[Handle Tab Switch] User requested switch to tab: ${tabId}`); switchActiveTabUI(tabId); loadTabData(tabId); }
	function switchActiveTabUI(tabId) { const targetTabButton = document.querySelector(`.content-tab[data-tab="${tabId}"]`); if (!targetTabButton) { console.warn(`[SwitchActiveTabUI] Tab button for '${tabId}' not found.`); return; } console.log(`[SwitchActiveTabUI] Setting active UI for tab: ${tabId}.`); ui.contentTabs?.forEach(tab => tab.classList.remove('active')); ui.tabContents?.forEach(content => { if (content) { content.classList.remove('active'); content.style.display = 'none'; } }); targetTabButton.classList.add('active'); let activeContentElement = document.getElementById(`${tabId}-content`); if (!activeContentElement) { const contentKey = `${tabId.replace(/-([a-z])/g, (g) => g[1].toUpperCase())}Content`; activeContentElement = ui[contentKey]; } if (activeContentElement) { activeContentElement.classList.add('active'); activeContentElement.style.display = 'block'; } else { console.warn(`[SwitchActiveTabUI] Content area for tab '${tabId}' not found.`); renderMessage(ui.practiceTabContent || document.body, 'error', 'Chyba zobrazení', `Obsah pro záložku '${tabId}' nelze zobrazit.`); } try { localStorage.setItem(LAST_ACTIVE_TAB_KEY, tabId); } catch (e) { console.warn("Could not save last active tab to localStorage:", e); } console.log(`[SwitchActiveTabUI] UI switched to ${tabId}.`); }
	async function handleRefreshClick() { if (!currentUser || !currentProfile) { showToast("Chyba", "Pro obnovení je nutné se přihlásit.", "error"); return; } if (currentlyLoadingTabId) { showToast("PROBÍHÁ SYNCHRONIZACE", `Data pro záložku '${currentlyLoadingTabId}' se již načítají.`, "info"); console.warn("[Refresh] Blocked: A tab is currently loading."); return; } console.log("🔄 Manual refresh triggered..."); const icon = ui.refreshDataBtn?.querySelector('i'); const text = ui.refreshDataBtn?.querySelector('.refresh-text'); if (icon) icon.classList.add('fa-spin'); if (text) text.textContent = 'RELOADING...'; if (ui.refreshDataBtn) ui.refreshDataBtn.disabled = true; const activeTabButton = document.querySelector('.content-tab.active'); const activeTabId = activeTabButton ? activeTabButton.dataset.tab : (localStorage.getItem(LAST_ACTIVE_TAB_KEY) || 'practice-tab'); console.log(`[Refresh] Reloading data for currently active tab: ${activeTabId}`); await loadTabData(activeTabId); if (icon) icon.classList.remove('fa-spin'); if (text) text.textContent = 'RELOAD'; if (ui.refreshDataBtn) ui.refreshDataBtn.disabled = false; console.log("[Refresh] Reload process initiated for active tab."); }
	...
	async function initializeApp() {
		console.log(`[INIT Procvičování] App Init Start v25.0.31...`); // Version bump
		try {
			cacheDOMElements(); // Cache elements first
			if (!initializeSupabase()) { throw new Error("Supabase initialization failed."); }

			// Basic UI setup
			applyInitialSidebarState(); updateCopyrightYear(); initMouseFollower(); initHeaderScrollDetection(); updateOnlineStatus(); initTooltips(); setupBaseEventListeners();
            ...
            // ** ADDED: Explicit check for session error **
            if (sessionError) {
                console.error("[INIT Procvičování] Session Error:", sessionError);
                throw new Error(`Chyba ověření session: ${sessionError.message}`);
            }

			if (session?.user) {
				currentUser = session.user;
				console.log(`[INIT Procvičování] User authenticated (ID: ${currentUser.id}). Fetching profile and titles...`);
				...
				// ** ADDED: More robust profile handling **
                if (profileResult.status === 'rejected') {
                     console.error("[INIT Procvičování] Profile fetch failed:", profileResult.reason);
                     currentProfile = await createDefaultProfile(currentUser.id, currentUser.email); // Attempt creation on fetch fail
                } else {
                     currentProfile = profileResult.value?.data;
                     if (!currentProfile) { // Profile doesn't exist
                          console.warn("[INIT Procvičování] Profile fetch returned no data, creating default...");
				...
				// Check learning goal and configure UI / load data
				const goal = currentProfile.learning_goal;
				...
				if (!goal) {
					...
					if (ui.mainContent) ui.mainContent.style.display = 'flex'; // Show main but hide tabs
					...
					console.log(`[INIT Procvičování] Cíl '${goal}' nastaven. Konfiguruji UI a načítám data...`);
                    ...
                    if(ui.tabContents) { ui.tabContents.forEach(el => {if (el) el.style.display='none';}); } // Hide all content initially before loading active
					configureUIForGoal(); // Configure tabs and initial active tab UI
					await loadPageData(); // Load data ONLY for the active tab
				}

				// Show main content (unless modal is active)
				if (ui.mainContent && (!ui.goalSelectionModal || getComputedStyle(ui.goalSelectionModal).display === 'none')) {
					...
					requestAnimationFrame(() => { if(ui.mainContent) ui.mainContent.classList.add('loaded'); initScrollAnimations(); });
					 ...
					 ui.mainContent.style.display = 'flex'; // Ensure main is visible behind modal
				}

				setupTabEventListeners(); // Setup tab switching listeners

				console.log("✅ [INIT Procvičování] Verze v25.0.31 Initialized.");
			...
			setLoadingState('all', false); // Attempt to reset loading states
		} finally {
            // ** ADDED: Ensure initial loader is ALWAYS hidden in finally block **
			console.log("[INIT Procvičování] Entering finally block.");
			const il = ui.initialLoader;
			if (il) { // Check if loader element exists
                if (!il.classList.contains('hidden')) {
                    il.classList.add('hidden');
                    console.log("[INIT Procvičování] Hiding initial loader via class.");
                }
                // Use setTimeout to ensure display:none happens after transition
				setTimeout(() => {
                    if (il) {
                         il.style.display = 'none';
                         console.log("[INIT Procvičování] Setting initial loader display to none.");
                    }
                 }, 600); // Match transition duration in CSS
			} else {
                 console.warn("[INIT Procvičování] Initial loader element not found in finally block.");
            }
		}
	}
	// --- END: Initialization ---
	...
	function cacheDOMElements() {
		...
		if (notFoundCritical.length > 0) { console.error(`[CACHE DOM v6.3] KRITICKÉ elementy nenalezeny (${notFoundCritical.length}):`, notFoundCritical.join(', ')); throw new Error(`Chyba načítání stránky: Kritické komponenty chybí (${notFoundCritical.join(', ')}).`); } else { console.log("[CACHE DOM v6.3] Všechny kritické elementy nalezeny."); }
		if (notFoundNonCritical.length > 0) console.warn(`[CACHE DOM v6.3] Některé nekritické elementy nenalezeny (${notFoundNonCritical.length}):`, notFoundNonCritical.join(', '));
    ...
    window.VyukaApp = { showToast: showToast }; // Expose helpers if needed globally
