...
// Version: 23.9 - Correct dashboardReady handling, element checks, use event stats
...
(function() { // Start IIFE
    ...
    let isLoading = {
        ...
        plan: false,
        topics: false,
        notifications: false // Keep consistent
    };

    // --- UI Elements Cache ---
    // Declared here, populated in cacheDOMElements
    const ui = {};

    function cacheDOMElements() {
        console.log("[Procvičování Cache DOM] Caching elements...");
        const ids = [
             // Shared elements (expected from dashboard.js context, but cache defensively)
             'sidebar', 'sidebarAvatar', 'sidebarName', 'sidebarUserTitle',
             'notificationBell', 'notificationCount', 'notificationsDropdown', 'notificationsList',
             'noNotificationsMsg', 'markAllReadBtn', 'toastContainer', 'globalError',
            ...
            // Individual stat card elements might not exist, renderStatsCards will handle it
            'shortcuts-grid', // Container for shortcuts
            ...
            'test-stats', 'testsChart', // <<< Chart canvas
            ...
            'main-plan-schedule', // <<< Grid for plan overview
            ...
            'topic-grid' // <<< Grid for topic cards
        ];
        const notFound = [];
        ids.forEach(id => {
            ...
            if (element) {
                ...
                // Only warn for elements expected to be essential for THIS page's logic
                ...
                if (essentialIds.includes(key)) {
                    console.warn(`[Procvičování Cache DOM] Essential element #${id} not found.`);
                }
                notFound.push(id);
    ...
    // Use functions from dashboard.js if available, otherwise provide minimal fallbacks
    const showToast = window.showToast || function(t,m,ty){ console.log(`[Toast Fallback] ${ty}: ${t} - ${m}`); };
    const showError = window.showError || function(m,g){ console.error(`[Error Fallback] Global=${g}: ${m}`); if(g && ui.globalError) { ui.globalError.innerHTML = `<p>${m}</p>`; ui.globalError.style.display = 'block'; } else { console.error(m); } };
    ...
    const renderNotificationSkeletons = window.renderNotificationSkeletons || function(c) { console.warn("renderNotificationSkeletons function not found."); if(ui.notificationsList) ui.notificationsList.innerHTML = `<p>Loading ${c} notifications...</p>`; }
    const renderNotifications = window.renderNotifications || function(c, n) { console.warn("renderNotifications function not found."); if(ui.notificationsList) ui.notificationsList.innerHTML = `<p>Loaded ${n?.length}/${c} notifications.</p>`; }
    ...
    function setLoadingState(section, isLoadingFlag) {
        ...
        if (!config) {
            // Ignore 'shortcuts' or other sections not managed here
        ...
        if (isLoadingFlag) {
            ...
            if (config.skeletonFn && config.content) config.skeletonFn(config.content); // Render skeleton into content area
            else if (config.skeletonFn && config.container && section === 'stats') config.skeletonFn(config.container); // Special case for stats
            else if (config.skeletonFn && config.container && section === 'notifications') config.skeletonFn(2); // Notifications skeleton
        } else {
    ...
    // --- UI Rendering ---
    // --- MODIFIED: renderStatsCards checks for element existence and uses global userStatsData ---
    function renderStatsCards(stats) {
         ...
         initScrollAnimations(); // Animate new cards
     }
    ...
    function renderTestResults(results) {
         ...
         ui.testResultsContent.innerHTML = ''; // Clear previous
         ...
         else { ui.testResultsEmpty.style.display = 'none'; ui.startTestBtnResults.style.display = 'inline-flex'; ui.testResultsContent.style.display = 'block';
             // Calculate Stats
    ...
    function renderStudyPlanOverview(plan, activities) {
         if (!ui.studyPlanContent || !ui.studyPlanEmpty || !ui.startTestBtnPlan || !ui.mainPlanScheduleGrid) { console.warn("Missing elements for renderStudyPlanOverview"); setLoadingState('plan', false); return; }
         console.log("[Render Plan] Rendering plan overview:", plan);
         ui.studyPlanContent.innerHTML = ''; // Clear previous
         ui.mainPlanScheduleGrid.innerHTML = ''; // Clear grid
         ...
         if (!plan) { ui.studyPlanEmpty.style.display = 'block'; ui.startTestBtnPlan.style.display = 'inline-flex'; ui.studyPlanContent.style.display = 'none'; }
             ...
             const daysOrder = [1, 2, 3, 4, 5, 6, 0]; const dayNames = { 0: 'Neděle', 1: 'Pondělí', 2: 'Úterý', 3: 'Středa', 4: 'Čtvrtek', 5: 'Pátek', 6: 'Sobota' };
    ...
    function handleTabSwitch(event) {
        ...
        document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active')); // Use querySelectorAll for safety
        event.currentTarget.classList.add('active');
        const activeContent = document.getElementById(tabId); // Get content by ID
    ...
    async function handleRefreshClick() {
        ...
        const refreshBtn = document.getElementById('refresh-data-btn'); // Use specific ID
    ...
    function setupEventListeners() {
        ...
        // Use safeAddListener pattern or direct checks
        ...
        if(ui.contentTabs) ui.contentTabs.forEach(tab => { safeAddListener(tab, 'click', handleTabSwitch, `tab-${tab.dataset.tab}`); });
        safeAddListener(document.getElementById('refresh-data-btn'), 'click', handleRefreshClick, 'refreshDataBtn'); // Use direct ID
        ...
        else if (ui.mainContent) { ui.mainContent.addEventListener('scroll', handleScroll, { passive: true }); }
    ...
    function initializeApp() {
        ...
        document.addEventListener('dashboardReady', async (event) => {
            console.log("[INIT Procvičování] 'dashboardReady' event received.");
             // --- MODIFIED: More robust data check ---
             if (!event || !event.detail || !event.detail.client || !event.detail.user || !event.detail.profile || !event.detail.stats) {
                 ...
                 const initialLoader = document.getElementById('initial-loader'); // Get loader here
                 if (initialLoader && !initialLoader.classList.contains('hidden')) { initialLoader.classList.add('hidden'); setTimeout(() => { if (initialLoader) initialLoader.style.display = 'none'; }, 300); }
                 return;
             }
             // --- END MODIFIED ---
            ...
            try {
                cacheDOMElements(); // Cache elements AFTER dashboard ensures DOM might be ready
                setupEventListeners();
                ...
                // Initial loader should be handled by dashboard.js dispatching the event
            }
