...
// Version: 25.0.33 - Aggressive loader hiding and deferred UI placeholders.
...
// НОВОЕ: Přidáno základní logování výkonu a pokus o paralelizaci části úvodního načítání.
// НОВОЕ (v25.0.33): Agresivnější skrývání úvodního loaderu a odložení neiniciálních UI funkcí.
...
(function() { // Start IIFE
	...
	function renderShortcutsForGoal(goal, container) {
        startPerformanceTimer('renderShortcutsForGoal');
        console.log(`[Render Shortcuts] Rendering for goal: ${goal}`);
        if (!container) {
            console.warn("[Render Shortcuts] Shortcut container not found.");
            stopPerformanceTimer('renderShortcutsForGoal');
            return;
        }
        container.innerHTML = '';
        let shortcutsHTML = '';
        ...
        // Initially, show the same set of shortcuts for all goals.
        // This set includes the most common/generic actions.
        shortcutsHTML = shortcuts.test + shortcuts.plan + shortcuts.tutor;
        // You can add shortcuts.progress if it's relevant for all.

        if (!shortcutsHTML) {
            console.log("[Render Shortcuts] No shortcuts defined (this should not happen with the new logic).");
	...
	async function fetchDashboardStats(userId, profileData) { startPerformanceTimer('fetchDashboardStats'); console.log("[Fetch Data Stub] fetchDashboardStats called."); await new Promise(resolve => setTimeout(resolve, 700)); console.log("[Fetch Data Stub] fetchDashboardStats finished delay."); const lastTest = diagnosticResultsData?.length > 0 ? diagnosticResultsData[0] : null; const stats = { totalPoints: profileData?.points || 0, completedExercises: profileData?.completed_exercises || 0, activeStreak: profileData?.streak_days || 0, streakRecord: profileData?.streak_record || 0, completedExercisesToday: profileData?.exercises_today || 0, lastTestScore: lastTest?.total_score ?? null, lastTestDate: lastTest?.completed_at ?? null }; stopPerformanceTimer('fetchDashboardStats'); return stats; }
	async function fetchDiagnosticResults(userId, goal) { startPerformanceTimer('fetchDiagnosticResults'); console.log("[Fetch Data Stub] fetchDiagnosticResults called."); await new Promise(resolve => setTimeout(resolve, 1000)); console.log("[Fetch Data Stub] fetchDiagnosticResults finished delay."); stopPerformanceTimer('fetchDiagnosticResults'); return []; }
	async function fetchActiveStudyPlan(userId, goal) { startPerformanceTimer('fetchActiveStudyPlan'); console.log("[Fetch Data Stub] fetchActiveStudyPlan called."); await new Promise(resolve => setTimeout(resolve, 800)); console.log("[Fetch Data Stub] fetchActiveStudyPlan finished delay."); stopPerformanceTimer('fetchActiveStudyPlan'); return null; }
	async function fetchPlanActivities(planId, goal) { startPerformanceTimer('fetchPlanActivities'); console.log("[Fetch Data Stub] fetchPlanActivities called."); await new Promise(resolve => setTimeout(resolve, 500)); console.log("[Fetch Data Stub] fetchPlanActivities finished delay."); stopPerformanceTimer('fetchPlanActivities'); return []; }
	async function fetchTopicProgress(userId, goal) { startPerformanceTimer('fetchTopicProgress'); console.log("[Fetch Data Stub] fetchTopicProgress called."); await new Promise(resolve => setTimeout(resolve, 900)); console.log("[Fetch Data Stub] fetchTopicProgress finished delay."); stopPerformanceTimer('fetchTopicProgress'); return [ { id: 'algebra', name: 'Algebra', progress: 0, last_practiced: null, strength: 'neutral' }, { id: 'geometry', name: 'Geometrie', progress: 0, last_practiced: null, strength: 'neutral' }, { id: 'functions', name: 'Funkce', progress: 0, last_practiced: null, strength: 'neutral' } ]; }
    ...
    async function fetchDemoInfiniteItems() { startPerformanceTimer('fetchDemoInfiniteItems'); console.log(`[InfiniteScroll] Fetching page ${demoCurrentPage}...`); demoIsLoadingMore = true; if (ui.demoInfiniteLoader) ui.demoInfiniteLoader.style.display = 'flex'; await new Promise(resolve => setTimeout(resolve, 1200)); const newItems = []; if (demoCurrentPage > DEMO_MAX_PAGES) { demoAllItemsLoaded = true; console.log("[InfiniteScroll] All demo items loaded."); } else { for (let i = 0; i < DEMO_ITEMS_PER_PAGE; i++) { const itemId = (demoCurrentPage -1) * DEMO_ITEMS_PER_PAGE + i + 1; newItems.push({ id: `demo-${itemId}`, title: `Položka ${itemId}`, description: `Toto je ukázková položka #${itemId} pro demonstraci nekonečného načítání.`, icon: ['fa-cube', 'fa-star', 'fa-puzzle-piece', 'fa-lightbulb', 'fa-flask', 'fa-atom'][itemId % 6] }); } demoCurrentPage++; } if (ui.demoInfiniteLoader) ui.demoInfiniteLoader.style.display = 'none'; demoIsLoadingMore = false; stopPerformanceTimer('fetchDemoInfiniteItems'); return newItems; }
    function renderDemoInfiniteItems(items) { startPerformanceTimer('renderDemoInfiniteItems'); if (!ui.demoInfiniteScrollContainer) { console.error("[InfiniteScroll] Demo items container not found."); stopPerformanceTimer('renderDemoInfiniteItems'); return; } if (!items || items.length === 0) { console.log("[InfiniteScroll] No new demo items to render."); if(demoAllItemsLoaded && ui.demoInfiniteScrollContainer.children.length > 0) { const noMoreMsg = document.createElement('p'); noMoreMsg.textContent = 'Všechny demo položky byly načteny.'; noMoreMsg.style.textAlign = 'center'; noMoreMsg.style.padding = '1rem'; noMoreMsg.style.color = 'var(--text-muted)'; noMoreMsg.style.gridColumn = '1 / -1'; if(!ui.demoInfiniteScrollContainer.querySelector('.no-more-items-msg')) { noMoreMsg.classList.add('no-more-items-msg'); ui.demoInfiniteScrollContainer.appendChild(noMoreMsg); } } stopPerformanceTimer('renderDemoInfiniteItems'); return; } const fragment = document.createDocumentFragment(); items.forEach(item => { const card = document.createElement('div'); card.className = 'shortcut-card card'; card.innerHTML = ` <div class="shortcut-icon" style="background: var(--gradient-info); color: var(--white);"><i class="fas ${item.icon || 'fa-question-circle'}"></i></div> <h3 class="shortcut-title">${sanitizeHTML(item.title)}</h3> <p class="shortcut-desc">${sanitizeHTML(item.description)}</p> `; fragment.appendChild(card); }); ui.demoInfiniteScrollContainer.appendChild(fragment); console.log(`[InfiniteScroll] Rendered ${items.length} demo items.`); stopPerformanceTimer('renderDemoInfiniteItems'); }
    async function handleDemoInfiniteScroll() { if (demoIsLoadingMore || demoAllItemsLoaded) return; const container = ui.practiceTabContent; if (!container) { console.warn("[InfiniteScroll] Scroll container (practiceTabContent) not found."); return; } const nearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < 200; if (nearBottom) { console.log("[InfiniteScroll] Near bottom, loading more items..."); const newItems = await fetchDemoInfiniteItems(); renderDemoInfiniteItems(newItems); } }
    async function initializeDemoInfiniteScroll() { startPerformanceTimer('initializeDemoInfiniteScroll'); console.log("[InfiniteScroll] Initializing demo infinite scroll..."); if (!ui.practiceTabContent || !ui.demoInfiniteScrollContainer) { console.error("[InfiniteScroll] Required elements for demo infinite scroll not found in cache."); stopPerformanceTimer('initializeDemoInfiniteScroll'); return; } demoCurrentPage = 1; demoIsLoadingMore = false; demoAllItemsLoaded = false; ui.demoInfiniteScrollContainer.innerHTML = ''; const existingNoMoreMsg = ui.demoInfiniteScrollContainer.querySelector('.no-more-items-msg'); if (existingNoMoreMsg) existingNoMoreMsg.remove(); const initialItems = await fetchDemoInfiniteItems(); renderDemoInfiniteItems(initialItems); if (demoScrollHandler && ui.practiceTabContent._scrollHandlerAttached) { ui.practiceTabContent.removeEventListener('scroll', demoScrollHandler); ui.practiceTabContent._scrollHandlerAttached = false; console.log("[InfiniteScroll] Removed previous scroll listener."); } demoScrollHandler = handleDemoInfiniteScroll; ui.practiceTabContent.addEventListener('scroll', demoScrollHandler); ui.practiceTabContent._scrollHandlerAttached = true; console.log("[InfiniteScroll] Scroll listener attached to practiceTabContent."); stopPerformanceTimer('initializeDemoInfiniteScroll'); }
	...
	function getGoalDisplayName(goalKey) {
        ...
        return goalMap[goalKey] || goalKey || 'Nenastaveno';
	}
	...
	async function initializeApp() {
		...
		try {
			...
			if (session?.user) {
                ...
                if (profileResult.status === 'rejected') { console.error("[INIT Procvičování] Profile fetch failed:", profileResult.reason); currentProfile = await createDefaultProfile(currentUser.id, currentUser.email); } else { currentProfile = profileResult.value?.data; if (!currentProfile) { console.warn("[INIT Procvičování] Profile fetch returned no data, creating default..."); currentProfile = await createDefaultProfile(currentUser.id, currentUser.email); } }
				...
				console.log("✅ [INIT Procvičování] Verze v25.0.33 Initialized.");
