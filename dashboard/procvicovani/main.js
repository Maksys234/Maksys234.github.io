...
// Version: 25.0.30 - Robust loading state handling & empty states.
// Opraveno: Zajištěno správné ukončení stavu načítání pro záložky "Plán" a "Obecné",
// i když podkladové funkce (stubs) nevrací reálná data. Vylepšeno zobrazení prázdných stavů.
...
(function() { // Start IIFE
	...
	function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        isLoading[sectionKey] = isLoadingFlag; // Update the state *before* checking DOM
		...
		switch (sectionKey) {
			case 'practice-tab': // Special case: handle both stats and shortcuts
                setLoadingState('stats', isLoadingFlag);
                setLoadingState('shortcuts', isLoadingFlag);
                return; // Exit after handling sub-sections
			case 'stats':
			...
			case 'shortcuts':
                ...
                displayType = 'grid';
				break;
			case 'study-plan-tab': // Special case: handle plan container
			...
			case 'notifications':
             ...
             case 'goalSelection':
                ...
                return; // Goal modal handles its own visual state
             case 'vyuka-tab':
                console.log("[SetLoadingState v2] Vyuka tab state (likely static).");
                //isLoading[sectionKey] = isLoadingFlag; // Already set above
		...
		if (isLoadingFlag) {
            ...
            if (primaryElement) {
                 // Only clear if not already showing skeletons (prevents flicker)
                 if (!primaryElement.querySelector('.loading-skeleton')) {
            ...
            // Skeletons are cleared by the rendering functions when they add new content.
            // If the content element exists and has skeletons, clear them IF no real content was added.
            // This requires render functions to signal success/failure or check content themselves.
            // The current structure assumes render functions handle clearing.
            // Example check (might need refinement based on render function behavior):
            // if (primaryElement && primaryElement.querySelector('.loading-skeleton') && primaryElement.children.length === 1) {
            //    primaryElement.innerHTML = ''; // Clear skeletons if nothing else was added
            // }
	...
	function renderStatsCards(stats) {
		...
		// Check if stats object is null, undefined, or effectively empty
		const isEmpty = !stats || (
			(stats.totalPoints === undefined || stats.totalPoints === 0) &&
			(stats.completedExercises === undefined || stats.completedExercises === 0) &&
			(stats.activeStreak === undefined || stats.activeStreak === 0) &&
			(stats.lastTestScore === undefined || stats.lastTestScore === null)
		);

		if (isEmpty) {
			console.log("[Render Stats] No significant stats data, rendering empty state.");
			// Use the new renderMessage helper
			renderMessage(
				container,
				'empty', // Use a neutral/empty type
				'Žádné statistiky',
				'Zatím zde nejsou žádná data ke zobrazení. Začněte procvičovat!',
				[{ id: 'startPracticeBtn', text: 'Začít procvičovat', onClick: () => switchActiveTabUI('vyuka-tab'), class: 'btn-primary btn-sm' }] // Example button
			);
            container.style.display = 'block'; // Ensure grid column span works if renderMessage uses a wrapper div
			return;
		}

        // --- Rendering actual stats cards (remains the same) ---
		const html = `
        ...
        // Skeletons cleared by innerHTML
        // initScrollAnimations(); // Potentially call if needed
	...
	function renderStudyPlanOverview(plan, activities, goal) {
        ...
        const container = ui.studyPlanContainer; // The wrapping card
        const contentEl = ui.studyPlanContent; // The div inside the card where content goes
        const emptyEl = ui.studyPlanEmpty; // The empty state div inside the card

        if (!container || !contentEl || !emptyEl) {
            console.error("[Render Plan] Study plan UI elements missing (container, content, or empty state)!");
            return; // Exit if crucial elements are missing
        }

        // Always clear previous content/empty state from both containers
        contentEl.innerHTML = '';
        emptyEl.innerHTML = '';
        contentEl.style.display = 'none'; // Hide content initially
        emptyEl.style.display = 'none'; // Hide empty state initially
        container.style.display = 'block'; // Ensure the main container card is visible

        if (plan) {
            console.log("[Render Plan] Active plan found, rendering summary.");
            // Render the plan summary into the content element
            contentEl.innerHTML = `
            ...
            contentEl.style.display = 'block'; // Show the content element
        } else {
            console.log("[Render Plan] No active plan found, rendering empty state message.");
            // Use the renderMessage helper to display the empty state within the emptyEl container
             renderMessage(
                 emptyEl,
                 'empty', // Type for styling
                 'Žádný aktivní plán',
                 'Momentálně nemáte aktivní studijní plán. Pokud jste již absolvoval/a diagnostický test, můžete si <a href="plan.html" onclick="VyukaApp.showToast(\'Info\', \'Přejděte na detailní stránku plánu pro vytvoření.\', \'info\'); return false;">vytvořit nový</a>.',
                 [ // Buttons for the empty state
                     { id: 'goToTestBtnPlan', text: 'K diagnostickému testu', onClick: () => window.location.href = 'test1.html', class: 'btn-secondary btn-sm' }
                 ]
             );
             // Note: renderMessage already makes the container (emptyEl) visible.
        }
	...
	function renderShortcutsForGoal(goal, container) {
        ...
        const shortcuts = {
            ...
            weakness: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce analýzy témat je ve vývoji.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-search"></i></div><h3 class="shortcut-title">Moje Slabiny</h3><p class="shortcut-desc">Témata k zlepšení.</p></button>`,
            review: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce opakování se připravuje.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-history"></i></div><h3 class="shortcut-title">Opakování</h3><p class="shortcut-desc">Procvičte si starší témata.</p></button>`,
            explore: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce procházení témat je ve vývoji.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-compass"></i></div><h3 class="shortcut-title">Procházet Témata</h3><p class="shortcut-desc">Vyberte si téma k učení.</p></button>`,
            random: `<button class="shortcut-card card" data-animate onclick="VyukaApp.showToast('Info','Funkce náhodného cvičení se připravuje.','info'); return false;"><div class="shortcut-icon"><i class="fas fa-dumbbell"></i></div><h3 class="shortcut-title">Náhodné Cvičení</h3><p class="shortcut-desc">Rychlé procvičení.</p></button>`,
	...
	// --- START: Data Fetching Stubs (Placeholder - Need Real Implementation) ---
    // IMPORTANT: These functions currently simulate data fetching with delays.
    // For real functionality, they need to be replaced with actual calls
    // to Supabase or another backend service. The fix applied mainly ensures
    // the UI handles the (lack of) data from these stubs gracefully.
	...
	function configureUIForGoal() {
		...
		switchActiveTabUI(activeTabId);

        console.log(`[Configure UI] UI configuration complete.`);
	}

	// ** MODIFIED FUNCTION **: Centralized data loading, includes improved error handling and finally block.
	async function loadTabData(tabId) {
        ...
        if (!currentProfile || !currentProfile.learning_goal) {
            ...
            } else { console.error(`[Load Tab Data v30] Content element '${contentKey}' not found.`); }
            currentlyLoadingTabId = null;
            // *** FIX: Ensure loading state is turned off even if we exit early ***
            setLoadingState(tabId, false);
            return;
        }

        const goal = currentProfile.learning_goal;
		setLoadingState(tabId, true); // Set loading true for the specific tab
		let success = false;
        let targetContentElement = null; // Initialize here

		try {
			targetContentElement = document.getElementById(`${tabId}-content`);
			if (!targetContentElement) {
				// Attempt to find a fallback container if the specific ID fails
				targetContentElement = ui[`${tabId.replace(/-([a-z])/g, (g) => g[1].toUpperCase())}Content`];
				if (!targetContentElement) {
					throw new Error(`Content element for tab '${tabId}' not found (ID: ${tabId}-content or fallback).`);
				} else {
					console.warn(`[Load Tab Data v30] Used fallback container for tab '${tabId}'.`);
				}
			}

            // Clear previous content/skeletons/errors
            // Ensure we don't clear while loading skeletons from setLoadingState
            if (!isLoading[tabId] || !targetContentElement.querySelector('.loading-skeleton')) {
			    targetContentElement.innerHTML = '';
            }
            targetContentElement.style.display = 'block'; // Ensure content area is visible for rendering
			...
			switch (tabId) {
				case 'practice-tab':
					...
					if (statsResult.status === 'fulfilled') {
						...
						renderStatsCards(userStatsData); // Renders into ui.statsCards
					} else {
						console.error(`[Load Tab Data v30] Error fetching stats for ${tabId}:`, statsResult.reason);
						renderMessage(ui.statsCards || targetContentElement, 'error', 'Chyba načítání statistik', statsResult.reason.message);
					}

					if (ui.shortcutsGrid) {
                        setLoadingState('shortcuts', true); // Manage shortcut loading separately
                        renderShortcutsForGoal(goal, ui.shortcutsGrid);
                        setLoadingState('shortcuts', false); // Clear shortcut loading
                    } else { console.warn(`[Load Tab Data v30] Shortcuts grid not found for ${tabId}.`); }

                    if(ui.diagnosticPrompt) await checkUserGoalAndDiagnostic();
					break;

				case 'study-plan-tab':
                    // ** Ensure the correct content element is used **
                    const planContentEl = ui.studyPlanContent; // The inner div for content
                    const planEmptyEl = ui.studyPlanEmpty;     // The inner div for empty state
					...
					renderStudyPlanOverview(studyPlanData, planActivitiesData, goal); // This function now handles content/empty rendering
					break;

				case 'vyuka-tab':
                    renderVyukaTabContent(); // Renders static content
					break;

				default:
					console.warn(`[Load Tab Data v30] No specific data loading logic defined for tab: ${tabId}`);
                    renderMessage(targetContentElement, 'info', 'Obsah se připravuje', `Obsah pro záložku '${tabId}' bude brzy dostupný.`);
					break;
			}
			success = true;
			console.log(`%c[Load Tab Data v30] Successfully finished loading and rendering for tab: ${tabId}`, "color: lime;");

		} catch (error) {
			console.error(`[Load Tab Data v30] Error processing tab ${tabId}:`, error);
			// Render error message in the specific tab's content area if possible
			if (targetContentElement) {
                 renderMessage(targetContentElement, 'error', 'Chyba načítání dat', `Nepodařilo se načíst obsah pro tuto záložku: ${error.message}`);
			} else {
				// Fallback to global error if the content element wasn't found
            ...
            // *** CRITICAL FIX: Ensure loading state is ALWAYS turned off ***
			setLoadingState(tabId, false);
	...
	async function loadPageData() {
        ...
        if (!activeTabButton || getComputedStyle(activeTabButton).display === 'none') {
            ...
            activeTabId = 'practice-tab';
            localStorage.setItem(LAST_ACTIVE_TAB_KEY, activeTabId);
        }
	...
	function handleTabSwitch(event) {
        ...
        switchActiveTabUI(tabId);
        loadTabData(tabId); // Load data for the new tab
	}
	...
	function switchActiveTabUI(tabId) {
        ...
        // ** Ensure the correct content element is targeted **
        let activeContentElement = document.getElementById(`${tabId}-content`);
        if (!activeContentElement) {
            // Fallback check using ui cache if ID fails
            const contentKey = `${tabId.replace(/-([a-z])/g, (g) => g[1].toUpperCase())}Content`;
            activeContentElement = ui[contentKey];
        }
        ...
        if (activeContentElement) {
            ...
            console.warn(`[SwitchActiveTabUI] Content area for tab '${tabId}' not found (tried ID and cache).`);
            // Display error in a default/fallback area if possible
	...
	async function initializeApp() {
		try {
			console.log(`[INIT Procvičování] App Init Start v25.0.30...`);
			cacheDOMElements();
	...
	function cacheDOMElements() {
		...
		const elementDefinitions = [
            ...
            { key: 'practiceTabContent', id: 'practice-tab-content', critical: true }, // Content area for Obecné
            { key: 'studyPlanTabContent', id: 'study-plan-tab-content', critical: true }, // Content area for Plán (inner div)
            { key: 'vyukaTabContent', id: 'vyuka-tab-content', critical: true }, // Content area for Výuka
			...
			{ key: 'statsCards', id: 'stats-cards', critical: false }, // Container for Obecné stats
			{ key: 'shortcutsGrid', id: 'shortcuts-grid', critical: false }, // Container for Obecné shortcuts
			{ key: 'studyPlanContainer', id: 'study-plan-container', critical: false }, // Parent card for Plán
			{ key: 'studyPlanContent', id: 'study-plan-content', critical: false }, // Div inside Plán card for content
			{ key: 'studyPlanEmpty', id: 'study-plan-empty', critical: false }, // Div inside Plán card for empty state
		...
		elementDefinitions.forEach(def => {
			...
			if (element) { ui[def.key] = element; } else { ui[def.key] = null; if (def.critical) notFoundCritical.push(`${def.key} (${def.id || def.query})`); else notFoundNonCritical.push(`${def.key} (${def.id || def.query})`); }
		});
    ...
    function setupTabEventListeners() {
         ...
         const tabs = ui.contentTabs;
         if (tabs && tabs.length > 0) {
             tabs.forEach(tab => {
                 if (tab._tabClickHandler) tab.removeEventListener('click', tab._tabClickHandler);
    ...
    // Make helper functions globally accessible if needed by other scripts (like onclick handlers)
    window.VyukaApp = {
        showToast: showToast // Expose showToast if used in onclick attributes (like in shortcuts)
        // Add other functions here if they are called directly from HTML attributes
    };
