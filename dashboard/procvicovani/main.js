...
// Version: 25.0.12 - Stricter goal prioritization, ensuring UI updates after data.
// Cíl z DB má absolutní přednost. Pokud není, zkusí localStorage. Pokud ani tam, zobrazí modal.
...
(function() { // Start IIFE
            ...
            if (loading) {
                ...
                setTimeout(() => {
                    if (key === 'stats' && ui.statsCards) {
                        if (ui.statsCards.querySelector('.dashboard-card:not(.loading-skeleton)')) { // Check if there's actual content
                            ui.statsCards.style.display = 'grid';
                        } else if(ui.statsCards.innerHTML.trim() === ''){ // If empty after clearing skeletons
                             // ui.statsCards.innerHTML = '<p class="empty-state-text">Statistiky nejsou k dispozici.</p>'; // Example empty state
                        }
                    }
                    else if (content && empty) {
                        const hasActualContent = content.innerHTML.trim() !== '' && !content.querySelector('.loading-skeleton');
                        ...
                        if (container && container.querySelector('.shortcut-card:not(.loading-skeleton)')) { // Check for actual content
                            container.style.display = 'grid';
                        } else if (container) {
                            // container.innerHTML = '<p class="empty-state-text" style="grid-column: 1 / -1; text-align:center;">Žádné zkratky.</p>';
                        }
                    }
                }, 50); // Malé zpoždění, aby se zajistilo, že DOM je aktualizován
            }
        };
    ...
    async function initializeApp() {
        try {
            console.log(`[INIT Procvičování] App Init Start v25.0.11...`); // Verze
            cacheDOMElements();
            ...
            if (session?.user) {
                ...
                if (!goal) { // Pokud není cíl v DB
                    ...
                    if (storedGoal) { // Zkusíme localStorage
                        ...
                        goal = currentProfile.learning_goal; // Znovu načteme cíl, mohl se uložit
                    }
                } else {
                     console.log(`[INIT Procvičování] Cíl '${goal}' již nastaven v profilu (DB).`);
                     // Můžeme zvážit smazání z localStorage, aby DB byla jediným zdrojem pravdy
                ...
                if (!goal) { // Pokud cíl stále není (ani z DB, ani se nepodařilo uložit z localStorage)
                    ...
                    if(ui.goalSelectionModal) ui.goalSelectionModal.style.display = 'none';
                    if(ui.tabsWrapper) ui.tabsWrapper.style.display = 'flex';
                    configureUIForGoal(); // Tento zavolá switchActiveTab, který zavolá loadTabData
                }
