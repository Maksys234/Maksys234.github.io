...
// Version: 24.10.6 - Refined initial tab activation, notification element checks, and stats rendering.
...
(function() { // Start IIFE
    ...
    function cacheDOMElements() {
        ...
        const ids = [
            ...
            'notifications-list', 'no-notifications-msg',
            // NEW_FUNCTION_START - Проверяем правильный ID для кнопки "Vymazat vše"
            'mark-all-read-btn' // ID из HTML должен быть 'mark-all-read-btn'
            // NEW_FUNCTION_END
        ];
        ...
        // NEW_FUNCTION_START - Логирование для элементов уведомлений
        if (!ui.notificationBell) console.warn("[CACHE DOM v3] Notification bell not found!");
        if (!ui.notificationCount) console.warn("[CACHE DOM v3] Notification count element not found!");
        if (!ui.notificationsList) console.warn("[CACHE DOM v3] Notifications list not found!");
        if (!ui.noNotificationsMsg) console.warn("[CACHE DOM v3] No notifications message element not found!");
        if (!ui.markAllReadBtn) console.warn("[CACHE DOM v3] Mark all read button (#mark-all-read-btn) not found!"); // Проверяем новый ID
        // NEW_FUNCTION_END
    ...
    function formatDate(dateString) { /* ... без изменений ... */ try { return dateString ? new Date(dateString).toLocaleDateString('cs-CZ') : '-'; } catch (e) { return '-'; } }
    function formatTime(seconds) { /* ... без изменений ... */ if (isNaN(seconds) || seconds < 0) return '--:--'; const m = Math.floor(seconds / 60); const ss = Math.round(seconds % 60); return `${m.toString().padStart(2, '0')}:${ss.toString().padStart(2, '0')}`; }
    ...
    // NEW_FUNCTION_START - Добавлена проверка ui.statsCards в секции stats
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        const updateSingleSection = (key, loading) => {
            ...
            if (loading) {
                ...
                setTimeout(() => {
                    if (key === 'stats' && ui.statsCards) { // Специальная обработка для stats
                        if (ui.statsCards.innerHTML.trim() === '' || ui.statsCards.querySelectorAll('.loading-skeleton').length > 0) {
                            // Если пусто или все еще скелетоны, значит renderStatsCards не отработал с данными
                            // Можно либо показать empty state для статистики, либо оставить скелетоны (сейчас renderStatsCards их заменит на N/A)
                            // renderStatsCards сама уберет класс loading
                        } else {
                    ...
                    } else if (content && (key === 'shortcuts')) { // shortcuts не имеют empty state
    ...
    async function fetchDashboardStats(userId, profileData) { /* ... без изменений ... */ console.log("[Fetch Data Stub] fetchDashboardStats called. Returning placeholder data."); await new Promise(resolve => setTimeout(resolve, 700)); return { totalPoints: profileData?.points || 0, completedExercises: profileData?.completed_exercises || 0, activeStreak: profileData?.streak_days || 0, lastTestScore: diagnosticResultsData.length > 0 ? diagnosticResultsData[0].total_score : null, }; }
    async function fetchDiagnosticResults(userId, goal) { /* ... без изменений ... */ console.log("[Fetch Data Stub] fetchDiagnosticResults called. Returning empty array."); await new Promise(resolve => setTimeout(resolve, 1000)); return []; }
    async function fetchActiveStudyPlan(userId, goal) { /* ... без изменений ... */ console.log("[Fetch Data Stub] fetchActiveStudyPlan called. Returning null."); await new Promise(resolve => setTimeout(resolve, 800)); return null; }
    async function fetchPlanActivities(planId, goal) { /* ... без изменений ... */ console.log("[Fetch Data Stub] fetchPlanActivities called. Returning empty array."); await new Promise(resolve => setTimeout(resolve, 500)); return []; }
    async function fetchTopicProgress(userId, goal) { /* ... без изменений ... */ console.log("[Fetch Data Stub] fetchTopicProgress called. Returning placeholder data."); await new Promise(resolve => setTimeout(resolve, 900)); return [ { id: 'algebra', name: 'Algebra', progress: 0, last_practiced: null, strength: 'neutral' }, { id: 'geometry', name: 'Geometrie', progress: 0, last_practiced: null, strength: 'neutral' }, { id: 'functions', name: 'Funkce', progress: 0, last_practiced: null, strength: 'neutral' } ]; }
    // --- Конец Загрузки Данных ---

    // --- Рендеринг UI (Заглушки) ---
    // NEW_FUNCTION_START - Улучшенный renderStatsCards для избежания "бесконечной загрузки"
    function renderStatsCards(stats) {
        ...
        // Заполняем данными, если они есть, или N/A
        ...
        `;
        ui.statsCards.classList.remove('loading'); // Важно убрать класс loading
        setLoadingState('stats', false); // Завершаем состояние загрузки для этой секции
    }
    // NEW_FUNCTION_END
    ...
    function handleTabSwitch(eventOrTabId) {
        ...
        if (typeof eventOrTabId === 'string') {
            ...
            if (!targetTabButton) {
                console.warn(`[Tabs] Programmatic switch: Tab button for '${tabId}' not found.`);
                return;
            }
            ...
            console.warn("[Tabs] Invalid argument for handleTabSwitch:", eventOrTabId);
            return;
        }

        // Предотвращаем избыточные вызовы, если вкладка уже активна (кроме первоначальной загрузки)
        if (!isInitialTabSwitch && targetTabButton.classList.contains('active')) {
            console.log(`[Tabs] Tab ${tabId} is already active. Switch ignored.`);
            return;
        }
        isInitialTabSwitch = false; // Сбрасываем флаг после первого фактического переключения
    ...
    async function fetchNotifications(userId, limit) {
        ...
        // NEW_FUNCTION_START - Имитируем загрузку данных, чтобы UI обновился
        const fakeNotifications = []; // Возвращаем пустой массив, как если бы данных не было
        renderNotifications(0, fakeNotifications); // Вызываем render с пустыми данными
        // NEW_FUNCTION_END
    ...
    function renderNotifications(count, notifications) {
        ...
        // NEW_FUNCTION_START - Проверяем наличие всех UI элементов перед использованием
        if (!ui.notificationBell || !ui.notificationCount || !ui.notificationsList || !ui.noNotificationsMsg || !ui.markAllReadBtn) {
            console.error("[Notifications Stub] UI elements for notifications are missing. Cannot render.");
    ...
    function setupEventListeners() {
        ...
        const safeAddListener = (element, eventType, handler, key) => {
            if (element) {
                ...
                // NEW_FUNCTION_START - Более мягкое предупреждение, если элемент не критичен
                const nonCriticalMissing = ['markAllReadBtn']; // Добавьте сюда ID некритичных элементов
                if (nonCriticalMissing.includes(key)) {
                     console.log(`[SETUP v3] Non-critical element not found for listener: ${key}. Это может быть нормально.`);
                } else {
        ...
        const safeAddListenerToAll = (elementsNodeList, eventType, handler, key) => { /* ... без изменений ... */ };
        ...
        // NEW_FUNCTION_START - Проверяем ui.markAllReadBtn перед добавлением слушателя
        if (ui.markAllReadBtn) {
            safeAddListener(ui.markAllReadBtn, 'click', markAllNotificationsRead, 'markAllReadBtn');
        } else {
            console.warn("[SETUP v3] Mark all read button not found, listener not attached.");
        }
        // NEW_FUNCTION_END
        ...
        safeAddListener(ui.notificationBell, 'click', (event) => {
            ...
            if (ui.notificationsDropdown?.classList.contains('active') && ui.notificationsList?.innerHTML.trim() === '' && !isLoading.notifications) {
                fetchNotifications(currentUser?.id, NOTIFICATION_FETCH_LIMIT); // Заглушка вызовет renderNotifications
            }
        }, 'notificationBell');
        ...
        try {
            ...
            if (session?.user) {
                ...
                if (!goal) {
                    ...
                    // NEW_FUNCTION_START - Активируем вкладку "Přehled" (practice-tab) один раз
                    const practiceTabButton = document.querySelector('.content-tab[data-tab="practice-tab"]');
                    if (practiceTabButton && !practiceTabButton.classList.contains('active')) {
                        handleTabSwitch({ currentTarget: practiceTabButton });
                        console.log("[INIT Procvičování] 'practice-tab' activated via handleTabSwitch on init.");
                        isInitialTabSwitch = false; // Сбрасываем флаг, чтобы дальнейшие вызовы handleTabSwitch работали как обычно
                    } else if (practiceTabButton && practiceTabButton.classList.contains('active')) {
                         console.log("[INIT Procvičování] 'practice-tab' is already active or forced active by previous step.");
                         // Убедимся, что контент видим, если вкладка уже активна
                         const practiceTabContent = document.getElementById('practice-tab');
                         if(practiceTabContent) practiceTabContent.style.display = 'block';
                         isInitialTabSwitch = false;
                    }
                     else {
                        console.warn("[INIT Procvičování] Could not find practice-tab button to ensure activation.");
                    }
                    // NEW_FUNCTION_END
