...
// Version: 25.0.11 - Prioritized profile goal over localStorage, reviewed setLoadingState logic.
// Cíl z profilu má přednost. Revize logiky pro zobrazení obsahu po načtení.
...
(function() { // Start IIFE
            ...
            if (loading) {
                ...
                setTimeout(() => {
                    if (key === 'stats' && ui.statsCards) { if (ui.statsCards.querySelectorAll('.dashboard-card:not(.loading-skeleton)').length > 0) ui.statsCards.style.display = 'grid'; }
                        ...
                        const hasActualContent = content.innerHTML.trim() !== '' && !content.querySelector('.loading-skeleton'); // Zkontrolujeme, zda tam nejsou jen skeletony
    ...
    function configureUIForGoal() {
        ...
        if (activeTabButton) {
            ...
            isInitialTabLoad = true; // Reset pro první načtení po konfiguraci
    ...
    async function initializeApp() {
        try {
            console.log(`[INIT Procvičování] App Init Start v25.0.10...`); // Verze
            cacheDOMElements();
            ...
            if (session?.user) {
                ...
                if (currentProfile.learning_goal) {
                    ...
                    // Odstraníme cíl z localStorage, pokud existuje, protože DB má přednost
                    ...
                    if (storedGoal) {
                        console.log(`[INIT Procvičování] Nalezen cíl '${storedGoal}' v localStorage, ukládání do profilu (protože v DB není).`);
                        ...
                        // Po úspěšném uložení do DB by se měl currentProfile.learning_goal aktualizovat
                    }
                }
                // --- Konec upravené logiky ---

                const goal = currentProfile.learning_goal; // Znovu načteme cíl z (potenciálně aktualizovaného) profilu
                if (!goal) {
                    console.log("[INIT Procvičování] Cíl nenastaven (ani v DB, ani v localStorage po uložení), zobrazuji modální okno.");
                    showGoalSelectionModal();
