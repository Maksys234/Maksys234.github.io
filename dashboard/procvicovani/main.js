...
// Version: 25.0.15 - Added missing cacheDOMElements function definition.
// Přidána chybějící definice funkce cacheDOMElements.
...
(function() { // Start IIFE
    ...
    // Объект ui будет заполнен функцией cacheDOMElements
    const ui = {};
    ...
    function configureUIForGoal() {
        if (!currentProfile || !currentProfile.learning_goal) {
            ...
            // Hide tabs wrapper if no goal is set yet
        ...
        if (!ui || Object.keys(ui).length === 0 || !ui.contentTabs || !ui.tabContents) {
            console.error("[UI Config v6.2] UI cache or tab elements are missing. Attempting re-cache.");
            cacheDOMElements();
        ...
        // --- Tab Visibility Logic MODIFIED ---
        // Ensure the main tab wrapper is visible now that a goal is selected
        if (ui.tabsWrapper) ui.tabsWrapper.style.display = 'flex';
        if (ui.tabContents.length > 0) { // Ensure tab content container is ready
            const container = ui.tabContents[0].parentElement; // Get the container
            if(container) container.style.display = 'flex'; // Use flex if that's the layout
        }


        // Define the ALWAYS visible tabs
        ...
        if (ui.contentTabs && ui.contentTabs.length > 0) {
            ui.contentTabs.forEach(tabButton => {
                ...
                if (alwaysVisibleTabs.includes(tabId)) {
                    tabButton.style.display = 'flex'; // Make sure it's visible
                } else {
                    // Hide any other tabs (like the old 'topic-analysis-tab' if it still exists in HTML)
        ...
        // Determine and set the active tab (no changes needed here, but ensure it selects a VISIBLE tab)
        ...
        // If the stored/default tab is now hidden, default to the first *visible* tab ('practice-tab' should always be visible)
        ...
        if (activeTabButton) {
             ...
             isInitialTabLoad = true; // Set flag for initial load after configuration
             switchActiveTab(activeTabId); // Switch UI and trigger data load
        } else {
             console.error("[UI Config v6.2] Failed to find a suitable active tab (even practice-tab).");
        }
    ...
    /**
     * Loads data for the specified tab ID.
     * This function now includes a case for 'vyuka-tab'.
     */
    async function loadTabData(tabId) {
        // Convert tab ID to camelCase for accessing ui cache (e.g., 'test-results-tab' -> 'testResultsTab')
        const camelCaseKey = tabId.replace(/-([a-z])/g, (g) => g[1].toUpperCase());
        const contentKey = `${camelCaseKey}Content`; // Key for the content element in ui cache

        if (!currentProfile || !currentProfile.learning_goal) {
            console.warn(`[Load Tab Data v6.2] Cannot load data for tab '${tabId}', missing profile or goal.`);
            const contentElement = ui[contentKey];
            if (contentElement) {
                 // Show message prompting goal selection if profile/goal missing
            ...
            } else { console.error(`[Load Tab Data v6.2] Content element '${contentKey}' not found for 'missing goal' message.`); }
            return;
        }
        ...
        const sectionKey = tabIdToSectionKey(tabId); // Map tab ID to loading state key
         if (sectionKey !== 'none') setLoadingState(sectionKey, true); // Set loading state if applicable
        ...
        try {
            ...
            // Display skeletons while data loads (except for 'vyuka-tab' for now)
            if (tabId !== 'vyuka-tab') {
                 switch (tabId) {
                     case 'practice-tab':
                     ...
                     case 'study-plan-tab':
                         ...
                         break;
                     // case 'topic-analysis-tab': // Removed
            ...
            switch (tabId) {
                case 'practice-tab':
                    ...
                    renderStatsCards(userStatsData); // Replaces skeleton
                    if (ui.shortcutsGrid) renderShortcutsForGoal(goal, ui.shortcutsGrid); // Replaces skeleton
                    if(ui.diagnosticPrompt) await checkUserGoalAndDiagnostic(); // Check if diagnostic needed
                    break;
                case 'test-results-tab':
                    diagnosticResultsData = await fetchDiagnosticResults(currentUser.id, goal);
                    renderTestResults(diagnosticResultsData, goal); // Replaces skeleton/shows empty
                    break;
                case 'study-plan-tab':
                    ...
                    renderStudyPlanOverview(studyPlanData, planActivitiesData, goal); // Replaces skeleton/shows empty
                    break;
                // --- ADDED Case for vyuka-tab ---
                case 'vyuka-tab':
                    // For now, just ensure the placeholder content added in HTML is displayed
                    // If the placeholder wasn't added or got cleared, add it here.
                    if (!targetContentElement.querySelector('h3')) { // Simple check if placeholder exists
                    ...
                    // No async data loading here yet, so set loading state false immediately if it was set
                    if (sectionKey !== 'none') setLoadingState(sectionKey, false);
                    break;
                // --- END ADDED Case ---
                default:
             ...
             // Ensure loading state is stopped if not handled by specific render function
            ...
            const contentEl = ui[contentKey];
             if (contentEl) { // Show error in the specific tab
              ...
              if (sectionKey !== 'none') setLoadingState(sectionKey, false); // Stop loading on error
        }
    }
    ...
    function tabIdToSectionKey(tabId) {
        switch (tabId) {
            case 'practice-tab': return 'stats'; // Represents the 'Obecné' tab's main data
            case 'test-results-tab': return 'tests';
            case 'study-plan-tab': return 'plan';
             case 'vyuka-tab': return 'none'; // No loading state needed for static content yet
            // case 'topic-analysis-tab': return 'topics'; // Removed
            default: return 'all'; // Fallback or handle as needed
        }
    }

    // Loads initial page data based on the active tab
    async function loadPageData() {
        if (!currentProfile) { console.error("[Load Page Data v6.2] Cannot load page data, profile missing."); setLoadingState('all', false); return; }
        ...
        // Determine active tab (considering visibility)
        let activeTabId = localStorage.getItem('lastActiveProcvicovaniTab') || 'practice-tab';
        let activeTabButton = document.querySelector(`.content-tab[data-tab="${activeTabId}"]`);
    ...
    function handleTabSwitch(eventOrTabId) {
        ...
        if (targetTabButton.classList.contains('active') && !isInitialTabLoad) { console.log(`[Tabs v6.2] Tab ${tabId} is already active.`); return; }
        ...
        // Load data for the newly activated tab (only if not initial load)
        if (!isInitialTabLoad) {
            loadTabData(tabId);
        }
        isInitialTabLoad = false; // Clear flag after first switch
    }
    ...
    // --- END: Core Application Logic ---
     ...
     function cacheDOMElements() {
         ...
         const elementDefinitions = [
             // Critical Elements (essential for core functionality)
             ...
             { key: 'tabsWrapper', id: 'tabs-wrapper', critical: true }, // Needed to show/hide tabs container
             { key: 'practiceTabContent', id: 'practice-tab-content', critical: true }, // Default visible content
             { key: 'goalSelectionModal', id: 'goal-selection-modal', critical: true }, // Essential for first-time users
             { key: 'goalStep1', id: 'goal-step-1', critical: true }, // First step of modal

             // Non-Critical Elements (used by features, app can partially function without them)
             ...
             { key: 'markAllReadBtn', id: 'mark-all-read-btn', critical: false }, // ID was corrected here if different previously
             ...
             { key: 'startTestBtnPlan', id: 'startTestBtnPlan', critical: false }, // Ensure correct ID
             { key: 'createPlanBtnEmpty', id: 'create-plan-btn-empty', critical: false }, // Ensure correct ID
             { key: 'mainPlanSchedule', id: 'main-plan-schedule', critical: false },
             //{ key: 'topicAnalysisContainer', id: 'topic-analysis-container', critical: false }, // Removed as per requirement
             //{ key: 'topicAnalysisLoading', id: 'topic-analysis-loading', critical: false }, // Removed
             //{ key: 'topicAnalysisContent', id: 'topic-analysis-content', critical: false }, // Removed
             //{ key: 'topicAnalysisEmpty', id: 'topic-analysis-empty', critical: false }, // Removed
             //{ key: 'startTestBtnAnalysis', id: 'start-test-btn-analysis', critical: false }, // Removed
             //{ key: 'topicGrid', id: 'topic-grid', critical: false }, // Removed
             ...
             // Tab Content Elements (ensure IDs match HTML)
             ...
             { key: 'vyukaTabContent', id: 'vyuka-tab-content', critical: true }, // Added new tab content
             //{ key: 'topicAnalysisTabContent', id: 'topic-analysis-tab-content', critical: false } // Removed
         ];
         ...
         elementDefinitions.forEach(def => {
             ...
             if (element) {
                 ui[def.key] = element; // Assign to the global ui object
             } else {
                 ui[def.key] = null; // Assign null if not found
                 if (def.critical) {
         ...
         // --- Post-Caching Checks and Warnings ---
         if (notFoundCritical.length > 0) {
             ...
             // Throw an error to stop initialization if critical elements are missing
         ...
         if (!ui.vyukaTabContent) console.warn("[CACHE DOM v6.2] Nový element ui.vyukaTabContent chybí (ID: vyuka-tab-content).");
    ...
    async function initializeApp() {
        try {
            ...
            if (session?.user) {
                ...
                if (!goal) {
                    ...
                    if (ui.tabContents && ui.tabContents.length > 0) { ui.tabContents.forEach(el => {if(el) el.style.display='none';}); }
                     ...
                     if(ui.tabsWrapper) ui.tabsWrapper.style.display = 'flex'; // Show tabs wrapper as goal exists
                      if(ui.tabContents) {
                          const container = ui.tabContents[0]?.parentElement; // Get the container
                          if(container) container.style.display = 'flex'; // Show tab content container
                      }
            ...
            if (ui.initialLoader && !ui.initialLoader.classList.contains('hidden')) {
                 ...
                 // Use showError for global error display if initial loader is gone
                 showError(`Chyba inicializace: ${error.message}`, true);
             }
            if (ui.mainContent) ui.mainContent.style.display = 'flex'; // Show main content area to display global error
            setLoadingState('all', false); // Turn off any loading indicators
        } finally {
            // Ensure initial loader is always hidden eventually
