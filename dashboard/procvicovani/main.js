...
// Version: 25.0.12 - Added missing skeleton function definitions.
// Doplněny chybějící definice funkcí pro vykreslování skeletonů.
...
(function() { // Start IIFE
    ...
    // --- START: Skeleton Rendering Functions (KONEČNĚ DOPLNĚNÉ!) ---
    ...
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        const updateSingleSection = (key, loading) => {
            if (typeof isLoading[key] === 'undefined') {
                console.warn(`[Procvičování UI Loading v6.2] Neznámý dílčí klíč sekce '${key}' při zpracování 'all'.`);
                return;
            }
            ...
            const sectionMap = {
                ...
                notifications: { skeletonFn: renderNotificationSkeletons }, // Added skeleton for consistency
                goalSelection: { }
            };
            ...
            if (loading) {
                ...
                if (skeletonFn && typeof skeletonFn === 'function') {
                    // Determine target for skeletons: notifications have a special list
                    let targetContainer;
                    ...
                    else console.warn(`[Procvičování UI Loading v6.2] Target container for skeletons not found for key '${key}'.`);
                } else if (skeletonFn) {
                    console.warn(`[Procvičování UI Loading v6.2] skeletonFn pro klíč '${key}' není funkce.`);
                }
            } else {
                // Clear skeletons slightly delayed to allow rendering to finish
                setTimeout(() => {
                     ...
                     const clearSkeletons = (el) => { el?.querySelectorAll(skeletonSelector).forEach(skel => skel.remove()); el?.classList.remove('loading'); el?.parentElement?.classList.remove('loading');}; // Also remove loading from parent if applicable

                    if (key === 'notifications' && ui.notificationsList) {
                        clearSkeletons(ui.notificationsList);
                        // Logic to show list/empty msg is handled by renderNotifications
                    } else if (content) {
                        ...
                        if (container.children.length === 0 && container.innerHTML.trim() === '') {
                            // Optionally add an empty state if container is truly empty after render
                            // container.innerHTML = '<p class="empty-state-text">Data nejsou k dispozici.</p>';
                        } else if (key === 'stats' || key === 'shortcuts') {
                            container.style.display = 'grid'; // Ensure grid display is restored
                        }
                    }
                }, 50); // Small delay might help rendering race conditions
            }
        };
    ...
    async function initializeApp() {
        try {
            console.log(`[INIT Procvičování] App Init Start v25.0.12...`); // Verze
            cacheDOMElements();
            ...
            if (session?.user) {
                ...
                if (!goal) { // Pokud není cíl v DB
                    ...
                    console.log(`[INIT Goal Check] Cíl z localStorage: ${storedGoal}`);
                    if (storedGoal) { // Zkusíme localStorage
                        console.log(`[INIT Procvičování] Cíl není v DB. Nalezen cíl '${storedGoal}' v localStorage, ukládání do profilu.`);
                        let storedGoalDetails = JSON.parse(localStorage.getItem(GOAL_DETAILS_KEY) || '{}');
                        // Pozor: saveGoalAndProceed aktualizuje currentProfile, takže goal se načte níže
                        await saveGoalAndProceed(storedGoal, Object.keys(storedGoalDetails).length > 0 ? storedGoalDetails : null);
                        goal = currentProfile.learning_goal; // Znovu načteme cíl po uložení
                        console.log(`[INIT Goal Check] Cíl po uložení z localStorage: ${goal}`);
                    }
                } else {
                     console.log(`[INIT Procvičování] Cíl '${goal}' již nastaven v profilu (DB).`);
                     // Volitelně smazat localStorage, pokud chcete, aby DB byla jediný zdroj
                     // localStorage.removeItem(LEARNING_GOAL_KEY);
                     // localStorage.removeItem(GOAL_DETAILS_KEY);
                }
                // --- Konec upravené logiky ---

                if (!goal) { // Pokud cíl stále není (ani z DB, ani se nepodařilo uložit z localStorage)
