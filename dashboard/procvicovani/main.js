...
// Version: 25.0.0 - Major refactor for tab visibility, goal selection modal, and scrolling fix.
// + Always show tabs.
...
(function() { // Start IIFE
    ...
    // NEW_FUNCTION_START - Přidáno pro ukládání cíle
    ...
    function cacheDOMElements() {
        console.log("[Procvičování Cache DOM v4] Caching elements...");
        const ids = [
            ...
            'study-plan-empty', 'start-test-btn-plan', 'create-plan-btn-empty', // Přidáno tlačítko z study-plan-empty
            'main-plan-schedule',
            ...
            // Tab content IDs
            'practice-tab-content', // Přejmenováno z practice-tab
            'test-results-tab-content', // Přejmenováno z test-results-tab
            'study-plan-tab-content', // Přejmenováno z study-plan-tab
            'topic-analysis-tab-content' // Přejmenováno z topic-analysis-tab
        ];
        // ... (zbytek funkce cacheDOMElements beze změny)
        ...
        ui.tabContents = document.querySelectorAll('.tab-content'); // Nyní cílí na .tab-content
        ...
        ui.goalOptionCards = document.querySelectorAll('#goal-step-1 .goal-option-card'); // Přidáno pro nový modal

        if (missingCritical.length > 0) { console.error("[CACHE DOM v4] CRITICAL elements missing after search:", missingCritical); }
        if (missingPotential.length > 0) { console.warn(`[CACHE DOM v4] Potential elements missing: (${missingPotential.length})`, missingPotential); }
        if (!ui.notificationBell) console.warn("[CACHE DOM v4] Notification bell not found!");
        if (!ui.markAllReadBtn) console.warn("[CACHE DOM v4] Mark all read button (#mark-all-read-btn) not found!");

        if (notFound.length === 0) { console.log("[CACHE DOM v4] All primary elements cached successfully."); }
        else if (notFound.length > 0 && missingCritical.length === 0) { console.log(`[CACHE DOM v4] Some non-critical elements not found: (${notFound.length})`, notFound); }
        console.log("[Procvičování Cache DOM v4] Caching attempt complete.");
    }
    ...
    function setLoadingState(sectionKey, isLoadingFlag) {
        // ... (zbytek funkce setLoadingState beze změny, ale teď bude pracovat s ui.practiceTabContent atd.)
        ...
        const updateSingleSection = (key, loading) => {
            ...
            if (loading) {
                ...
                // Odstranění skeletonů a zobrazení obsahu/prázdného stavu
                const skeletonSelector = '.loading-skeleton, .dashboard-card.loading > .loading-skeleton, .card.loading > .loading-skeleton'; // Obecnější selektor

                const clearSkeletons = (el) => {
                    el?.querySelectorAll(skeletonSelector).forEach(skel => skel.parentElement.classList.remove('loading')); // Odeber třídu z rodiče
                    el?.querySelectorAll(skeletonSelector).forEach(skel => skel.remove()); // Odeber samotný skeleton
                };
                ...
                setTimeout(() => {
                    if (key === 'stats' && ui.statsCards) {
                        ...
                        } else if (!isLoading.stats) { // Jen pokud už není loading
                           // Může zde být prázdný stav pro statistiky, pokud je třeba
                           // renderStatsCards by měl být zodpovědný za finální zobrazení
                        }
    ...
    // NEW_FUNCTION_START - Vylepšená logika pro modální okno a ukládání cíle
    function showGoalSelectionModal() {
        if (!ui.goalSelectionModal || !ui.goalStep1) {
            console.error("[GoalModal] Critical: Modal or Step 1 element NOT FOUND!");
            showError("Chyba zobrazení výběru cíle.", true);
            return;
        }
        console.log("[GoalModal] Showing goal selection modal.");
        // Reset all steps to inactive and clear forms
        ...
        document.body.classList.add('modal-open'); // Zabrání scrollu stránky za modalem

        requestAnimationFrame(() => ui.goalSelectionModal.classList.add('active')); // Pro animaci

        // Přidání listenerů na karty cílů v prvním kroku
        ui.goalOptionCards.forEach(button => {
            const goal = button.dataset.goal;
            if (!goal) { console.warn("[GoalModal] Goal option button missing data-goal attribute:", button); return; }
            // Odstraníme starý listener, pokud existuje, abychom předešli duplikaci
            ...
            button._goalHandler = newHandler; // Uložíme referenci pro případné odstranění
        });
        console.log("[GoalModal] Listeners attached to goal option cards.");
    }

    function hideGoalSelectionModal() {
        if (!ui.goalSelectionModal) return;
        ui.goalSelectionModal.classList.remove('active');
        document.body.classList.remove('modal-open'); // Povolí scroll stránky
        setTimeout(() => {
    ...
    function handleInitialGoalSelection(selectedGoal) {
        ...
        pendingGoal = selectedGoal; // Uložíme vybraný cíl

        // Přímo uložíme, pokud není potřeba druhý krok
    ...
    function showStep2(goalType) {
        ...
        if (!ui.goalSelectionModal || !ui.goalStep1 || !step2Element) {
            console.error(`[GoalModal] Cannot show step 2: Critical element missing. Step ID: ${step2Id}, Found: ${!!step2Element}`);
        ...
        // Nastavení listenerů pro tlačítka Zpět a Potvrdit v tomto kroku
        ...
        if (backBtn) {
            ...
            backBtn.addEventListener('click', newHandler, { once: true }); // { once: true } pro automatické odstranění
            backBtn._backHandler = newHandler;
        } else { console.warn(`[GoalModal] Back button not found in step: #${step2Id}`); }
        ...
        if (confirmBtn) {
            ...
            confirmBtn.disabled = false; // Reset tlačítka
            confirmBtn.innerHTML = 'Potvrdit a pokračovat';
        } else { console.error(`[GoalModal] Confirm button not found in step: #${step2Id}`); }
    }

    function handleBackToStep1(step1Element, currentStep2Element) {
        console.log("[GoalModal] Going back to step 1...");
    ...
    function handleStep2Confirm(goalType) {
        ...
        const step2Id = `goal-step-${goalType.replace('math_', '')}`;
        const step2Element = document.getElementById(step2Id);
        if (!step2Element) { console.error(`[GoalModal] Step 2 element ${step2Id} not found during confirm.`); return; }
        ...
        } catch (e) { console.error("[GoalModal] Error getting step 2 details:", e); isValid = false; showToast("Chyba", "Nastala chyba při zpracování výběru.", "error"); }

        if (isValid) {
            console.log(`[GoalModal] Step 2 details collected for ${goalType}:`, details);
    ...
    async function saveGoalAndProceed(goal, details = null) {
        ...
        console.log(`[GoalModal Save] Saving goal: ${goal}, with details:`, details);
        ...
        try {
            ...
            if (details && Object.keys(details).length > 0) {
                ...
                delete finalPreferences.goal_details; // Odebereme, pokud nejsou žádné detaily
            }
            ...
            console.log("[GoalModal Save] Payload to update Supabase profile:", updatePayload);

            const { data: updatedProfileData, error } = await supabase
                .from('profiles')
                .update(updatePayload)
                .eq('id', currentUser.id)
                .select('*, selected_title, preferences') // Načteme aktualizovaný profil
                .single();

            if (error) throw error;

            currentProfile = updatedProfileData; // Aktualizujeme lokální profil
            if (!currentProfile.preferences) currentProfile.preferences = {}; // Zajistíme existenci
            console.log("[GoalModal Save] Goal and preferences saved successfully in Supabase:", currentProfile.learning_goal, currentProfile.preferences);

            let goalText = goal; // Pro toast notifikaci
            switch(goal) {
            ...
            hideGoalSelectionModal(); // Skryjeme modální okno

            // Zde je důležité, aby configureUIForGoal a loadPageData byly volány *PO* úspěšném uložení
            // a aktualizaci currentProfile.learning_goal
            // Tyto funkce nyní budou brát `goal` z `currentProfile.learning_goal`
            configureUIForGoal(); // Bez argumentu, vezme z currentProfile
            await loadPageData();   // Bez argumentu, vezme z currentProfile
            ...
            console.error("[GoalModal Save] Error saving goal/preferences:", error);
    ...
    // NEW_FUNCTION_START - Aktualizace configureUIForGoal
    function configureUIForGoal() {
        if (!currentProfile) {
            console.error("[UI Config] Profile not loaded. Cannot configure UI.");
            return;
        }
        const goal = currentProfile.learning_goal; // Získáme cíl z aktuálního profilu
        console.log(`[UI Config] Configuring UI for goal: ${goal}`);

        if (!ui || Object.keys(ui).length === 0) {
            console.error("[UI Config] UI cache empty, cannot configure.");
            return;
        }
        ...
        // Zobrazení/skrytí záložek podle cíle
        ...
        // Záložky Přehled (practice-tab) a Témata (topic-analysis-tab) jsou vždy viditelné

        if (testTabButton) testTabButton.style.display = isExamPrep ? 'flex' : 'none';
        else console.warn("[UI Config] Test results tab button not found.");

        if (planTabButton) planTabButton.style.display = (isExamPrep || goal === 'math_review' || goal === 'math_accelerate') ? 'flex' : 'none'; // Povoleno i pro review a accelerate
        else console.warn("[UI Config] Study plan tab button not found.");

        // Aktualizace obsahu aktivní záložky
        const activeTabButton = document.querySelector('.content-tab.active');
        if (activeTabButton) {
            const activeTabId = activeTabButton.dataset.tab;
            // Pokud je aktivní záložka nyní skrytá, přepneme na "Přehled"
            if (activeTabButton.style.display === 'none') {
                console.log(`[UI Config] Active tab '${activeTabId}' is now hidden, switching to 'practice-tab'.`);
                switchActiveTab('practice-tab'); // Funkce pro přepnutí záložky
            } else {
                // Znovu načteme data pro aktuálně aktivní a viditelnou záložku,
                // aby se obsah aktualizoval podle nového cíle (pokud je to nutné)
                // Toto je zjednodušení, v reálné aplikaci by se volala specifická funkce pro danou záložku
                console.log(`[UI Config] Reloading content for active tab: ${activeTabId}`);
                loadTabData(activeTabId); // Předpokládá existenci funkce loadTabData
            }
        } else {
            // Pokud žádná záložka není aktivní (mělo by nastat jen při první inicializaci), aktivujeme Přehled
            console.log("[UI Config] No active tab found, activating 'practice-tab'.");
    ...
    // NEW_FUNCTION_START - Funkce pro načtení dat specifické záložky
    async function loadTabData(tabId) {
        ...
        console.log(`[Load Tab Data] Loading data for tab: ${tabId} with goal: ${goal}`);

        setLoadingState(tabIdToSectionKey(tabId), true); // např. tests, plan, topics

        try {
            switch (tabId) {
                case 'practice-tab':
                    // Načte statistiky a zkratky, které jsou na záložce Přehled
                ...
                case 'test-results-tab':
                    if (goal === 'exam_prep') {
                        ...
                        renderTestResults([], goal); // Zobrazí prázdný stav nebo zprávu
                    }
                    break;
                case 'study-plan-tab':
                ...
                case 'topic-analysis-tab':
                    ...
                    break;
                default:
                    console.warn(`[Load Tab Data] No specific data loading logic for tab: ${tabId}`);
            }
        } catch (error) {
            console.error(`[Load Tab Data] Error loading data for tab ${tabId}:`, error);
            showError(`Nepodařilo se načíst data pro záložku: ${error.message}`);
            // Zobrazit chybový stav pro danou záložku
            const contentEl = document.getElementById(tabId); // Např. test-results-tab-content
            if (contentEl) {
    ...
    // Pomocná funkce pro mapování tabId na sectionKey pro setLoadingState
    function tabIdToSectionKey(tabId) {
        switch (tabId) {
            case 'practice-tab': return 'stats'; // Nebo 'shortcuts', podle toho co se primárně načítá
    ...
    async function loadPageData() {
        if (!currentProfile || !currentProfile.learning_goal) {
            console.error("[Load Page Data] Profile or learning goal not loaded. Cannot load page data.");
            // Zobrazíme modální okno pro výběr cíle, pokud ještě nebylo zobrazeno
            if (!ui.goalSelectionModal?.classList.contains('active')) {
                showGoalSelectionModal();
                if(ui.mainContent) ui.mainContent.classList.add('interaction-disabled');
                if(ui.tabsWrapper) ui.tabsWrapper.style.display = 'none'; // Skryjeme záložky, dokud není cíl
        ...
        if (ui.goalSelectionModal) ui.goalSelectionModal.style.display = 'none'; // Skryjeme modal, pokud byl zobrazen
        if(ui.mainContent) ui.mainContent.classList.remove('interaction-disabled');
        if(ui.tabsWrapper) ui.tabsWrapper.style.display = 'flex'; // Zobrazíme záložky

        console.log(`🔄 [Load Page Data v4] Loading data for goal: ${goal}...`);
        setLoadingState('all', true);
        hideError();
        // configureUIForGoal(); // Nyní se volá po uložení cíle nebo při inicializaci

        // Načteme data pro aktuálně aktivní záložku
        const activeTabButton = document.querySelector('.content-tab.active');
        let activeTabId = 'practice-tab'; // Výchozí, pokud nic není aktivní
        if (activeTabButton) {
            activeTabId = activeTabButton.dataset.tab;
        } else {
            // Pokud žádná záložka není aktivní, aktivujeme "Přehled"
            const practiceTabElem = document.querySelector('.content-tab[data-tab="practice-tab"]');
            if (practiceTabElem) {
                handleTabSwitch({ currentTarget: practiceTabElem }); // Aktivuje a načte
            } else {
                console.error("[Load Page Data] Practice tab button not found for default activation.");
            }
            // Následující loadTabData by se nemělo volat, pokud handleTabSwitch již načetl data.
            // Je potřeba zvážit, zda loadTabData není redundantní, pokud handleTabSwitch již dělá load.
        }
        
        // Místo načítání všech dat najednou, načteme jen data pro aktivní záložku.
        // Ostatní se načtou při přepnutí.
        await loadTabData(activeTabId);

        // Speciální logika pro diagnostic-prompt
        if (goal === 'exam_prep' && diagnosticResultsData.length === 0 && ui.diagnosticPrompt) {
            ui.diagnosticPrompt.style.display = 'flex';
             // Skryjeme empty states ostatních záložek, pokud se zobrazí diagnostic prompt
        ...
        console.log("✅ [Load Page Data v4] Relevant data for active tab loaded.");
        setLoadingState('all', false); // Ukončíme obecný loading
        initTooltips();
    }


    // NEW_FUNCTION_START - Vylepšená logika přepínání záložek
    let isInitialTabLoad = true; // Sledujeme první načtení stránky
    ...
    function handleTabSwitch(eventOrTabId) {
        ...
        if (typeof eventOrTabId === 'string') {
            ...
            if (!targetTabButton) {
                console.warn(`[Tabs v4] Programmatic switch: Tab button for '${tabId}' not found.`);
                return;
            }
            ...
            if (!tabId) {
                console.warn("[Tabs v4] Clicked element has no data-tab attribute.");
                return;
            }
        } else {
            console.warn("[Tabs v4] Invalid argument for handleTabSwitch:", eventOrTabId);
            return;
        }

        // Pokud je вкладка již aktivní a nejedná se o první načtení, nedělej nic
        if (targetTabButton.classList.contains('active') && !isInitialTabLoad) {
            console.log(`[Tabs v4] Tab ${tabId} is already active. Switch ignored.`);
            return;
        }

        console.log(`[Tabs v4] Switching to tab: ${tabId}`);

        ui.contentTabs.forEach(tab => tab.classList.remove('active'));
        targetTabButton.classList.add('active');

        ui.tabContents.forEach(content => {
            // Mapujeme data-tab atribut tlačítka na ID obsahu
            const contentId = `${tabId}`; // Např. practice-tab -> practice-tab (obsah má stejné ID)
            const isActive = content.id === contentId;
            content.classList.toggle('active', isActive);
            content.style.display = isActive ? 'block' : 'none'; // Explicitně řídíme display
        });

        // Pokud je to přepnutí uživatelem (ne první načtení), načteme data pro novou záložku
        if (!isInitialTabLoad) {
            loadTabData(tabId);
        }
        isInitialTabLoad = false; // Po prvním úspěšném přepnutí (nebo nastavení) se stává false
    }
    // NEW_FUNCTION_END
    ...
    function switchActiveTab(tabId) {
        ...
        if (tabButton) {
            ...
            console.warn(`[SwitchActiveTab] Tab button for '${tabId}' not found.`);
        }
    }
    ...
    function setupEventListeners() {
        console.log("[Procvičování SETUP v4] Setting up event listeners...");
        const safeAddListener = (element, eventType, handler, key) => { /* ... no change ... */ if (element) { if (element._eventHandlers && element._eventHandlers[eventType]) { element.removeEventListener(eventType, element._eventHandlers[eventType]); } element.addEventListener(eventType, handler); if (!element._eventHandlers) element._eventHandlers = {}; element._eventHandlers[eventType] = handler; } else { const nonCriticalMissing = ['markAllReadBtn']; if (nonCriticalMissing.includes(key)) { console.log(`[SETUP v4] Non-critical element not found for listener: ${key}.`); } else { console.warn(`[SETUP v4] Element not found for listener: ${key}`); } } };
        const safeAddListenerToAll = (elementsNodeList, eventType, handler, key) => { /* ... no change ... */ if(elementsNodeList && elementsNodeList.length > 0) { elementsNodeList.forEach(el => safeAddListener(el, eventType, handler, `${key}-${el.dataset?.tab || 'item'}`)); } else { console.warn(`[SETUP v4] NodeList empty for key: ${key}`); } };
        ...
        safeAddListener(ui.createPlanBtnEmpty, 'click', () => switchActiveTab('study-plan-tab'), 'createPlanBtnEmpty'); // Nové tlačítko
        ...
        // Listeners for goal modal steps
        safeAddListenerToAll(ui.modalBackBtns, 'click', (event) => {
            ...
            pendingGoal = null; // Reset if going back to step 1
        }, 'modalBackBtns');

        safeAddListenerToAll(ui.modalConfirmBtns, 'click', (event) => {
            const goal = event.currentTarget.dataset.goal;
            if (goal === pendingGoal) { // Potvrzení pro druhý krok
                handleStep2Confirm(goal);
            } else { // Výběr z prvního kroku (i když by to mělo být řešeno přes goalOptionCards)
                console.warn("[GoalModal] Confirm button clicked without matching pendingGoal. Goal from button:", goal);
                // Fallback to handle initial selection if somehow triggered here.
                if (ui.goalStep1?.classList.contains('active') && goal) {
        ...
        console.log("[Procvičování SETUP v4] Event listeners set up.");
    }
    ...
    async function initializeApp() {
        try {
            console.log("[INIT Procvičování] App Init Start v25.0.0...");
            cacheDOMElements(); // Musí být první pro ui objekt
            if (!initializeSupabase()) return;
            setupEventListeners(); // Nastaví posluchače
            ...
            if (ui.tabsWrapper) ui.tabsWrapper.style.display = 'none'; // Záložky skryty, dokud není cíl
            ...
            if (session?.user) {
                ...
                if (!currentProfile.preferences) currentProfile.preferences = {}; // Zajistíme, že preferences existují
                ...
                // Získání uloženého cíle z localStorage, POKUD v profilu ještě není
                ...
                if (!currentProfile.learning_goal && storedGoal) {
                    ...
                    // Uložíme cíl z localStorage do profilu, pokud v profilu není
                    await saveGoalAndProceed(storedGoal, Object.keys(storedGoalDetails).length > 0 ? storedGoalDetails : null);
                    // saveGoalAndProceed aktualizuje currentProfile
                ...
                if (!goal) {
                    showGoalSelectionModal();
                    setLoadingState('all', false);
                    if (ui.mainContent) ui.mainContent.style.display = 'block'; // Zobrazíme hlavní kontejner
                    if (ui.tabsWrapper) ui.tabsWrapper.style.display = 'none'; // Záložky skryty
                    ui.tabContents.forEach(el => el.style.display = 'none'); // Obsah záložek skryt
                } else {
                    if(ui.goalSelectionModal) ui.goalSelectionModal.style.display = 'none';
                    if(ui.tabsWrapper) ui.tabsWrapper.style.display = 'flex'; // Zobrazíme záložky (použijeme flex)
                    configureUIForGoal(); // Konfiguruje UI na základě cíle z currentProfile

                    // Aktivujeme defaultní/poslední záložku a načteme její data
                    // Předpokládáme, že 'practice-tab' je výchozí, pokud nic jiného není uloženo
                    const lastTab = localStorage.getItem('lastActiveProcvicovaniTab') || 'practice-tab';
                    isInitialTabLoad = true; // Nastavíme pro první load
                    switchActiveTab(lastTab); // Toto zavolá handleTabSwitch, které zavolá loadTabData
                    // isInitialTabLoad se resetuje v handleTabSwitch
                }

                // Zobrazíme hlavní obsah až po kontrole cíle
