...
// Version: 24.10 - Fixed SyntaxError, fixed goal button selection, fixed updateOnlineStatus call, added element checks.
...
(function() { // Start IIFE
    ...
    function cacheDOMElements() {
        ...
        const ids = [
            ...
            'goal-step-1', // Кэшируем сам шаг 1
            // Кнопки шага 1 будем искать динамически
        ...
        const criticalMissing = ['practiceTabContent', 'goalSelectionModal', 'goalStep1', 'goalStepAccelerate', 'goalStepReview', 'goalStepExplore']
                                .filter(key => !ui[key]);
        if (criticalMissing.length > 0) { console.error("[CACHE DOM] CRITICAL modal step elements missing:", criticalMissing); }
    ...
    function showToast(title, message, type = 'info', duration = 4500) {
        if (!ui.toastContainer) { console.log(`No toast container found. Toast (${type}): ${title} - ${message}`); return; } // Добавлена проверка
        try { const toastId = `toast-${Date.now()}`; const toastElement = document.createElement('div'); /* ... остальная логика toast ... */ } catch (e) { console.error("Error showing toast:", e); }
    }
    ...
    function renderTestChart(chartData) { /* ... no implementation needed as canvas is missing ... */ }
    ...
    function showGoalSelectionModal() {
        ...
        if (!modal) { console.error("Modal #goal-selection-modal not found!"); return; }
        if (!step1) { console.error("Modal step #goal-step-1 not found! Cannot attach listeners."); showError("Chyba zobrazení výběru cíle (krok 1).", true); return; } // Critical check
        ...
        // Ищем кнопки по data-атрибуту ВНУТРИ step1
        const optionButtons = step1.querySelectorAll('.goal-option-card[data-goal]');
        if (optionButtons.length === 0) { console.error("No goal option buttons found in step 1!"); return; }

        optionButtons.forEach(button => {
            const goal = button.dataset.goal;
            if (!goal) { console.warn("Goal button missing data-goal attribute:", button); return; }
            ...
            button._goalHandler = handler; // Сохраняем ссылку на новый
        });
    ...
    function renderShortcutsForGoal(goal, container) {
        // ... (Полная реализация renderShortcutsForGoal из предыдущего ответа) ...
         ...
         const shortcutExplore = `<div class="shortcut-card card" data-animate><div class="shortcut-icon"><i class="fas fa-compass"></i></div><h3 class="shortcut-title">Procházet Témata</h3><p class="shortcut-desc">Vyberte si libovolné matematické téma k učení.</p><a href="#" class="btn btn-primary btn-sm" style="margin-top: auto;" onclick="document.querySelector('.content-tab[data-tab=topic-analysis-tab]').click(); return false;">Vybrat Téma</a></div>`;
         ...
         requestAnimationFrame(() => { if(container) { container.innerHTML = shortcutsHTML; container.classList.remove('loading'); setLoadingState('shortcuts', false); initScrollAnimations(); } });
    }
    ...
    async function loadPageData() {
        // ... (Полная реализация loadPageData из предыдущего ответа) ...
    ...
    function setupEventListeners() {
        ...
        // Listeners for modal buttons added dynamically
        // Sidebar listeners (basic)
        ...
        safeAddListenerToAll(document.querySelectorAll('.sidebar-link'), 'click', () => { if (window.innerWidth <= 992) closeMenu(); }, '.sidebar-link');
    ...
    function initializeSupabase() {
        try {
            ...
            // Избегаем повторной инициализации, если она уже есть глобально
            if (typeof supabase === 'undefined' || supabase === null) {
                 supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
                 if (!supabase) throw new Error("Supabase client creation failed.");
                 console.log('[Supabase] Client initialized by main.js.');
            } else {
                 console.log('[Supabase] Client already initialized (likely by dashboard.js). Using existing instance.');
            }
            return true;
        } catch (error) {
    ...
    async function initializeApp() {
        console.log("[INIT Procvičování] App Init Start v24.10...");
        ...
        updateCopyrightYear();
        initMouseFollower(); // Call stubs
        initHeaderScrollDetection();
        updateOnlineStatus();
        // Removed online/offline listeners here, should be global
        ...
        try {
            ...
            if (session?.user) {
                ...
                if (!currentProfile.learning_goal) {
                    ...
                    if (ui.mainContent) ui.mainContent.style.display = 'block'; // Show main for modal
                    ...
                    if(ui.goalSelectionModal) ui.goalSelectionModal.style.display = 'none'; // Hide modal
                    if(ui.tabsWrapper) ui.tabsWrapper.style.display = 'block'; // Show tabs
                    document.querySelectorAll('.tab-content').forEach(el => el.style.display = 'block'); // Show content area
