...
(function() {
    ...
    const updateOnlineStatus = () => { if (ui.offlineBanner) {ui.offlineBanner.style.display = navigator.onLine ? 'none' : 'block'; }};
    ...
    const setLoadingState = (sectionKey, isLoadingFlag) => {
        ...
        } else if (sectionKey === 'currentPlan') {
    ...
    async function saveLearningGoal(goal, details = {}) {
        ...
        const preferencesUpdate = { ...state.currentProfile.preferences, goal_details: details };
        try {
    ...
    async function loadDashboardStats() {
        ...
        } catch (error) { console.error("Error loading dashboard stats:", error); if(ui.statsCardsContainer) ui.statsCardsContainer.innerHTML = '<p class="error-message">Statistiky nelze načíst.</p>';
        } finally { setLoadingState('stats', false); }
    }
    function renderStatsCards(stats) {
        if (!ui.statsCardsContainer) return;
        ui.statsCardsContainer.innerHTML = ''; ui.statsCardsContainer.classList.remove('loading');
        if (!stats || stats.length === 0) { ui.statsCardsContainer.innerHTML = '<p>Žádné statistiky k zobrazení.</p>'; return; }
        stats.forEach((stat, index) => {
            const card = document.createElement('div'); card.className = 'dashboard-card card'; card.setAttribute('data-animate', ''); card.style.setProperty('--animation-order', index + 1);
            card.innerHTML = `<div class="card-header"><h3 class="card-title">${sanitizeHTML(stat.title)}</h3>${stat.badge ? `<span class="card-badge ${stat.badge.toLowerCase()}">${sanitizeHTML(stat.badge)}</span>` : ''}</div><div class="card-content"><div class="card-value">${sanitizeHTML(String(stat.value))}</div><p class="card-description">${sanitizeHTML(stat.desc)}</p></div>${stat.footerText ? `<div class="card-footer ${stat.footerClass || ''}">${stat.footerIcon ? `<i class="fas ${stat.footerIcon}"></i>` : ''} ${sanitizeHTML(stat.footerText)}</div>` : ''}`;
    ...
    function renderTopicProgressTable(topics) {
        ...
        topics.forEach(topic => {
            const row = ui.topicProgressTableBody.insertRow(); const progress = Math.round(topic.progress_percentage || 0);
            let progressColor = 'var(--accent-primary)'; if (progress >= 75) progressColor = 'var(--accent-lime)'; else if (progress < 40) progressColor = 'var(--accent-pink)';
            row.innerHTML = `<td><i class="fas ${topic.topic_icon || 'fa-question-circle'}" style="margin-right: 0.7em; color: ${progressColor};"></i>${sanitizeHTML(topic.topic_name)}</td><td><div class="progress-bar-cell"><div class="progress-bar-track"><div class="progress-bar-fill" style="width: ${progress}%; background: ${progressColor};"></div></div><span class="progress-bar-text" style="color: ${progressColor};">${progress}%</span></div></td><td>${topic.last_studied_at ? formatDate(topic.last_studied_at) : '-'}</td>`;
        });
    }
    ...
    async function switchTabContent(tabId) {
        ...
        if (tabId === 'practice-tab') { targetElement = ui.practiceTabContent; }
        else if (tabId === 'current') { targetElement = ui.currentPlanSection; }
        else if (tabId === 'vyuka-tab') { targetElement = ui.vyukaTabContent; }
        else { console.warn(`[Main Tab Switch] Unknown tabId: ${tabId}`); return; }
        ...
        else if (tabId === 'current') { await loadCurrentStudyPlanData(); }
    }
    ...
    async function loadCurrentStudyPlanData() {
        ...
        if (typeof loadCurrentPlan === 'function') {
            ...
            if(ui.currentPlanSection) { const emptyStateContainer = ui.currentPlanSection.querySelector('#currentPlanEmptyState') || ui.currentPlanSection; renderMessage(emptyStateContainer, 'error', 'Chyba', 'Logika pro načtení plánu není dostupná.'); }
        }
    }
    ...
    const groupActivitiesByDayAndDateArray = (activities) => {
        ...
        let currentDayOfWeek = referenceDate.getDay(); if (currentDayOfWeek === 0) currentDayOfWeek = 7; if (planStartDayOfWeek === 0) planStartDayOfWeek = 7;
        ...
        for (let i = 0; i < 7; i++) { const currentDate = new Date(state.planStartDate); currentDate.setDate(state.planStartDate.getDate() + i); dayToDateMap[(currentDate.getDay() + 6) % 7] = dateToYYYYMMDD(currentDate); }
    ...
    async function loadCurrentPlan() {
        ...
        setLoadingState('currentPlan', true); setLoadingState('schedule', true);
        ...
        try {
            ...
            if (plans && plans.length > 0) {
                state.currentStudyPlan = plans[0]; console.log("[CurrentPlan] Active plan found:", state.currentStudyPlan.id);
            ...
            if (ui.dailyPlanCarouselContainer) ui.dailyPlanCarouselContainer.style.display = 'none';
        } finally { setLoadingState('currentPlan', false); setLoadingState('schedule', false); console.log("[CurrentPlan] Loading finished."); }
    }
    ...
    async function getLatestDiagnosticTest(userId, showLoaderFlag = true) {
        if (!userId || !supabaseClient) return null;
        if (showLoaderFlag && typeof setLoadingState === 'function') setLoadingState('create', true);
        try {
    ...
    function setupEventListeners() {
        ...
        if(ui.refreshDataBtn) ui.refreshDataBtn.addEventListener('click', async () => { showToast("Obnovuji data...", "info", 2000); setLoadingState('all', true); await loadDashboardStats(); await loadTopicProgress(); if(state.currentMainTab === 'current') await loadCurrentStudyPlanData(); setLoadingState('all', false); showToast("Data obnovena!", "success"); });
        ...
        ui.goalModalConfirmBtns.forEach(btn => btn.addEventListener('click', () => {
            const goal = btn.dataset.goal; let details = { goal_set_timestamp: new Date().toISOString() };
            if (goal === 'math_accelerate') { details.grade = ui.accelerateGradeSelect.value; details.intensity = ui.accelerateIntensitySelect.value; details.accelerate_areas = Array.from(ui.accelerateAreasCheckboxes).filter(cb => cb.checked).map(cb => cb.value); details.accelerate_reason = document.querySelector('input[name="accelerate_reason"]:checked')?.value; if(details.accelerate_reason === 'professional_needs') { details.profession = ui.accelerateProfessionTextarea.value; } }
            else if (goal === 'math_review') { details.grade = ui.reviewGradeSelect.value; details.topic_ratings = {}; ui.topicRatingsContainer.querySelectorAll('.rating-stars').forEach(rs => { const topicId = rs.dataset.topicId; const rating = rs.dataset.currentRating; if (topicId && rating) details.topic_ratings[topicId] = { overall: parseInt(rating) }; }); }
            else if (goal === 'math_explore') { details.grade = ui.exploreGradeSelect.value; }
            saveLearningGoal(goal, details);
        }));
        document.querySelector('input[name="accelerate_reason"][value="professional_needs"]')?.addEventListener('change', (e) => { if (ui.accelerateProfessionGroup) ui.accelerateProfessionGroup.style.display = e.target.checked ? 'block' : 'none'; });
    ...
    async function initializeApp() {
        ...
        if (!initializeSupabase()) { setLoadingState('page', false); showGlobalError("Kritická chyba: Nelze inicializovat databázi."); if(ui.mainContent) ui.mainContent.style.display = 'block'; return; }
        ...
        try {
            ...
            if (!state.currentProfile) { console.error("[Init Main] Profile could not be loaded for user:", state.currentUser.id); showGlobalError("Nepodařilo se načíst váš profil. Zkuste obnovit stránku."); setLoadingState('page', false); if(ui.mainContent) ui.mainContent.style.display = 'block'; return; }
            ...
            await fetchNotifications(state.currentUser.id, NOTIFICATION_FETCH_LIMIT).then(({ unreadCount, notifications }) => renderNotifications(unreadCount, notifications)).catch(err => { console.error("Failed to load notifications initially:", err); renderNotifications(0, []); });
            const { completedGoalSetting, completedDiagnostic } = await checkUserInitialSetup(state.currentUser.id);
            state.hasCompletedGoalSetting = completedGoalSetting; state.hasCompletedDiagnostic = completedDiagnostic;
            ...
            if (ui.mainContent) { ui.mainContent.style.display = 'block'; requestAnimationFrame(() => { if(ui.mainContent) ui.mainContent.classList.add('loaded'); initScrollAnimations(); }); }
