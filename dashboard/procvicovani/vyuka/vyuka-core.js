...
// Версия v28: Upraveno manageButtonStates a manageUIState pro nový koncept finálního testu.
...
(function(VyukaApp) {
	...
	try {
		VyukaApp.config = {
            ...
            POINTS_QUIZ_COMPLETION_BONUS: 10, // Bonus za úspěšné dokončení testu
		};
		...
		VyukaApp.ui = {
            ...
            chatActionTemplates: document.getElementById('chat-action-templates'), // Přidáno pro skrývání/zobrazování
            ...
            // submitQuizBtn se vytváří dynamicky, není třeba ho cachovat zde, ale budeme s ním pracovat v manageButtonStates
		};
        ...
        VyukaApp.toggleVyukaSidebarAi = (forceState) => { const ui = VyukaApp.ui; const state = VyukaApp.state; if (!ui.vyukaSidebarAi) return; const shouldBeExpanded = typeof forceState === 'boolean' ? forceState : !ui.vyukaSidebarAi.classList.contains('expanded'); ui.vyukaSidebarAi.classList.toggle('expanded', shouldBeExpanded); state.isVyukaSidebarAiExpanded = shouldBeExpanded; if (ui.sidebarAiDesktopToggle) { const icon = ui.sidebarAiDesktopToggle.querySelector('i'); if (icon) icon.className = `fas fa-chevron-${shouldBeExpanded ? 'left' : 'right'}`; ui.sidebarAiDesktopToggle.title = shouldBeExpanded ? "Sbalit AI panel" : "Rozbalit AI panel"; ui.sidebarAiDesktopToggle.setAttribute('aria-expanded', shouldBeExpanded.toString()); } localStorage.setItem('vyukaSidebarAiState', shouldBeExpanded ? 'expanded' : 'collapsed'); };
        VyukaApp.toggleVyukaSidebarAiMobile = (forceState) => { const ui = VyukaApp.ui; const state = VyukaApp.state; if (!ui.vyukaSidebarAi || !ui.mainMobileMenuToggle) return; const shouldBeActive = typeof forceState === 'boolean' ? forceState : !ui.vyukaSidebarAi.classList.contains('active-mobile'); ui.vyukaSidebarAi.classList.toggle('active-mobile', shouldBeActive); ui.vyukaSidebarAi.classList.toggle('expanded', shouldBeActive); if (ui.sidebarOverlayAi) ui.sidebarOverlayAi.classList.toggle('active', shouldBeActive); else if (shouldBeActive) { ui.sidebarOverlayAi = document.getElementById('sidebar-overlay-ai'); if (!ui.sidebarOverlayAi) { ui.sidebarOverlayAi = document.createElement('div'); ui.sidebarOverlayAi.className = 'sidebar-overlay-ai'; ui.sidebarOverlayAi.id = 'sidebar-overlay-ai'; const pageContainer = document.querySelector('.vyuka-page-container') || document.body; pageContainer.appendChild(ui.sidebarOverlayAi); ui.sidebarOverlayAi.addEventListener('click', () => VyukaApp.toggleVyukaSidebarAiMobile(false)); } requestAnimationFrame(() => ui.sidebarOverlayAi.classList.add('active')); } state.isVyukaSidebarAiMobileActive = shouldBeActive; ui.mainMobileMenuToggle.setAttribute('aria-expanded', shouldBeActive.toString()); };
		...
		VyukaApp.manageUIState = (mode, options = {}) => {
            ...
            console.log("[UI State v28]:", mode, options);
            ...
            if (state.currentTopic && !state.finalQuizActive && !state.finalQuizOffered && !state.finalQuizActive && mode !== 'requestingFinalQuiz' && mode !== 'quizEvaluating' && mode !== 'quizEvaluated') {
                topicDisplayText = `Téma: <strong>${VyukaApp.sanitizeHTML(state.currentTopic.name)}</strong>`;
                subjectTitleText = state.currentTopic.subject || "Předmět";
            ...
            switch (mode) {
                ...
                case 'quizOffered':
                    topicDisplayText = `Nabídka Testu: <strong>${VyukaApp.sanitizeHTML(state.currentTopic?.name || "Aktuální Téma")}</strong>`;
                    subjectTitleText = "Návrh Testu"; topicSubtitleText = "Potvrďte v chatu"; break;
                case 'requestingFinalQuiz':
                    topicDisplayText = `<span class="placeholder"><i class="fas fa-spinner fa-spin"></i> Připravuji test...</span>`;
                    subjectTitleText = "Generování Testu"; topicSubtitleText = VyukaApp.sanitizeHTML(state.currentTopic?.name || "K tématu"); break;
                case 'finalQuizInProgress':
                    topicDisplayText = `Závěrečný Test: <strong>${VyukaApp.sanitizeHTML(state.currentTopic?.name || "Aktuální Téma")}</strong>`;
                    subjectTitleText = "Testování Znalostí"; topicSubtitleText = "Odpovězte na otázky na tabuli"; break;
                case 'quizEvaluating':
                    topicDisplayText = `<span class="placeholder"><i class="fas fa-spinner fa-spin"></i> Vyhodnocuji test...</span>`;
                    subjectTitleText = "Vyhodnocení Testu"; topicSubtitleText = VyukaApp.sanitizeHTML(state.currentTopic?.name || "K tématu"); break;
                case 'quizEvaluated':
                    topicDisplayText = `Výsledky Testu: <strong>${VyukaApp.sanitizeHTML(state.currentTopic?.name || "Aktuální Téma")}</strong>`;
                    subjectTitleText = "Test Dokončen"; topicSubtitleText = "Výsledky jsou na tabuli"; break;
                 case 'waitingForAnswer': // Explicitní stav pro čekání na odpověď v chatu
                    topicDisplayText = `Čekám na odpověď: <strong>${VyukaApp.sanitizeHTML(state.currentTopic?.name || "Aktuální Téma")}</strong>`;
                    subjectTitleText = "Úloha pro vás"; topicSubtitleText = "Odpovězte v chatu"; break;
                 case 'learning': // Standardní výukový mód
                 default:
		...
		VyukaApp.manageButtonStates = () => {
            ...
            if (state.finalQuizActive) {
                chatIsEnabled = false; // Během testu na TABULI je chat pro běžné zprávy vypnutý
                chatPlaceholder = "Odpovědi vyplňujte na tabuli a poté test odevzdejte.";
            } else if (state.finalQuizOffered || isQuickReplyPresent) {
                chatIsEnabled = true;
                chatPlaceholder = "Vyberte možnost nebo napište odpověď...";
            } else if (state.aiIsWaitingForAnswer) {
                chatIsEnabled = baseInteractReady && !aiBusy && !state.isListening;
                chatPlaceholder = "Napište svou odpověď...";
            } else if (baseInteractReady && !aiBusy && !state.isListening) {
                chatIsEnabled = true;
                chatPlaceholder = "Zeptejte se nebo odpovězte...";
            }

            if (ui.sendButton) {
                ui.sendButton.disabled = !chatIsEnabled;
                ui.sendButton.innerHTML = state.geminiIsThinking ? '<i class="fas fa-spinner fa-spin"></i>' : '<i class="fas fa-paper-plane"></i>';
            }
            if (ui.chatInput) {
                ui.chatInput.disabled = !chatIsEnabled;
                ui.chatInput.placeholder = state.isListening ? "Poslouchám..." : chatPlaceholder;
            }
            if (ui.chatActionTemplates) { // Skryjeme/zobrazíme template tlačítka
                ui.chatActionTemplates.style.display = (chatIsEnabled && !state.finalQuizActive && !state.finalQuizOffered && !state.aiIsWaitingForAnswer && !isQuickReplyPresent) ? 'flex' : 'none';
            }
            ...
            if (ui.micBtn) {
                const canUseMic = baseInteractReady && state.speechRecognitionSupported && !aiBusy && !state.finalQuizActive && !state.finalQuizOffered && !isQuickReplyPresent;
            ...
            if (ui.clearChatBtn) ui.clearChatBtn.disabled = aiBusy || isChatEmptyOrInitial || state.finalQuizActive;
            if (ui.saveChatBtn) ui.saveChatBtn.disabled = aiBusy || isChatEmptyOrInitial || state.finalQuizActive;
            if (ui.aiModelSelect) ui.aiModelSelect.disabled = aiBusy || state.finalQuizActive || state.finalQuizOffered;

            // Tlačítko pro odevzdání kvízu (musí se hledat dynamicky, protože se přidává JS)
            const submitQuizBtnInstance = document.getElementById('submit-quiz-btn');
            if (submitQuizBtnInstance) {
                submitQuizBtnInstance.disabled = aiBusy; // Deaktivujeme, pokud AI něco dělá
                submitQuizBtnInstance.style.display = state.finalQuizActive ? 'inline-flex' : 'none';
            }
        };
		...
		VyukaApp.setupBaseEventListeners = () => { /* ... stejné ... */ };

		document.addEventListener('DOMContentLoaded', VyukaApp.initializeApp);

	} catch (e) { /* ... stejné zobrazení chyby ... */ }
