...
(function(VyukaApp) {
	...
	try {
        ...
        VyukaApp.loadNextUncompletedTopic = async () => {
			...
			try {
				const { data: plans, error: planError } = await state.supabase
					...
					.eq('status', 'active')
					.order('created_at', { ascending: false }) // Ensure we get the latest active if multiple (should not happen)
					.limit(1);
				...
				const { data: activities, error: activityError } = await state.supabase
					...
					.eq('completed', false)
					.order('day_of_week') // Ensure logical progression
				...
				if (activities && activities.length > 0) {
                    ...
                    let subject = 'Matematika'; // Default subject, or fetch from plans/topics if available

					if (activity.topic_id) {
						try {
							const { data: topic, error: topicError } = await state.supabase
								.from('exam_topics')
								.select('name, description, subject') // Assuming 'subject' column exists in exam_topics
    	...
    	VyukaApp.requestContinue = async () => {
			const state = VyukaApp.state;
			console.log("[RequestContinue v29] Triggered. AI Waiting:", state.aiIsWaitingForAnswer, "FinalQuizOffered:", state.finalQuizOffered, "Final Quiz Active:", state.finalQuizActive);
    	...
    	VyukaApp.addChatMessage = async (displayMessage, sender, saveToDb = true, timestamp = new Date(), ttsText = null, originalContent = null, quickReplies = null) => {
			...
			ui.chatMessages.appendChild(div); if (window.MathJax && typeof window.MathJax.typesetPromise === 'function' && (displayMessage.includes('$') || displayMessage.includes('\\'))) { setTimeout(() => { window.MathJax.typesetPromise([bubbleContentDiv]).catch((err) => console.error(`[MathJax v29 Chat] Typeset error: ${err.message}`)); }, 0); }
        ...
        VyukaApp.handleQuickReplyAction = async (actionPayload) => {
            ...
            console.log(`[QuickReply v29 AI] Handling action: ${actionPayload}`);
             const allQuickReplyButtons = document.querySelectorAll('.quick-reply-btn');
             allQuickReplyButtons.forEach(btn => {btn.disabled = true; btn.style.opacity="0.5";}); // Visually disable

            if (actionPayload === 'ACTION_USER_ACCEPTS_QUIZ') {
                console.log("[QuickReply v29 AI] User accepts final quiz.");
                ...
                console.log("[QuickReply v29 AI] User declines final quiz. Continuing lesson.");
                ...
                console.log("[QuickReply v29 AI] User marks topic complete after quiz.");
                ...
                console.log("[QuickReply v29 AI] User continues lesson after quiz evaluation.");
            ...
            } else { console.warn("[QuickReply v29 AI] Unknown action payload:", actionPayload); }
             VyukaApp.manageButtonStates();
        };
    	...
    	VyukaApp.handleSendMessage = async () => {
            ...
            if (state.aiIsWaitingForAnswer && !state.finalQuizActive && !state.finalQuizOffered) {
                console.log("[HandleSend v29 AI] Resetting aiIsWaitingForAnswer state (standard flow).");
                state.aiIsWaitingForAnswer = false;
            }
        ...
        VyukaApp.parseGeminiResponse = (rawText) => {
            console.log("[ParseGemini v29] Raw input:", rawText ? rawText.substring(0, 150) + "..." : "EMPTY");
			const config = VyukaApp.config;
            const boardMarkerLegacy = "[BOARD_MARKDOWN]:"; // Legacy
            ...
            // Vylepšené Regexy pro zachycení obsahu mezi značkami, včetně ```markdown ... ```
            ...
            let legacyBoardMarkdown = ""; // Pro zpětnou kompatibilitu

            const extractContent = (text, regex) => {
                const match = text.match(regex);
                if (match) {
                    let content = (match[1] || match[2] || "").trim();
                    // Odstranění případného úvodního "markdown" z obsahu, pokud je to jen značka bloku
            ...
            // Check for legacy board markdown if new blocks are empty but legacy marker is present
            if (!keyConcepts && !detailedExplanation && !examples && remainingText.match(boardRegexLegacy)) {
                ...
                console.log("[ParseGemini v29] Found and used legacy [BOARD_MARKDOWN].");
            }
            ...
            console.log("[ParseGemini v29] Results - KC:", !!keyConcepts, "DE:", !!detailedExplanation, "EX:", !!examples, "TTS:", !!ttsCommentary, "Chat:", !!chatText, "Action:", actionSignal, "LegacyBoard:", !!legacyBoardMarkdown);
			return { keyConcepts, detailedExplanation, examples, ttsCommentary, chatText, actionSignal, legacyBoardMarkdown };
		};
    	...
    	VyukaApp.processGeminiResponse = async (rawText, timestamp) => {
			...
			console.log("[ProcessGemini v29] Processing Raw Response:", rawText ? rawText.substring(0, 100) + "..." : "Empty");
            ...
            console.log(`[ProcessGemini v29] Parsed-> KC: ${!!keyConcepts}, DE: ${!!detailedExplanation}, EX: ${!!examples}, TTS: ${!!ttsCommentary}, Chat: ${!!cleanedChatText}, Action: ${actionSignal}, LegacyBoard: ${!!legacyBoardMarkdown}`);

            if (actionSignal === 'INITIATE_FINAL_QUIZ') {
                console.log("[ProcessGemini v29] AI offers final quiz."); aiResponded = true; state.finalQuizOffered = true; state.aiIsWaitingForAnswer = true;
                ...
                console.log("[ProcessGemini v29] AI provides quiz content for board."); aiResponded = true;
                const boardContentForQuiz = detailedExplanation || keyConcepts || examples || legacyBoardMarkdown; // Prioritize new blocks if available
                ...
                console.log("[ProcessGemini v29] AI provides quiz evaluation for board."); aiResponded = true;
                ...
                if (boardContentForEval) { VyukaApp.appendToWhiteboard({ type: 'detailed_explanation', content: boardContentForEval }, ttsCommentary || "Výsledky testu jsou na tabuli."); }
            ...
            // Běžné zpracování odpovědi s novými bloky
            if (keyConcepts) { VyukaApp.appendToWhiteboard({ type: 'key_concepts', content: keyConcepts }, ttsCommentary || keyConcepts); aiResponded = true; }
            if (detailedExplanation) { VyukaApp.appendToWhiteboard({ type: 'detailed_explanation', content: detailedExplanation }, ttsCommentary || detailedExplanation); aiResponded = true; }
            if (examples) { VyukaApp.appendToWhiteboard({ type: 'examples', content: examples }, ttsCommentary || examples); aiResponded = true; }
            if (legacyBoardMarkdown && !keyConcepts && !detailedExplanation && !examples) { // Pouze pokud nové bloky nejsou a je legacy
                VyukaApp.appendToWhiteboard({ type: 'detailed_explanation', content: legacyBoardMarkdown }, ttsCommentary || legacyBoardMarkdown); aiResponded = true;
            }

            if (aiResponded) { // Zkontrolujeme, zda byl nějaký obsah na tabuli
            ...
            if (cleanedChatText) {
                const ttsForChat = (!keyConcepts && !detailedExplanation && !examples && !legacyBoardMarkdown && ttsCommentary) ? ttsCommentary : null;
            ...
            if (!aiResponded) {
                if (ttsCommentary) { VyukaApp.addChatMessage(`(Pouze komentář: ${ttsCommentary})`, 'gemini', true, timestamp, ttsCommentary, `(Komentář: ${ttsCommentary})`); aiResponded = true;}
                else { VyukaApp.addChatMessage("(AI neodpovědělo očekávaným formátem nebo odpověď byla prázdná)", 'gemini', false, timestamp); }
                state.aiIsWaitingForAnswer = false;
            }
        ...
        VyukaApp.requestFinalQuizContent = async () => {
            const state = VyukaApp.state;
            console.log("[RequestFinalQuizContent v29 AI] Requesting quiz content for board.");
            VyukaApp.updateGeminiThinkingState(true);
        ...
        VyukaApp.renderQuizOnBoard = async (quizMarkdownWithPlaceholders) => {
            console.log("[RenderQuizOnBoard v29 AI] Rendering quiz on whiteboard with input fields.");
            ...
            if (questionIndex > 0) {
                ...
                if (ui.vyukaLessonControls) ui.vyukaLessonControls.style.justifyContent = 'center'; // Center submit button
            }
            ...
            console.log("[RenderQuizOnBoard v29 AI] Quiz rendered with input fields.");
        };

        VyukaApp.handleSubmitQuiz = async () => {
            const state = VyukaApp.state;
            console.log("[SubmitQuiz v29 AI] Submitting quiz answers.");
            ...
            if (VyukaApp.ui.vyukaLessonControls) VyukaApp.ui.vyukaLessonControls.style.justifyContent = 'flex-end'; // Reset controls justification
...
3.  **Chat (Text mimo značky):** VYUŽÍVEJ MINIMÁLNĚ. NIKDY v chatu NEUVÁDĚJ nový výukový obsah.
...
PRVNÍ KROK: Začni se ZÁKLADNÍ DEFINICÍ tématu "${topicName}" v bloku \`[KEY_CONCEPTS]:\`. Poté poskytni PODROBNĚJŠÍ VYSVĚTLENÍ v bloku \`[DETAILED_EXPLANATION]:\`. Nakonec uveď **alespoň JEDEN ŘEŠENÝ PŘÍKLAD** (jednoduchý) v bloku \`[EXAMPLES]:\`. Přidej krátký [TTS_COMMENTARY]:. NEPIŠ nic do chatu.
...
* **Struktura odpovědi na tabuli:** VŽDY strukturuj odpověď do bloků: \`[KEY_CONCEPTS]:\`, \`[DETAILED_EXPLANATION]:\`, \`[EXAMPLES]:\`. Všechny NOVÉ informace, **VÍCE ŘEŠENÝCH PŘÍKLADŮ** a ÚLOHY K ŘEŠENÍ patří VÝHRADNĚ do těchto bloků.
...
11. **Vyhodnocení Finálního Testu z Tabule:** Pokud obdržíš prompt **OBSAHUJÍCÍ frázi 'VYHODNOCENÍ FINÁLNÍHO TESTU Z TABULE'** a seznam otázek s odpověďmi, vyhodnoť KAŽDOU odpověď. Poskytni celkové skóre a zpětnou vazbu do bloku \`[DETAILED_EXPLANATION]:\`. Do [TTS_COMMENTARY] dej krátké shrnutí. Nic do CHATU. Odpověď ukonči signálem ${config.ACTION_EVALUATE_BOARD_QUIZ}.`;
    	...
    	VyukaApp.handleGeminiError = (msg, time) => {
            ...
            if (VyukaApp.ui.continueBtn) VyukaApp.ui.continueBtn.disabled = false; // Re-enable continue on error
            VyukaApp.manageUIState('learning');
		};

	} catch (e) {
		console.error("FATAL SCRIPT ERROR (AI Interaction v29 - Revolutionary Update):", e);
