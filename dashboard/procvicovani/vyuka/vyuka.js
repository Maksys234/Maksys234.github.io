...
// Версия 15:
// - Исправление рендеринга Markdown/MathJax в чате.
// - Улучшенное логирование и обработка ошибок для awardPoints.
// - Добавлена логика для предложения ИИ завершить тему ([ACTION:SUGGEST_COMPLETION]).
// - Обновлены промпты для ИИ (больше примеров, предложение завершения).
...
(function() { // IIFE для изоляции области видимости
	...
	try {
		...
		const MAX_GEMINI_HISTORY_TURNS = 12; // Increased slightly
		const NOTIFICATION_FETCH_LIMIT = 5;
		const POINTS_TOPIC_COMPLETE = 25; // Убедитесь, что это значение > 0
		...
		const ui = {
			...
			// TODO: Add element for AI completion prompt if using modal
		};
		...
		const openMenu = () => { /* ... (no changes) ... */ if (ui.sidebar && ui.sidebarOverlay) { ui.sidebar.classList.add('active'); ui.sidebarOverlay.classList.add('active'); } };
		const closeMenu = () => { /* ... (no changes) ... */ if (ui.sidebar && ui.sidebarOverlay) { ui.sidebar.classList.remove('active'); ui.sidebarOverlay.classList.remove('active'); } };
		// *** renderMarkdown: Adjusted for chat messages potentially having raw markdown ***
		const renderMarkdown = (el, text, isChat = false) => {
			...
			try {
				if (typeof marked === 'undefined') {
					 ...
					 el.innerHTML = `<p>${sanitizeHTML(originalText)}</p>`; // Fallback to safe text
					 return;
				}
                // Use options suitable for rendering potentially unsafe/raw markdown in chat
                // For whiteboard, stricter options might be needed if content is controlled differently
                const options = {
                    gfm: true,       // Enable GitHub Flavored Markdown
                    breaks: true,    // Convert single line breaks to <br>
                    sanitize: !isChat, // Sanitize ONLY if NOT chat (assume chat might contain intended raw markdown like ```)
                    smartypants: false // Avoid converting quotes, dashes etc.
                };
				marked.setOptions(options);
				const htmlContent = marked.parse(originalText);

				// Avoid wrapping plain text in paragraphs if it's already simple
				// Or if marked didn't do anything substantial (except maybe line breaks)
				...
				if (htmlContent === originalText.replace(/\n/g, '<br>') || (simpleTextRegex.test(originalText) && !originalText.includes('\n'))) {
					el.innerHTML = `<p>${sanitizeHTML(originalText)}</p>`; // Use <p> for simple lines for styling consistency
				} else {
					el.innerHTML = htmlContent; // Use marked's output
				}
		...
		// triggerWhiteboardMathJax: No changes needed
		const triggerWhiteboardMathJax = () => {
			 if (ui.whiteboardContent && window.MathJax && typeof window.MathJax.typesetPromise === 'function') {
				 console.log("[MathJax v15] Triggering global typeset for whiteboard...");
				 setTimeout(() => {
					 window.MathJax.typesetPromise([ui.whiteboardContent])
						 .then(() => console.log("[MathJax v15] Whiteboard typeset completed."))
						 .catch(e => console.error("[MathJax v15] Whiteboard typeset error:", e));
				 }, 100); // Delay slightly to ensure DOM update
			 } else { if (!ui.whiteboardContent) console.warn("[MathJax v15] Whiteboard content element not found for typesetting."); if (!(window.MathJax && typeof window.MathJax.typesetPromise === 'function')) console.warn("[MathJax v15] MathJax or typesetPromise not available."); }
		};
		// autoResizeTextarea, generateSessionId, etc. - No changes needed
 ...
 if (ui.chatMessages) { ui.chatMessages.innerHTML = ''; } clearWhiteboard(false); state.geminiChatContext = []; state.aiIsWaitingForAnswer = false; manageUIState('loadingTopic'); try { const { data: plans, error: planError } = await state.supabase.from('study_plans').select('id').eq('user_id', state.currentUser.id).eq('status', 'active').limit(1); if (planError) throw planError; if (!plans || plans.length === 0) { manageUIState('noPlan'); return; } state.currentPlanId = plans[0].id; const { data: activities, error: activityError } = await state.supabase.from('plan_activities').select('id, title, description, topic_id').eq('plan_id', state.currentPlanId).eq('completed', false).order('day_of_week').order('time_slot').limit(1); if (activityError) throw activityError; if (activities && activities.length > 0) { const activity = activities[0]; let name = activity.title || 'N/A'; let desc = activity.description || ''; if (activity.topic_id) { try { const { data: topic, error: topicError } = await state.supabase.from('exam_topics').select('name, description').eq('id', activity.topic_id).single(); if (topicError && topicError.code !== 'PGRST116') throw topicError; if (topic) { name = topic.name || name; desc = topic.description || desc; } } catch(e) { console.warn("Could not fetch topic details:", e); } } state.currentTopic = { activity_id: activity.id, plan_id: state.currentPlanId, name: name, description: desc, user_id: state.currentUser.id, topic_id: activity.topic_id }; if (ui.currentTopicDisplay) { ui.currentTopicDisplay.innerHTML = `Téma: <strong>${sanitizeHTML(name)}</strong>`; } await startLearningSession(); } else { manageUIState('planComplete'); } } catch (error) { console.error('Error loading next topic:', error); showToast(`Chyba načítání tématu: ${error.message}`, "error"); manageUIState('error', { errorMessage: error.message }); } finally { state.topicLoadInProgress = false; setLoadingState('currentTopic', false); } };
		const handleMarkTopicComplete = async () => {
			...
			try {
				...
				console.error(`[MarkComplete] Error marking topic complete:`, error);
		...
		// --- Points System ---
		// *** MODIFIED awardPoints v15: Added detailed logging and checks ***
		async function awardPoints(pointsValue) {
			console.log(`[Points v15] Attempting to award ${pointsValue} points.`);
			if (!state.currentUser || !state.currentUser.id) { console.warn("[Points v15] Skipping: No current user ID."); return; }
			if (!state.currentProfile || !state.currentProfile.id) { console.warn("[Points v15] Skipping: No current profile data."); // Attempt to fetch profile again? Maybe not here, rely on init.
			 showToast('Profil nenalezen, body nelze připsat.', 'warning'); return; }
			if (!state.supabase) { console.warn("[Points v15] Skipping: Supabase client not available."); return; }
			if (pointsValue <= 0) { console.log("[Points v15] Skipping: Zero or negative points value."); return; }
			...
			console.log(`[Points v15] User: ${userId}, Current Points: ${currentPoints}, Awarding: ${pointsValue}, New Total: ${newPoints}`);
			...
			try {
				...
				if (error) {
					console.error(`[Points v15] Supabase update error for user ${userId}:`, error);
					throw error; // Re-throw to be caught by the outer catch
				}

				if (data && data.points === newPoints) {
					// Update local state ONLY after successful DB update and confirmation
					state.currentProfile.points = newPoints;
					console.log(`[Points v15] User ${userId} points updated successfully in DB and state to ${newPoints}.`);
					showToast('+', `${pointsValue} kreditů získáno!`, 'success', 3000);
				} else {
					// This case should ideally not happen if the update succeeded without error and returned data
					console.warn(`[Points v15] DB update discrepancy for user ${userId}. Expected ${newPoints}, got ${data?.points}. State NOT updated locally.`);
					showToast('Varování', 'Nekonzistence při aktualizaci kreditů.', 'warning');
				}

			} catch (error) {
				console.error(`[Points v15] Exception updating user points for ${userId}:`, error);
		...
		const requestContinue = async () => { /* ... */ console.log("[RequestContinue] Triggered. AI Waiting:", state.aiIsWaitingForAnswer, "AI Suggested Completion:", state.aiSuggestedCompletion); if (state.geminiIsThinking || !state.currentTopic) return; if (state.aiIsWaitingForAnswer) { showToast("Nejprve odpovězte na úlohu v chatu.", "warning", 3000); console.warn("[RequestContinue] Blocked: AI is waiting for an answer."); return; } if (state.aiSuggestedCompletion) { showToast("AI navrhlo dokončení tématu. Použijte tlačítko Dokončit nebo požádejte AI o pokračování.", "info"); return; } // Don't automatically continue if completion suggested
 const prompt = _buildContinuePrompt(); await sendToGemini(prompt); };
		// *** MODIFIED addChatMessage v15: Reworked MathJax triggering ***
		const addChatMessage = async (displayMessage, sender, saveToDb = true, timestamp = new Date(), ttsText = null, originalContent = null) => {
			...
			// Only trigger if MathJax is ready and message contains potential math
			if (window.MathJax && typeof window.MathJax.typesetPromise === 'function' && (displayMessage.includes('$') || displayMessage.includes('\\'))) {
				 console.log(`[MathJax v15] Queueing typeset for chat message bubble: ${id}`);
				 // Use timeout 0 to queue after current execution context allows rendering
				 setTimeout(() => {
					 window.MathJax.typesetPromise([bubbleContentDiv]) // Target the whole bubble content div
						 .then(() => console.log(`[MathJax v15] Typeset successful for chat bubble ${id}`))
						 .catch((err) => console.error(`[MathJax v15] Typeset error for chat bubble ${id}: ${err.message}`));
				 }, 0);
			 } else {
                  // console.log(`[MathJax v15] Skipping typeset for chat message ${id} (No MathJax or no math symbols)`);
             }
			 // ---------------------------------------------
 ...
 if (state.aiIsWaitingForAnswer) { console.log("[HandleSend] Resetting aiIsWaitingForAnswer state."); state.aiIsWaitingForAnswer = false; manageUIState('learning'); } if (ui.chatInput) { ui.chatInput.value = ''; autoResizeTextarea(); } await addChatMessage(text, 'user', true, new Date(), null, text); state.geminiChatContext.push({ role: "user", parts: [{ text }] }); updateGeminiThinkingState(true); let promptForGemini = _buildChatInteractionPrompt(text); await sendToGemini(promptForGemini, true); };
		...
		// *** NEW Function v15: Prompt user for topic completion ***
		const promptTopicCompletion = () => {
			console.log("[CompletionPrompt] AI suggested topic completion.");
            state.aiSuggestedCompletion = true; // Set flag
            manageButtonStates(); // Update button states (e.g., disable 'Continue')

			// Use a simple confirm dialog for now
			// TODO: Replace with a custom modal/toast with Yes/No buttons for better UX
			const userConfirmation = confirm("AI Tutor navrhuje, že toto téma je dostatečně probráno a můžete ho označit za dokončené.\n\nChcete téma nyní dokončit?");

			if (userConfirmation) {
				console.log("[CompletionPrompt] User chose YES.");
				handleMarkTopicComplete(); // Call the existing function to complete the topic
			} else {
				console.log("[CompletionPrompt] User chose NO.");
                state.aiSuggestedCompletion = false; // Reset flag if user wants to continue
                // Maybe send a message to AI? Optional.
                // sendToGemini("Student chce ještě pokračovat v tématu.", true);
				showToast("Dobře, pokračujte kliknutím na 'Pokračuj' nebo položením otázky.", "info", 5000);
		...
		// *** MODIFIED parseGeminiResponse v15: Added detection for ACTION_SUGGEST_COMPLETION ***
		const parseGeminiResponse = (rawText) => {
			console.log("[ParseGemini v15] Raw input:", rawText ? rawText.substring(0, 150) + "..." : "EMPTY");
            ...
            const actionRegex = /\[ACTION:SUGGEST_COMPLETION]/i; // Regex for the action signal

			let boardMarkdown = "";
			let ttsCommentary = "";
            let actionSignal = null; // To store detected action
			let chatText = rawText || "";

			// Check for action signal FIRST
			const actionMatch = chatText.match(actionRegex);
			if (actionMatch) {
				actionSignal = 'SUGGEST_COMPLETION';
				chatText = chatText.replace(actionMatch[0], "").trim(); // Remove signal
				console.log(`[ParseGemini v15] Found action signal: ${actionSignal}`);
			}

			// Extract Board Content
			const boardMatch = chatText.match(boardRegex);
			if (boardMatch) {
				boardMarkdown = (boardMatch[3] || boardMatch[4] || "").trim();
				chatText = chatText.replace(boardMatch[0], ""); // Remove matched board section
				console.log(`[ParseGemini v15] Found board content. Length: ${boardMarkdown.length}`);
			} else {
				console.log(`[ParseGemini v15] Marker "${boardMarker}" not found.`);
			}

			// Extract TTS Content
			const ttsMatch = chatText.match(ttsRegex);
			if (ttsMatch) {
				ttsCommentary = (ttsMatch[2] || ttsMatch[3] || "").trim();
				chatText = chatText.replace(ttsMatch[0], ""); // Remove matched TTS section
				console.log(`[ParseGemini v15] Found TTS content. Length: ${ttsCommentary.length}`);
			} else {
				console.log(`[ParseGemini v15] Marker "${ttsMarker}" not found.`);
			}

			// Final cleanup of chatText
			chatText = chatText
				.replace(/\[BOARD_MARKDOWN]:/gi, '')
				.replace(/\[TTS_COMMENTARY]:/gi, '')
				.replace(/\[ACTION:SUGGEST_COMPLETION]/gi, '') // Ensure action signal is gone
				.replace(/```(markdown)?\s*|\s*```/g, '')
				.trim();

			console.log("[ParseGemini v15] Result - Board:", boardMarkdown ? boardMarkdown.substring(0, 50) + "..." : "None");
			console.log("[ParseGemini v15] Result - TTS:", ttsCommentary ? ttsCommentary.substring(0, 50) + "..." : "None");
			console.log("[ParseGemini v15] Result - Chat:", chatText ? chatText.substring(0, 50) + "..." : "None");
            console.log("[ParseGemini v15] Result - Action:", actionSignal);

			return { boardMarkdown, ttsCommentary, chatText, actionSignal }; // Return actionSignal too
		};

		// *** MODIFIED processGeminiResponse v15: Handle actionSignal ***
		const processGeminiResponse = (rawText, timestamp) => {
			removeThinkingIndicator();
			state.lastInteractionTime = Date.now();
			console.log("[ProcessGemini v15] Processing Raw Response:", rawText ? rawText.substring(0, 100) + "..." : "Empty Response");
			...
			console.log(`[ProcessGemini v15] Parsed-> Board: ${!!boardMarkdown}, TTS: ${!!ttsCommentary}, Chat: ${!!cleanedChatText}, Action: ${actionSignal}`);

            // Handle Action Signal FIRST
            if (actionSignal === 'SUGGEST_COMPLETION') {
                promptTopicCompletion(); // Trigger the user confirmation prompt
                aiResponded = true; // Mark as responded even if no other content
                // Don't process board/chat/tts below if only action signal was sent
                // Unless there's also commentary to go with the suggestion? Decide based on expected AI behavior.
                // For now, let's assume the action takes precedence.
                // If there IS commentary alongside the action, we might want to speak it.
                 if (ttsCommentary) {
                    speakText(ttsCommentary); // Speak accompanying TTS if any
                 }
                 manageUIState('suggestedCompletion'); // Update UI state
			...
			if (boardMarkdown) {
				...
				if (taskKeywords.some(kw => lowerBoard.includes(kw)) || taskHeaderRegex.test(boardMarkdown) || zadaniEndsWithQuestion.test(boardMarkdown.replace(/\s+/g, ' '))) {
					...
					console.log("[ProcessGemini v15] Task DETECTED on board, setting aiIsWaitingForAnswer = true.");
				} else {
					console.log("[ProcessGemini v15] No task detected on board.");
				}
			}

			if (cleanedChatText) {
				// Use commentary only for chat TTS if there was no board content to speak it with
				...
				// Speak TTS even if there's no chat text, but only if no board was spoken for
				speakText(ttsCommentary);
				aiResponded = true;
			}

			// Handle cases where nothing usable was sent (and no action signal)
			...
			// Update UI state based on whether AI is now waiting for an answer
			if (state.aiIsWaitingForAnswer) {
				manageUIState('waitingForAnswer');
			} else {
                // Don't switch back to 'learning' if completion was just suggested
		...
		// *** MODIFIED PROMPTS v15: Include instructions for suggesting completion and more examples ***
		const _buildInitialPrompt = () => {
			...
			// Enhanced instruction for examples
...
1.  **Obsah na tabuli ([BOARD_MARKDOWN]):** Všechny klíčové informace (definice, věty, vzorce), **MINIMÁLNĚ DVA ŘEŠENÉ PŘÍKLADY (nejprve jednoduchý, pak složitější)** a ÚLOHY K ŘEŠENÍ MUSÍ být ve formátu Markdown zde. Tabule je HLAVNÍ výukový prostor.
...
(Text do chatu - VOLITELNÉ, velmi krátký, např. "Začněme.")`;
		...
		const _buildContinuePrompt = () => {
			const level = state.currentProfile?.skill_level || 'středně pokročilá';
			const topicName = state.currentTopic?.name || 'Neznámé téma';
			// Enhanced instructions for examples and completion suggestion
