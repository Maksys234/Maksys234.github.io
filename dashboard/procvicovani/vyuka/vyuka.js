(function() { // IIFE для изоляции области видимости
    ...
    try {
        ...
        const ui = {
            ...
            learningInterface: document.querySelector('.call-interface'), // Main container
        ...
        const renderMarkdown = (el, text) => {
             ...
             try {
                 ...
                 // IMPORTANT FIX: Render first, then typeset
                 el.innerHTML = marked.parse(text || '');
                 if (window.MathJax && typeof window.MathJax.typesetPromise === 'function') {
                     // Use timeout 0 to allow browser render cycle before typesetting
                     setTimeout(() => {
        ...
        const loadVoices = () => {
            ...
            try {
                ...
                if (!voices || voices.length === 0) { console.warn("No voices available yet."); return; }
        ...
        const stopSpeech = () => { if (state.speechSynthesisSupported) { window.speechSynthesis.cancel(); ui.aiAvatarCorner?.classList.remove('speaking'); ui.boardSpeakingIndicator?.classList.remove('active'); removeBoardHighlight(); console.log("Speech cancelled."); } };
        ...
        const initializeSpeechRecognition = () => {
            ...
            state.speechRecognition.onresult = (event) => { const transcript = event.results[0][0].transcript; console.log('Speech recognized:', transcript); if (ui.chatInput) { ui.chatInput.value = transcript; autoResizeTextarea(); } };
            state.speechRecognition.onerror = (event) => { console.error('Speech recognition error:', event.error); let errorMsg = "Chyba rozpoznávání řeči"; if (event.error === 'no-speech') errorMsg = "Nerozpoznal jsem žádnou řeč."; else if (event.error === 'audio-capture') errorMsg = "Chyba mikrofonu."; else if (event.error === 'not-allowed' || event.error === 'service-not-allowed') { errorMsg = "Přístup k mikrofonu zamítnut."; if(ui.micBtn) ui.micBtn.disabled = true; } showToast(errorMsg, 'error'); stopListening(); };
            state.speechRecognition.onend = () => { console.log('Speech recognition ended.'); stopListening(); };
        ...
        const startListening = () => {
            ...
            navigator.mediaDevices.getUserMedia({ audio: true })
                .then(() => {
                    try { state.speechRecognition.start(); state.isListening = true; ui.micBtn?.classList.add('listening'); if(ui.micBtn) ui.micBtn.title = "Zastavit hlasový vstup"; console.log('Speech recognition started.'); manageButtonStates(); } // Update button state
                    catch (e) { console.error("Error starting speech recognition:", e); showToast("Nepodařilo se spustit rozpoznávání.", "error"); stopListening(); }
                })
                .catch(err => { console.error("Microphone access denied:", err); showToast("Přístup k mikrofonu je nutný pro hlasový vstup.", "warning"); if(ui.micBtn) ui.micBtn.disabled = true; stopListening(); });
        };

        const stopListening = () => {
            if (!state.speechRecognitionSupported || !state.speechRecognition || !state.isListening) return;
            try { state.speechRecognition.stop(); } catch (e) {}
            finally { state.isListening = false; ui.micBtn?.classList.remove('listening'); if(ui.micBtn) ui.micBtn.title = "Zahájit hlasový vstup"; console.log('Speech recognition stopped.'); manageButtonStates(); } // Update button state
        };
        ...
        const initializeUI = () => {
            try {
                ...
                if (state.speechSynthesisSupported) { if (window.speechSynthesis.getVoices().length > 0) { loadVoices(); } else if (window.speechSynthesis.onvoiceschanged !== undefined) { window.speechSynthesis.onvoiceschanged = loadVoices; } } else { console.warn("Speech Synthesis not supported."); }
        ...
        const manageButtonStates = () => {
            ...
            const canChat = canInteractNormally || (!!state.currentTopic && !state.geminiIsThinking && !state.topicLoadInProgress && state.aiIsWaitingForAnswer); // Allow chat when AI is waiting
            ...
            if (ui.continueBtn) {
                ui.continueBtn.disabled = !canContinueOrComplete; // Disable continue if AI is waiting for answer
                ui.continueBtn.style.display = state.currentTopic ? 'inline-flex' : 'none';
            }
            if (ui.markCompleteBtn) {
                ui.markCompleteBtn.disabled = !canContinueOrComplete; // Disable complete if AI is waiting
            ...
            // Mic Button
            if (ui.micBtn) {
                const canUseMic = canChat && state.speechRecognitionSupported; // Allow mic when waiting for answer
        ...
        const clearWhiteboard = (showToastMsg = true) => { if (!ui.whiteboardContent) return; ui.whiteboardContent.innerHTML = ''; state.boardContentHistory = []; console.log("Whiteboard cleared."); if (showToastMsg) showToast('Vymazáno', "Tabule vymazána.", "info"); };
        const appendToWhiteboard = (markdownContent, commentaryText) => {
            ...
            // IMPORTANT FIX: Render markdown BEFORE appending to DOM
            ...
            if (state.speechSynthesisSupported) { ttsButton.addEventListener('click', (e) => { e.stopPropagation(); speakText(textForSpeech, chunkDiv); }); }
        ...
        const loadNextUncompletedTopic = async () => {
            ...
            try {
                ...
                if (activities && activities.length > 0) {
                    ...
                    let name = activity.title || 'N/A'; let desc = activity.description || '';
                    if (activity.topic_id) { try { const { data: topic, error: topicError } = await state.supabase.from('exam_topics').select('name, description').eq('id', activity.topic_id).single(); if (topicError && topicError.code !== 'PGRST116') throw topicError; if (topic) { name = topic.name || name; desc = topic.description || desc; } } catch(e) { console.warn("Could not fetch topic details:", e); } }
                    ...
                    if (ui.currentTopicDisplay) { ui.currentTopicDisplay.innerHTML = `Téma: <strong>${sanitizeHTML(name)}</strong>`; }
                    await startLearningSession();
                } else { manageUIState('planComplete'); }
            } catch (error) { console.error('Error loading next topic:', error); showToast(`Chyba načítání tématu: ${error.message}`, "error"); manageUIState('error', { errorMessage: error.message }); }
            finally { state.topicLoadInProgress = false; setLoadingState('currentTopic', false); manageButtonStates(); }
        };
        ...
        async function awardPoints(pointsValue) {
            ...
            console.log(`[Points] Awarding ${pointsValue} points to user ${state.currentUser.id}`);
        ...
        const handleMarkTopicComplete = async () => {
            ...
            try {
                ...
                await awardPoints(POINTS_TOPIC_COMPLETE); // AWARD POINTS HERE

                showToast(`Téma "${state.currentTopic.name}" dokončeno.`, "success");
                await loadNextUncompletedTopic();
            } catch (error) { console.error(`Error marking topic complete:`, error); showToast("Chyba při označování tématu jako dokončeného.", "error"); state.topicLoadInProgress = false; manageButtonStates(); }
        };
        ...
        const parseGeminiResponse = (rawText) => {
             const boardMarker = "[BOARD_MARKDOWN]:"; const ttsMarker = "[TTS_COMMENTARY]:";
             let boardMarkdown = ""; let ttsCommentary = ""; let chatText = "";
             const boardStart = rawText.indexOf(boardMarker); const ttsStart = rawText.indexOf(ttsMarker);

             if (boardStart !== -1) { let blockStart = rawText.indexOf("```", boardStart + boardMarker.length); if (blockStart !== -1) { let blockEnd = rawText.indexOf("```", blockStart + 3); if (blockEnd !== -1) { boardMarkdown = rawText.substring(blockStart + 3, blockEnd).trim(); } else { boardMarkdown = rawText.substring(blockStart + 3).trim(); console.warn("parseGeminiResponse: Missing closing ``` for board markdown."); } } }
             if (ttsStart !== -1) { let commentaryEnd = rawText.length; let nextMarkerPos = -1; if (boardStart > ttsStart + ttsMarker.length) { nextMarkerPos = boardStart; } if (nextMarkerPos !== -1) { commentaryEnd = nextMarkerPos; } ttsCommentary = rawText.substring(ttsStart + ttsMarker.length, commentaryEnd).trim(); }

             let lastIndex = 0; let textSegments = []; const markers = [];
             if (boardStart !== -1) markers.push({ start: boardStart, end: boardStart + boardMarker.length + (boardMarkdown.length > 0 ? boardMarkdown.length + 6 : 0) });
             if (ttsStart !== -1) markers.push({ start: ttsStart, end: ttsStart + ttsMarker.length + ttsCommentary.length });
             markers.sort((a, b) => a.start - b.start);
             markers.forEach(marker => { if (marker.start > lastIndex) { textSegments.push(rawText.substring(lastIndex, marker.start)); } lastIndex = marker.end; });
             if (lastIndex < rawText.length) { textSegments.push(rawText.substring(lastIndex)); }
        ...
        const processGeminiResponse = (rawText, timestamp) => {
            ...
            if (!rawText) { handleGeminiError("AI vrátilo prázdnou odpověď.", timestamp); return; }
...
- Pokud jsi právě položil otázku, očekávej odpověď studenta.`;
        ...
        const sendToGemini = async (prompt, isChatInteraction = false) => {
            if (!GEMINI_API_KEY || !GEMINI_API_KEY.startsWith('AIzaSy')) { showToast("Chyba Konfigurace", "Chybí API klíč pro AI.", "error"); updateGeminiThinkingState(false); return; }
            if (!state.currentTopic) { showToast("Chyba", "Není vybráno téma.", "error"); updateGeminiThinkingState(false); return; }
            if (!navigator.onLine) { showToast("Offline", "Nelze komunikovat s AI bez připojení.", "warning"); updateGeminiThinkingState(false); return; }
            ...
            try {
                ...
                if (data.promptFeedback?.blockReason) { throw new Error(`Požadavek blokován: ${data.promptFeedback.blockReason}. Zkuste přeformulovat.`); }
                const candidate = data.candidates?.[0];
                if (!candidate) { throw new Error('AI neposkytlo platnou odpověď.'); }
                if (candidate.finishReason && !["STOP", "MAX_TOKENS"].includes(candidate.finishReason)) { console.warn(`Gemini finishReason: ${candidate.finishReason}.`); if (candidate.finishReason === 'SAFETY') throw new Error('Odpověď blokována bezpečnostním filtrem AI.'); }
                ...
                if (state.geminiChatContext.length > MAX_GEMINI_HISTORY_TURNS * 2 + 2) { // +2 for system message
                    state.geminiChatContext.splice(2, state.geminiChatContext.length - (MAX_GEMINI_HISTORY_TURNS * 2 + 2)); // Keep system message
                }
                processGeminiResponse(text || "", timestamp);

            } catch (error) { console.error('Chyba komunikace s Gemini:', error); showToast(`Chyba AI: ${error.message}`, "error"); handleGeminiError(error.message, timestamp); }
