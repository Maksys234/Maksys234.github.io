...
(function(VyukaApp) { // Используем IIFE для локальной области видимости, передаем VyukaApp
	...
	try {
        ...
        // ZMĚNA v features: Přidání TTS tlačítka a listeneru přímo zde
    	VyukaApp.appendToWhiteboard = (markdownContent, commentaryText) => {
            ...
            // ZMĚNA v features: Uložení textu pro TTS do data atributu tlačítka
        ...
        VyukaApp.handleConfirmCompletion = () => {
            ...
            // Call the function which should be in the other file (vyuka-ai-interaction.js)
            if (typeof VyukaApp.handleMarkTopicComplete === 'function') {
			    VyukaApp.handleMarkTopicComplete(); // Trigger the topic completion logic
            } else {
                console.error("ERROR: VyukaApp.handleMarkTopicComplete is not defined!");
                VyukaApp.showToast("Chyba: Funkce pro dokončení tématu nenalezena.", "error");
            }
		};
        ...
        VyukaApp.checkRequirements = (profileData, requirements) => {
            ...
            try {
                switch (reqType) {
                    ...
                    // ZMĚNA v features: Odebráno completed_tests
        ...
        // ZMĚNA v features: Odebrán sloupec completed_tests z dotazu
        VyukaApp.checkAndAwardAchievements = async (userId) => {
            ...
            try {
                // ZMĚNA v features: Odebráno completed_tests ze SELECT
                ...
                for (const badge of unearnedBadges) {
                    // ZMĚNA v features: Předání profileData do checkRequirements
                    if (VyukaApp.checkRequirements(profileData, badge.requirements)) {
                         ...
                         // ZMĚNA v features: Předání userId, badgeId, badgeTitle, badge.points
        ...
        // ZMĚNA v features: Odebrány delegované listenery pro TTS, nyní jsou přímo v appendToWhiteboard a addChatMessage
		// Этот setup будет вызывать функции, которые могут быть в этом файле ИЛИ в vyuka-ai-interaction.js,
        // так как все они находятся в пространстве имен VyukaApp.
		VyukaApp.setupFeatureListeners = () => {
			const ui = VyukaApp.ui;
			console.log("[SETUP UI Features] Setting up UI/Feature event listeners...");
			// Chat Listeners (Input/Send/Clear/Save are UI actions, but trigger AI interaction)
			if (ui.chatInput) {
                ui.chatInput.addEventListener('input', VyukaApp.autoResizeTextarea);
                ui.chatInput.addEventListener('keypress', (e) => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                        e.preventDefault();
                        if (typeof VyukaApp.handleSendMessage === 'function') {
			...
			if (ui.clearChatBtn) {
                 ui.clearChatBtn.addEventListener('click', () => {
                     if (typeof VyukaApp.confirmClearChat === 'function') {
			...
			if (ui.saveChatBtn) {
                 ui.saveChatBtn.addEventListener('click', () => {
                     if (typeof VyukaApp.saveChatToPDF === 'function') {
                         VyukaApp.saveChatToPDF();
                     } else { console.error("VyukaApp.saveChatToPDF not found"); }
                 });
             }

			// STT/TTS Listeners (UI Features)
            ...
            // TTS listeners are now added dynamically within appendToWhiteboard and addChatMessage

			// Learning Flow Listeners (UI Features triggering potential AI interaction)
			if (ui.continueBtn) {
                ui.continueBtn.addEventListener('click', () => {
                    if (typeof VyukaApp.requestContinue === 'function') {
                        VyukaApp.requestContinue();
			...
			// Notification Listeners (UI Features)
