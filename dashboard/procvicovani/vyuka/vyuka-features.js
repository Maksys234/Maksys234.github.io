...
// Логика специфичных функций: Темы, Чат, Gemini, TTS/STT, Доска, Уведомления, Очки, Модальные окна
...
(function(VyukaApp) { // Используем IIFE для локальной области видимости, передаем VyukaApp
    ...
    const cleanChatMessage = (text) => {
        ...
        const filteredLines = lines.filter(line => { const trimmedLine = line.trim(); return trimmedLine !== '.' && trimmedLine !== '?'; });
        cleanedText = filteredLines.join('\n');
        if (cleanedText.trim() === "(Poslechněte si komentář)") { console.log("[Clean] Removing placeholder text."); return ""; }
    ...
    VyukaApp.loadVoices = () => {
        ...
        try {
            ...
            if (!voices || voices.length === 0) { console.warn("No voices available yet."); setTimeout(VyukaApp.loadVoices, 100); return; }
        ...
        } catch (e) { console.error("Error loading voices:", e); state.czechVoice = null; }
    };
    ...
    VyukaApp.speakText = (text, targetChunkElement = null) => {
        ...
        else { VyukaApp.loadVoices(); if (state.czechVoice) { utterance.voice = state.czechVoice; } else { console.warn("Czech voice not found, using default."); } }
        utterance.onstart = () => { console.log("TTS started."); ui.aiAvatarCorner?.classList.add('speaking'); ui.boardSpeakingIndicator?.classList.add('active'); if (targetChunkElement) { targetChunkElement.classList.add('speaking-highlight'); state.currentlyHighlightedChunk = targetChunkElement; } VyukaApp.manageButtonStates(); };
        utterance.onend = () => { console.log("TTS finished."); ui.aiAvatarCorner?.classList.remove('speaking'); ui.boardSpeakingIndicator?.classList.remove('active'); VyukaApp.removeBoardHighlight(); VyukaApp.manageButtonStates(); };
    ...
    VyukaApp.stopSpeech = () => {
        const state = VyukaApp.state;
        const ui = VyukaApp.ui;
        if (state.speechSynthesisSupported) { window.speechSynthesis.cancel(); ui.aiAvatarCorner?.classList.remove('speaking'); ui.boardSpeakingIndicator?.classList.remove('active'); VyukaApp.removeBoardHighlight(); console.log("Speech cancelled."); VyukaApp.manageButtonStates(); }
    };
    VyukaApp.initializeSpeechRecognition = () => {
        ...
        state.speechRecognition.onresult = (event) => { const transcript = event.results[0][0].transcript; console.log('Speech recognized:', transcript); if (ui.chatInput) { ui.chatInput.value = transcript; VyukaApp.autoResizeTextarea(); } };
    ...
    VyukaApp.stopListening = () => {
        ...
        const ui = VyukaApp.ui;
        if (!state.speechRecognitionSupported || !state.speechRecognition || !state.isListening) return;
        try { state.speechRecognition.stop(); } catch (e) {} finally { state.isListening = false; ui.micBtn?.classList.remove('listening'); if(ui.micBtn) ui.micBtn.title = "Zahájit hlasový vstup"; console.log('Speech recognition stopped.'); VyukaApp.manageButtonStates(); }
    };
    VyukaApp.handleMicClick = () => { const state = VyukaApp.state; if (!state.speechRecognitionSupported) { VyukaApp.showToast("Rozpoznávání řeči není podporováno.", "warning"); return; } if (state.isListening) { VyukaApp.stopListening(); } else { VyukaApp.startListening(); } };

    // --- Whiteboard ---
    VyukaApp.clearWhiteboard = (showToastMsg = true) => { const ui = VyukaApp.ui; const state = VyukaApp.state; if (!ui.whiteboardContent) return; ui.whiteboardContent.innerHTML = ''; state.boardContentHistory = []; console.log("Whiteboard cleared."); if (showToastMsg) VyukaApp.showToast('Vymazáno', "Tabule vymazána.", "info"); VyukaApp.manageButtonStates(); };
	VyukaApp.appendToWhiteboard = (markdownContent, commentaryText) => {
		...
		VyukaApp.renderMarkdown(contentDiv, originalText, false);
		...
		if (state.speechSynthesisSupported) { ttsButton.addEventListener('click', (e) => { e.stopPropagation(); VyukaApp.speakText(textForSpeech, chunkDiv); }); chunkDiv.appendChild(ttsButton); }
		...
		if (typeof chunkDiv.scrollIntoView === 'function') { chunkDiv.scrollIntoView({ behavior: 'smooth', block: 'start' }); console.log("Scrolled to start of new whiteboard chunk."); }
		else { ui.whiteboardContainer.scrollTop = chunkDiv.offsetTop; console.warn("scrollIntoView not fully supported, using offsetTop fallback."); }
    ...
    VyukaApp.triggerWhiteboardMathJax = () => {
		...
		if (ui.whiteboardContent && window.MathJax && typeof window.MathJax.typesetPromise === 'function') {
			...
			setTimeout(() => { window.MathJax.typesetPromise([ui.whiteboardContent]).then(() => console.log("[MathJax v19] Whiteboard typeset completed.")).catch(e => console.error("[MathJax v19] Whiteboard typeset error:", e)); }, 100);
    ...
    VyukaApp.loadNextUncompletedTopic = async () => {
        ...
        try {
            ...
            if (!plans || plans.length === 0) { VyukaApp.manageUIState('noPlan'); return; }
            ...
            if (activities && activities.length > 0) {
                ...
                if (activity.topic_id) {
                    try {
                        const { data: topic, error: topicError } = await state.supabase.from('exam_topics').select('name, description').eq('id', activity.topic_id).single();
                        if (topicError && topicError.code !== 'PGRST116') throw topicError;
                        if (topic) { name = topic.name || name; desc = topic.description || desc; }
                    } catch(e) { console.warn("Could not fetch topic details:", e); }
                }
                state.currentTopic = { activity_id: activity.id, plan_id: state.currentPlanId, name: name, description: desc, user_id: state.currentUser.id, topic_id: activity.topic_id };
                if (ui.currentTopicDisplay) { ui.currentTopicDisplay.innerHTML = `Téma: <strong>${VyukaApp.sanitizeHTML(name)}</strong>`; }
                await VyukaApp.startLearningSession(); // Call function from features.js
            } else { VyukaApp.manageUIState('planComplete'); }
        } catch (error) { console.error('Error loading next topic:', error); VyukaApp.showToast(`Chyba načítání tématu: ${error.message}`, "error"); VyukaApp.manageUIState('error', { errorMessage: error.message }); }
        finally { state.topicLoadInProgress = false; VyukaApp.setLoadingState('currentTopic', false); }
    };
    ...
    VyukaApp.awardPoints = async (pointsValue) => {
		...
		try {
			...
			if (data && data.points === newPoints) { state.currentProfile.points = newPoints; console.log(`[Points v19] User ${userId} points updated successfully in DB and state to ${newPoints}.`); VyukaApp.showToast('+', `${pointsValue} kreditů získáno!`, 'success', 3000); VyukaApp.updateUserInfoUI(); }
			else { console.warn(`[Points v19] DB update discrepancy for user ${userId}. Expected ${newPoints}, got ${data?.points}. State NOT updated locally.`); VyukaApp.showToast('Varování', 'Nekonzistence při aktualizaci kreditů.', 'warning'); }
    ...
    VyukaApp.startLearningSession = async () => { const state = VyukaApp.state; if (!state.currentTopic) return; state.currentSessionId = VyukaApp.generateSessionId(); VyukaApp.clearInitialChatState(); VyukaApp.manageUIState('requestingExplanation'); const prompt = VyukaApp._buildInitialPrompt(); await VyukaApp.sendToGemini(prompt); };
	VyukaApp.requestContinue = async () => { const state = VyukaApp.state; console.log("[RequestContinue] Triggered. AI Waiting:", state.aiIsWaitingForAnswer, "AI Suggested Completion:", state.aiSuggestedCompletion); if (state.geminiIsThinking || !state.currentTopic) return; if (state.aiIsWaitingForAnswer) { VyukaApp.showToast("Nejprve odpovězte na úlohu v chatu.", "warning", 3000); console.warn("[RequestContinue] Blocked: AI is waiting for an answer."); return; } if (state.aiSuggestedCompletion) { VyukaApp.showToast("AI navrhlo dokončení tématu. Pro dokončení použijte modální okno nebo požádejte AI o pokračování.", "info"); return; } const prompt = VyukaApp._buildContinuePrompt(); await VyukaApp.sendToGemini(prompt); };
	VyukaApp.addChatMessage = async (displayMessage, sender, saveToDb = true, timestamp = new Date(), ttsText = null, originalContent = null) => {
        ...
        let avatarContent = sender === 'user' ? VyukaApp.getInitials(state.currentProfile, state.currentUser?.email) : 'AI'; // Get avatar HTML
		...
		if (window.MathJax && typeof window.MathJax.typesetPromise === 'function' && (displayMessage.includes('$') || displayMessage.includes('\\'))) { console.log(`[MathJax v19] Queueing typeset for chat message bubble: ${id}`); setTimeout(() => { window.MathJax.typesetPromise([bubbleContentDiv]).then(() => console.log(`[MathJax v19] Typeset successful for chat bubble ${id}`)).catch((err) => console.error(`[MathJax v19] Typeset error for chat bubble ${id}: ${err.message}`)); }, 0); }
		div.scrollIntoView({ behavior: 'smooth', block: 'end' });
		requestAnimationFrame(() => { div.style.opacity = '1'; });
		VyukaApp.initTooltips();
		const contentToSave = originalContent !== null ? originalContent : displayMessage;
		if (saveToDb && state.supabase && state.currentUser && state.currentTopic && state.currentSessionId) { try { await state.supabase.from('chat_history').insert({ user_id: state.currentUser.id, session_id: state.currentSessionId, topic_id: state.currentTopic.topic_id, topic_name: state.currentTopic.name, role: sender === 'gemini' ? 'model' : 'user', content: contentToSave }); } catch (e) { console.error("Chat save error:", e); VyukaApp.showToast("Chyba ukládání chatu.", "error"); } }
		VyukaApp.manageButtonStates();
	};
    VyukaApp.addThinkingIndicator = () => { const ui = VyukaApp.ui; const state = VyukaApp.state; if (state.thinkingIndicatorId || !ui.chatMessages) return; VyukaApp.clearInitialChatState(); const id = `thinking-${Date.now()}`; const div = document.createElement('div'); div.className = 'chat-message model'; div.id = id; div.innerHTML = `<div class="message-avatar">AI</div><div class="message-thinking-indicator"><span class="typing-dot"></span><span class="typing-dot"></span><span class="typing-dot"></span></div>`; ui.chatMessages.appendChild(div); div.scrollIntoView({ behavior: 'smooth', block: 'end' }); state.thinkingIndicatorId = id; VyukaApp.manageButtonStates(); };
    ...
    VyukaApp.parseGeminiResponse = (rawText) => {
        ...
        if (actionMatch) { actionSignal = 'SUGGEST_COMPLETION'; remainingText = remainingText.replace(actionMatch[0], "").trim(); console.log(`[ParseGemini v19] Found action signal: ${actionSignal}`); if (remainingText.length === 0) { return { boardMarkdown: "", ttsCommentary: "", chatText: "", actionSignal }; } }
        const boardMatch = remainingText.match(boardRegex); console.log("[ParseGemini v19] Board Regex Match:", boardMatch);
        if (boardMatch) { boardMarkdown = (boardMatch[2] || boardMatch[3] || "").trim(); console.log(`[ParseGemini v19] Extracted Board Content (Raw): "${boardMarkdown.substring(0,70)}..."`); remainingText = remainingText.replace(boardMatch[0], ""); console.log(`[ParseGemini v19] Found board content. Length: ${boardMarkdown.length}`); if (boardMarkdown.toLowerCase().startsWith('markdown')) { const potentialNewlineIndex = boardMarkdown.indexOf('\n'); if (potentialNewlineIndex !== -1 && potentialNewlineIndex < 15) { boardMarkdown = boardMarkdown.substring(potentialNewlineIndex + 1).trim(); console.warn("[ParseGemini v19] Cleaned leading 'markdown' word."); } else if (boardMarkdown.length < 15) { boardMarkdown = ""; console.warn("[ParseGemini v19] Discarded short content starting with 'markdown'."); } } }
        else { console.log(`[ParseGemini v19] Marker "${boardMarker}" not found or malformed.`); }
        const ttsMatch = remainingText.match(ttsRegex);
        if (ttsMatch) { ttsCommentary = (ttsMatch[2] || ttsMatch[3] || "").trim(); remainingText = remainingText.replace(ttsMatch[0], ""); console.log(`[ParseGemini v19] Found TTS content. Length: ${ttsCommentary.length}`); }
        else { console.log(`[ParseGemini v19] Marker "${ttsMarker}" not found or malformed.`); }
        let chatText = remainingText.replace(/```(markdown)?\s*|\s*```/g, '').replace(/\[BOARD_MARKDOWN]:/gi, '').replace(/\[TTS_COMMENTARY]:/gi, '').replace(/\[ACTION:SUGGEST_COMPLETION]/gi, '').trim();
	...
	VyukaApp.setupFeatureListeners = () => {
        ...
        // Chat message delegation for TTS
