...
(function() {
    ...
    let allTitles = []; // <<< NEW: Store fetched titles globally
    let isLoading = { stats: false, activities: false, notifications: false, titles: false }; // Added 'titles'
    const SIDEBAR_STATE_KEY = 'sidebarCollapsedState';
    const DAILY_LOGIN_BASE_REWARD = 5; // Base points for day 1
    const DAILY_LOGIN_STREAK_MULTIPLIER = 2; // Points = BASE + (days-1)*MULTIPLIER

    // DOM Elements Cache (Added sidebar user title)
    const ui = {
        // Loaders & Overlays
    ...
    // Визуальные настройки для активностей (без изменений)
    ...
    // (showToast, showError, hideError, updateOnlineStatus, getInitials, formatRelativeTime, openMenu, closeMenu, sanitizeHTML, setLoadingState, initMouseFollower, initScrollAnimations, initHeaderScrollDetection, updateCopyrightYear, applyInitialSidebarState, toggleSidebar)
    // Most helpers remain the same, only setLoadingState needs 'titles' key added
    ...
    function setLoadingState(section, isLoadingFlag) {
         const sections = section === 'all' ? ['stats', 'activities', 'notifications', 'titles'] : [section]; // Added 'titles'
         sections.forEach(sec => {
             ...
             // Note: 'titles' loading doesn't have a direct visual indicator in this context, but we track the state.
         });
     }
    ...
    const initHeaderScrollDetection = () => { /* ... (same as before) ... */ let lastScrollY = window.scrollY; window.addEventListener('scroll', () => { const currentScrollY = window.scrollY; if (currentScrollY > 50) { document.body.classList.add('scrolled'); } else { document.body.classList.remove('scrolled'); } lastScrollY = currentScrollY <= 0 ? 0 : currentScrollY; }, { passive: true }); if (window.scrollY > 50) document.body.classList.add('scrolled'); };
    ...
    function applyInitialSidebarState() { /* ... (same as before) ... */ const savedState = localStorage.getItem(SIDEBAR_STATE_KEY); if (savedState === 'collapsed') { document.body.classList.add('sidebar-collapsed'); console.log("[Sidebar State] Initial state applied: collapsed"); } else { document.body.classList.remove('sidebar-collapsed'); console.log("[Sidebar State] Initial state applied: expanded"); } }
    function toggleSidebar() { /* ... (same as before) ... */ const isCollapsed = document.body.classList.toggle('sidebar-collapsed'); localStorage.setItem(SIDEBAR_STATE_KEY, isCollapsed ? 'collapsed' : 'expanded'); console.log(`[Sidebar Toggle] Sidebar toggled. New state: ${isCollapsed ? 'collapsed' : 'expanded'}`); }
    ...
    async function initializeApp() {
        ...
        try {
            ...
            if (session?.user) {
                ...
                console.log(`[INIT Dashboard] User authenticated (ID: ${currentUser.id}). Loading data...`);

                // Fetch profile and titles concurrently
                const [profileResult, titlesResult] = await Promise.allSettled([
                    fetchUserProfile(currentUser.id),
                    fetchTitles() // <<< NEW: Fetch titles
                ]);
                ...
                if (profileResult.status === 'fulfilled' && profileResult.value) {
                    ...
                    // Attempt to create a default profile if none exists (basic structure)
                    ...
                    try {
                         ...
                         if (!currentProfile) throw new Error("Nepodařilo se vytvořit výchozí profil.");
                         console.log("[INIT Dashboard] Default profile created/retrieved.");
                    } catch (creationError) {
                         console.error("CRITICAL: Failed to load or create profile:", creationError);
                         showError("Nepodařilo se načíst/vytvořit základní data uživatele.", true);
                         throw creationError; // Stop initialization if profile is essential
                    }
                }
                ...
                // Perform daily login check AFTER profile is loaded
                await handleDailyLoginCheck(); // <<< NEW: Call login check

                updateSidebarProfile(currentProfile); // Now updates title as well
    ...
    async function fetchUserProfile(userId) {
        ...
        setLoadingState('stats', true); // Show loading for stats as profile contains some stats
        try {
            const { data: profile, error } = await supabase
                .from('profiles')
                // <<< MODIFIED: Select all needed fields, including new login ones >>>
    ...
    // <<< NEW: Function to create a default profile if none exists >>>
    async function createDefaultProfile(userId, userEmail) {
        ...
        try {
            const defaultData = {
                 ...
                 level: 1,
                 points: 0,
                 experience: 0, // Assuming experience starts at 0
                 badges_count: 0,
                 streak_days: 0,
                 consecutive_login_days: 0, // Initialize new field
                 last_daily_login: null,   // Initialize new field
            ...
            const { data: newProfile, error } = await supabase
                .from('profiles')
                .insert(defaultData)
                .select('*, selected_title, last_daily_login, consecutive_login_days') // Select all needed fields, including new ones
                .single();
            if (error) {
                 // Handle potential duplicate error if profile was created concurrently
                 if (error.code === '23505') { // Unique violation
                     console.warn("[Profile Create] Profile likely already exists, fetching again.");
                     return await fetchUserProfile(userId); // Re-fetch
                 }
                 throw error;
            }
            ...
            console.error("[Profile Create] Failed to create default profile:", error);
             showToast('Chyba', 'Nepodařilo se vytvořit profil.', 'error');
            return null;
        }
    }


    // <<< NEW: Function to fetch titles (reused from oceneni.js logic) >>>
    ...
    async function handleDailyLoginCheck() {
        ...
        const lastLoginDate = lastLoginTimestamp ? new Date(new Date(lastLoginTimestamp).getFullYear(), new Date(lastLoginTimestamp).getMonth(), new Date(lastLoginTimestamp).getDate()) : null; // Start of last login day
        ...
        if (!lastLoginDate || !isSameDay(lastLoginDate, today)) { // First login today (or ever)
             needsDbUpdate = true;
             if (lastLoginDate && isYesterday(lastLoginDate, today)) {
                 ...
                 // First login or non-consecutive login
        ...
        if (needsDbUpdate) {
            try {
                ...
                const { data: updatedProfile, error } = await supabase
                    ...
                    .select('*, selected_title, last_daily_login, consecutive_login_days') // Re-select needed fields
                    .single();

                if (error) throw error;

                console.log("[Login Check] Profile updated successfully in DB.");
                currentProfile = updatedProfile; // Update local profile state
                updateSidebarProfile(currentProfile); // Update UI immediately

                if (showRewardToast && pointsToAdd > 0) {
                    showToast(`Denní Odměna! (+${pointsToAdd} kr.)`, `Vaše série je ${newConsecutiveDays} ${newConsecutiveDays === 1 ? 'den' : 'dní'}!`, 'success', 5000);
                }

                 // Optionally, trigger achievement check after points update
                 if (typeof window.VyukaApp?.checkAndAwardAchievements === 'function') { // Assuming check function might be globally available
                     console.log('[Login Check] Triggering achievement check after login reward.');
    ...
    async function loadDashboardData(user, profile) {
         ...
         renderActivitySkeletons(5); // Show activity skeletons immediately

         try {
             updateSidebarProfile(profile); // Initial update with potentially stale points/streak
             console.log("[MAIN] loadDashboardData: Načítání statistik, aktivit, oznámení souběžně...");
             const results = await Promise.allSettled([
                 fetchUserStats(user.id, profile), // Re-fetch stats which might include points updated by login check
             ...
             if (results[0].status === 'fulfilled') {
                  updateStatsCards(results[0].value || profile); // Use updated stats, fallback to profile if fetch failed
             } else { console.error("❌ Chyba při načítání statistik uživatele:", results[0].reason); showError("Nepodařilo se načíst statistiky."); updateStatsCards(profile); } // Fallback to profile data
             setLoadingState('stats', false);
    ...
    function updateSidebarProfile(profile) {
        ...
        // <<< MODIFIED: Added check for sidebarUserTitle >>>
    ...
    function setupUIEventListeners() { /* ... (added sidebar toggle listener) ... */
         ...
         if (ui.sidebarToggleBtn) ui.sidebarToggleBtn.addEventListener('click', toggleSidebar); // <<< NEW: Sidebar toggle listener
