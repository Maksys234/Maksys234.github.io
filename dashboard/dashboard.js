...
(function() {
    ...
    // --- MODIFIED: Ensure window.supabase exists and initialize client property ---
    // Initialize the global namespace if it doesn't exist
    window.supabase = window.supabase || {};
    // Initialize the client property to null within the global namespace
    window.supabase.client = null;
    // Keep local variable 'supabase' for internal use within this script
    let supabase = null;
    // --- END MODIFIED ---
    ...
    let userStats = null; // Changed from currentUserStats for clarity
    let recentActivities = [];
    let isLoading = {
        stats: false,
        activities: false,
        notifications: false,
        titles: false, // Added title loading state
        monthlyRewards: false, // Added monthly rewards loading state
        streakMilestones: false // Added streak milestones loading state
    };
    const SIDEBAR_STATE_KEY = 'sidebarCollapsedState'; // Consistent key
    const NOTIFICATION_FETCH_LIMIT = 5; // Limit for notifications dropdown

    // DOM Elements Cache
    const ui = {}; // Cache object

    // --- MODIFIED: cacheDOMElements function ---
    // Handles missing elements more gracefully
    function cacheDOMElements() {
        console.log("[CACHE DOM] Caching elements...");
        // Elements primarily expected on dashboard.html
        const dashboardElementIds = [
            'dashboard-title', 'welcome-title', 'start-practice-btn', 'open-monthly-modal-btn',
            'open-streak-modal-btn', 'progress-card', 'points-card', 'streak-card',
            'activity-list-container', 'activity-list', 'monthly-reward-modal',
            'modal-monthly-calendar-grid', 'modal-monthly-calendar-empty', 'modal-current-month-year',
            'close-monthly-modal-btn', 'streak-milestones-modal', 'modal-milestones-grid',
            'modal-milestones-empty', 'modal-current-streak-value', 'close-streak-modal-btn'
            // Add other potential IDs from dashboard.html if needed
        ];
        // Elements expected on most/all dashboard pages
        const commonElementIds = [
            ...
            'refresh-data-btn', 'notification-bell', 'notification-count', 'notifications-dropdown',
            'notifications-list', 'no-notifications-msg', 'mark-all-read', 'toast-container',
            'global-error', 'offline-banner', 'mouse-follower', 'currentYearSidebar', 'currentYearFooter'
        ];
        // Selectors (potentially NodeList)
        const elementQueries = {
            dashboardHeader: '.dashboard-header', // Common
            statCards: '#stats-section .stat-card', // Dashboard specific
            activityListEmptyState: '#activity-list-container .empty-state', // Dashboard specific
            activityListErrorState: '#activity-list-container .card-error-state' // Dashboard specific
        };

        const allIds = [...commonElementIds, ...dashboardElementIds];
        const notFound = [];
        ...
        allIds.forEach(id => {
            ...
            if (element) {
                ui[key] = element;
                 ...
                 // Only warn if *common* elements are missing, otherwise just note it
                 if (commonElementIds.includes(id)) {
                    console.warn(`[CACHE DOM] Common element with ID '${id}' not found.`);
                 }
                notFound.push(id);
                ui[key] = null; // Set to null if not found to avoid errors later
            }
        });
        ...
        for (const key in elementQueries) {
            ...
            if (elements && elements.length > 0) {
                ...
                // Log missing query elements as warnings
        ...
        // Log all missing elements at the end (useful for debugging specific pages)
        if (notFound.length > 0) {
             console.log(`[CACHE DOM] Следующие элементы не были найдены: (${notFound.length}) ['${notFound.join("', '")}']`);
        }

        console.log("[CACHE DOM] Caching complete.");
        return true; // Always return true, allowing initialization even if dashboard-specific elements are missing
    }
    // --- END MODIFIED ---
    ...
    // --- Helper Functions (No changes needed for core helpers) ---
    ...
    function showError(message, isGlobal = false) { console.error("Došlo k chybě:", message); if (isGlobal && ui.globalError) { ui.globalError.innerHTML = `<div class="error-message"><i class="fas fa-exclamation-triangle"></i><div>${sanitizeHTML(message)}</div><button class="retry-button btn" onclick="location.reload()">Obnovit Stránku</button></div>`; ui.globalError.style.display = 'block'; } else { showToast('CHYBA SYSTÉMU', message, 'error', 6000); } }
    ...
    function setLoadingState(section, isLoadingFlag) { const sections = section === 'all' ? Object.keys(isLoading) : [section]; sections.forEach(sec => { if (isLoading[sec] === isLoadingFlag && section !== 'all') return; isLoading[sec] = isLoadingFlag; console.log(`[setLoadingState] Section: ${sec}, isLoading: ${isLoadingFlag}`); const element = { stats: ui.statCards, activities: ui.activityListContainer, notifications: ui.notificationsList, monthlyRewards: ui.modalMonthlyCalendarGrid, streakMilestones: ui.modalMilestonesGrid }[sec]; const emptyElement = { activities: ui.activityListEmptyState, notifications: ui.noNotificationsMsg, monthlyRewards: ui.modalMonthlyCalendarEmpty, streakMilestones: ui.modalMilestonesEmpty }[sec]; const loaderElement = { monthlyRewards: ui.monthlyRewardModal?.querySelector('.loading-overlay'), streakMilestones: ui.streakMilestonesModal?.querySelector('.loading-overlay'), activities: ui.activityListContainer /* Use container class */ }[sec]; const loaderOverlay = loaderElement?.matches('.loading-overlay') ? loaderElement : null; if (sec === 'stats') { (element || []).forEach(card => card?.classList.toggle('loading', isLoadingFlag)); } else if (element || loaderElement) { const container = loaderOverlay ? loaderElement.parentElement : (loaderElement || element?.parentElement); container?.classList.toggle('loading', isLoadingFlag); if (loaderOverlay) loaderOverlay.classList.toggle('hidden', !isLoadingFlag); if (isLoadingFlag) { if(element) element.innerHTML = ''; if(emptyElement) emptyElement.style.display = 'none'; if (sec === 'activities') renderActivitySkeletons(5); else if (sec === 'monthlyRewards') renderMonthlyCalendarSkeletons(); else if (sec === 'streakMilestones') renderMilestoneSkeletons(); else if (sec === 'notifications') renderNotificationSkeletons(2); } else { if (element && !element.hasChildNodes() && emptyElement) { emptyElement.style.display = 'block'; } } } if (sec === 'notifications' && ui.notificationBell) { ui.notificationBell.style.opacity = isLoadingFlag ? 0.5 : 1; if (ui.markAllReadBtn) { const currentUnreadCount = parseInt(ui.notificationCount?.textContent?.replace('+', '') || '0'); ui.markAllReadBtn.disabled = isLoadingFlag || currentUnreadCount === 0; } } }); }
    ...
    // --- Supabase Initialization (MODIFIED) ---
    function initializeSupabase() {
        ...
        try {
            ...
            if (!supabase) {
                throw new Error("Vytvoření klienta Supabase selhalo.");
            }
            // Expose client globally
            window.supabase.client = supabase; // Assign to the pre-initialized global object
            ...
            // Ensure client is null on failure in both scopes
            window.supabase.client = null;
            supabase = null;
            return false;
        }
    ...
    // --- Data Loading (No changes needed, keep existing functions) ---
    ...
    function isYesterday(date1, date2) { const yesterday = new Date(date2); yesterday.setDate(date2.getDate() - 1); return isSameDate(date1, yesterday); }
    function getCurrentMonthYearString() { const now = new Date(); return `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`; }
    ...
    function renderMonthlyCalendar() { /* ... (keep existing) ... */ }
    function renderMonthlyCalendarSkeletons() { /* ... (keep existing) ... */ }
    function renderStreakMilestones() { /* ... (keep existing) ... */ }
    function renderMilestoneSkeletons() { /* ... (keep existing) ... */ }
    function showModal(modalId) { /* ... (keep existing) ... */ }
    function hideModal(modalId) { /* ... (keep existing) ... */ }

    // --- Reward Claiming Logic (Keep existing) ---
    async function claimMonthlyReward(day, buttonElement) { /* ... (keep existing) ... */ }
    async function claimMilestoneReward(milestoneDay, buttonElement) { /* ... (keep existing) ... */ }
    ...
    async function initializeApp() {
        ...
        if (!cacheDOMElements()) {
            console.warn("[INIT Dashboard] DOM caching finished with some missing elements (potentially expected).");
        }

        // Attempt to initialize Supabase and set the global client
        if (!initializeSupabase()) {
             if (ui.initialLoader) { ui.initialLoader.classList.add('hidden'); setTimeout(() => { ui.initialLoader.style.display = 'none'; }, 500); }
        ...
        try {
            ...
            // Use the locally scoped 'supabase' which is now guaranteed to be the client or null
            if (!supabase) { // Check the local variable
                throw new Error("Supabase client is not available after initialization attempt.");
            }
            ...
            if (session?.user) {
                ...
                // Load dashboard data only if on the dashboard page (check for a specific dashboard element)
                if (ui.welcomeTitle) { // Use welcomeTitle as the indicator for dashboard.html
                     console.log("[INIT Dashboard] Dashboard page detected, loading dashboard data...");
                     await loadDashboardData(currentUser, currentProfile);
                 } else {
                     console.log("[INIT Dashboard] Not on main dashboard page, skipping dashboard-specific data load.");
                      // Load common elements like notifications
                ...
                document.dispatchEvent(new CustomEvent('dashboardReady', {
                    detail: {
                        ...
                        titles: allTitles,
                        client: supabase // Pass the initialized client instance
                    }
                }));
                // **** END DISPATCH ****
            ...
            console.error("❌ [INIT Dashboard] Kritická chyba inicializace:", error);
             // Show specific error for Supabase auth failure
             if (error.message.includes("Nepodařilo se ověřit přihlášení")) {
                 showError(error.message + " Zkuste prosím obnovit stránku.", true);
             } else if (error.message.includes("Supabase client is not available")) {
                 // Error already shown in initializeSupabase
             }
             else {
    ...
    // Run the app initialization on DOMContentLoaded
