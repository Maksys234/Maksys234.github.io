...
(function() {
    ...
    function cacheDOMElements() {
        ...
        const elementDefinitions = [
            ...
            { key: 'mainContentAreaPlaceholder', id: 'main-content-area-placeholder', critical: true }, // Zůstává kritický
            ...
            if (targetElement === ui.mainContentAreaPlaceholder && !isGlobal) {
                 ...
                 if (retryBtn) {
                     retryBtn.addEventListener('click', () => {
                         ...
                         initializeApp(); // Restart the whole init
                     });
                 }
            } else {
            ...
            if (ui.mainContentAreaPlaceholder) {
                ui.mainContentAreaPlaceholder.classList.toggle('loading', isLoadingFlag);
                 if(isLoadingFlag) {
                     ...
                     // Placeholder will be hidden by subsequent content loading or error display
                 }
            }
            return;
        }
...
async function initializeApp() {
    ...
    // NEW: Pokus o skrytí initialLoaderu a zobrazení základního UI co nejdříve
    ...
    if (initialLoaderElement) {
        ...
        setTimeout(() => { if (initialLoaderElement) initialLoaderElement.style.display = 'none'; }, 50); // Velmi krátký timeout
    }
    if (sidebarElement) sidebarElement.style.display = 'flex';
    if (headerElement) headerElement.style.display = 'flex';
    if (mainContentElement) mainContentElement.style.display = 'block'; // Zobrazit hlavní oblast

    // Nyní cachovat DOM elementy, když je základní UI viditelné
    try {
        cacheDOMElements();
        ...
        // Skrýt skutečný obsah, dokud se nenahrají data
        ...
        try {
            ...
            if (session?.user) {
                ...
                // Zobrazit skutečné obsahové bloky před načítáním jejich specifických dat
        ...
        const initialLoaderElementForError = document.getElementById('initial-loader'); // Znovu získat, ui nemusí být inicializováno
        if (initialLoaderElementForError && !initialLoaderElementForError.classList.contains('hidden')) {
            ...
            // Pokud initialLoader už není, zkusit zobrazit globální chybu nebo alert
        ...
        if (typeof setLoadingState === 'function') setLoadingState('all', false); // Zkusit vypnout načítání, pokud je funkce dostupná
    }
}
