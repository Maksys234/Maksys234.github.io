// dashboard.js
// Verze: 26.0.8 - Plná verze s optimalizacemi, logováním a timeouty
(function() {
    'use strict';
    ...
    let allTitles = []; // Pro tituly uživatele
    let userStatsData = null; // Pro statistiky

    // NEW: Konfigurace timeoutů (v milisekundách)
    const AUTH_TIMEOUT = 15000; // 15 sekund pro getSession
    const DATA_FETCH_TIMEOUT = 20000; // 20 sekund pro fetchUserProfile/fetchTitles
    ...
    let isLoading = {
        ...
        creditHistory: false,
        notifications: false,
        titles: false, // Přidáno pro sledování načítání titulů
        monthlyRewards: false,
        streakMilestones: false,
        all: false
    };
    const SIDEBAR_STATE_KEY = 'sidebarCollapsedState';
    const MONTHLY_REWARD_DAYS = 31; // Max days for monthly calendar
    ...
    const STREAK_MILESTONES_CONFIG = {
        ...
        // Lze přidat další milníky
    };
    const sortedMilestoneDays = Object.keys(STREAK_MILESTONES_CONFIG).map(Number).sort((a, b) => a - b);

    // Example rewards for May - this should ideally come from a config or DB
    const mayRewards = { // Příklad pro květen
        ...
        14: { type: 'credits', key: 'may_credits_14_day', name: '10 Kreditů (Den 14)', icon: 'fa-coins', value: 10 }, // Příklad pro den 14
        // ... další dny a odměny
    };

    let ui = {}; // Cache for UI elements

    // Mapování typů aktivit na ikony a třídy
    ...
    function cacheDOMElements() {
        ...
        const elementDefinitions = [
            ...
            { key: 'markAllReadBtn', id: 'mark-all-read', critical: false }, // Assuming this is the correct ID
            ...
            { key: 'progressCard', id: 'progress-card', critical: false }, // Might be inside statsCardsContainer
            ...
            { key: 'pointsCard', id: 'points-card', critical: false }, // Might be inside statsCardsContainer
            { key: 'totalPointsValue', id: 'total-points-value', critical: false },
            { key: 'totalPointsFooter', id: 'total-points-footer', critical: false },
            { key: 'streakCard', id: 'streak-card', critical: false }, // Might be inside statsCardsContainer
        ...
        elementDefinitions.forEach(def => {
            ...
            if (element) {
                ...
                ui[def.key] = null; // Store null if not found
                if (def.critical) {
        ...
        // Fallback for stats cards if container is found but individual cards are not by ID
    ...
    function showError(message, isGlobal = false, targetElement = ui.globalError) {
        ...
        if (errorDisplayElement) {
            ...
            // Fallback if no specific error element is found, show a general toast
    ...
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        if (sectionKey === 'stats') {
            ...
            if (isLoadingFlag && ui.notificationsList && typeof renderNotificationSkeletons === 'function') {
                renderNotificationSkeletons(2); // Počet skeletonů
                if(ui.noNotificationsMsg) ui.noNotificationsMsg.style.display = 'none'; // Skrýt "žádné zprávy"
            } else if (!isLoadingFlag && ui.notificationsList && ui.noNotificationsMsg && ui.notificationsList.children.length === 0) {
                 // Po načtení, pokud je seznam prázdný, zobrazit "žádné zprávy"
                 if(ui.noNotificationsMsg) ui.noNotificationsMsg.style.display = 'block';
            }
        } else if (sectionKey === 'activities') {
            // Ovládání .loading-section pro obalovač seznamu aktivit
            ...
            // Ovládání .loading-section pro obalovač historie kreditů
            ...
            const loaderOverlay = modalBody?.querySelector('.loading-overlay'); // Předpokládáme, že máte .loading-overlay v HTML
            if (loaderOverlay) loaderOverlay.classList.toggle('hidden', !isLoadingFlag);
            if (modalBody) modalBody.classList.toggle('loading', isLoadingFlag); // Třída pro ztmavení/stylizaci

            if (isLoadingFlag) {
                // Zde byste mohli volat funkce pro renderování skeletonů specifických pro modální okna
            ...
            // Může být tichý stav načítání, bez specifického UI indikátoru
            console.log(`[SetLoadingState] Loading state for 'titles' is now ${isLoadingFlag}.`);
        } else {
            // Fallback pro neznámé sekce
    ...
    function applyInitialSidebarState() {
        const startTime = performance.now();
        if (!ui.sidebarToggleBtn) { console.warn("[Sidebar State] Sidebar toggle button (#sidebar-toggle-btn) not found for initial state application."); return; }
        try {
            ...
            if (icon) {
                ...
                const endTime = performance.now();
                console.log(`[Sidebar State] Initial icon and attributes set. Icon class: ${icon.className}. Time: ${(endTime - startTime).toFixed(2)}ms`);
            } else { console.warn("[Sidebar State] Sidebar toggle button icon not found for initial state update."); }
        } catch (error) { console.error("[Sidebar State] Error applying initial state:", error); document.body.classList.remove('sidebar-collapsed'); }
    }
    ...
    function initializeSupabase() {
        ...
        try {
            ...
            window.supabaseClient = supabase; // Make it globally accessible if needed by other scripts
            const endTime = performance.now();
            console.log(`[Supabase] Klient úspěšně inicializován a globálně dostupný. Time: ${(endTime - startTime).toFixed(2)}ms`);
            return true;
        } catch (error) {
    ...
    function withTimeout(promise, ms, timeoutError = new Error('Operace vypršela')) {
    ...
    async function fetchUserStats(userId, profileData) {
        const startTime = performance.now();
        if (!supabase || !userId || !profileData) { console.error("[Stats] Chybí Supabase klient, ID uživatele nebo data profilu."); return null; }
        console.log(`[Stats] Načítání statistik pro uživatele ${userId}...`);
        let fetchedStats = null; let statsError = null;
        try {
            const { data, error } = await supabase.from('user_stats').select('progress, progress_weekly, points_weekly, streak_longest, completed_tests').eq('user_id', userId).maybeSingle();
            fetchedStats = data; statsError = error;
            if (statsError) { console.warn("[Stats] Chyba Supabase při načítání user_stats:", statsError.message); }
        } catch (error) { console.error("[Stats] Neočekávaná chyba při načítání user_stats:", error); statsError = error; }
        ...
        const finalStats = {
            ...
            longest_streak_days: profileData.longest_streak_days ?? Math.max(fetchedStats?.streak_longest ?? 0, profileData.streak_days ?? 0),
        ...
        const endTime = performance.now();
        if (statsError) { console.warn(`[Stats] Vracení statistik primárně z profilu kvůli chybě načítání. Time: ${(endTime - startTime).toFixed(2)}ms`); }
        else { console.log(`[Stats] Statistiky úspěšně načteny/sestaveny. Time: ${(endTime - startTime).toFixed(2)}ms`, finalStats); }
        return finalStats;
    }
    ...
    async function checkAndUpdateLoginStreak() {
        ...
        if (!lastLogin || !isSameDate(today, lastLogin)) { // Není-li poslední přihlášení dnes
            needsDbUpdate = true;
            console.log("[StreakCheck] First login of the day detected.");
            if (lastLogin && isYesterday(lastLogin, today)) {
                currentStreak++;
                console.log(`[StreakCheck] Streak continued! New current streak: ${currentStreak}`);
            } else if (lastLogin) { // Pokud bylo přihlášení, ale ne včera
                currentStreak = 1; // Reset série
                console.log("[StreakCheck] Streak broken. Resetting to 1.");
            } else { // První přihlášení vůbec
                currentStreak = 1;
            ...
            // Oprava logiky: Pokud je aktuální série větší než nejdelší (což by se nemělo stát, pokud je logika správná, ale pro jistotu)
            if (currentProfile.streak_days > (currentProfile.longest_streak_days || 0) ) {
                console.warn(`[StreakCheck] Discrepancy found: streak_days (<span class="math-inline">\{currentProfile\.streak\_days\}\) \> longest\_streak\_days \(</span>{currentProfile.longest_streak_days || 0}). Updating longest_streak.`);
                updateData.longest_streak_days = currentProfile.streak_days;
                longestStreak = currentProfile.streak_days; // Oprava
                needsDbUpdate = true;
            }
        }
        // Update local profile cache immediately
        currentProfile.streak_days = currentStreak;
        currentProfile.longest_streak_days = longestStreak;
        if (ui.modalCurrentStreakValue) ui.modalCurrentStreakValue.textContent = currentStreak; // Update modal if open

        // Zkontrolujte a inicializujte měsíční odměny, pokud je to první přihlášení v měsíci
        currentProfile.monthly_claims = currentProfile.monthly_claims || {}; // Jistota
        if (!currentProfile.monthly_claims[currentMonthYear]) {
            ...
            currentProfile.monthly_claims = updatedClaims; // Aktualizace lokálního profilu
            updateData.monthly_claims = updatedClaims; // Přidání do DB payloadu
            needsDbUpdate = true;
        } else {
        ...
        if (needsDbUpdate) {
            ...
            try {
                ...
                // Aktualizace lokálního profilu po úspěšném zápisu do DB
                ...
                const endTime = performance.now();
                console.log(`[StreakCheck] Profile updated successfully in DB. Time: ${(endTime - startTime).toFixed(2)}ms`);
                return true; // True if DB update occurred
            } catch (error) {
        ...
        return false; // False if no DB update was needed
    }
    ...
    async function updateLastMilestoneClaimedInDB(milestoneDay, rewardKey, rewardName) {
        ...
        try {
            // Nejprve zkontrolujeme, zda již záznam existuje
            const { data: existingClaim, error: checkError } = await supabase
                .from('claimed_streak_milestones')
                .select('id')
                .eq('user_id', currentUser.id)
                .eq('milestone_day', milestoneDay)
                .eq('reward_key', rewardKey) // Přidáno pro větší specificitu, pokud by se odměny měnily
                .maybeSingle();

            if (checkError) {
                console.error(`[DB Update] Error checking existing claim for milestone ${milestoneDay}:`, checkError);
                throw checkError;
            }
            ...
            if (existingClaim) {
                ...
                // Pokud již existuje, a je to ten samý reward_key, nemusíme nic dělat.
                // Pokud byste chtěli povolit vyzvednutí odlišné odměny pro stejný milník (nepravděpodobné), logika by byla složitější.
            } else {
                // Vložíme nový záznam, pouze pokud neexistuje
                const { error: insertError } = await supabase
                    ...
                    .insert({
                        ...
                        reward_name: rewardName // Ukládáme název odměny pro referenci
                    });
            ...
            // Aktualizujeme last_milestone_claimed v profilu, pokud je tento milník větší
            if (currentProfile && milestoneDay > (currentProfile.last_milestone_claimed || 0)) {
                ...
                if (profileUpdateError) {
                    ...
                    // Chyba zde není fatální pro vyzvednutí, ale logujeme ji
                } else {
                    currentProfile.last_milestone_claimed = milestoneDay; // Aktualizace lokálního profilu
    ...
    async function awardPoints(pointsValue, reason = "Nespecifikováno", transactionType = 'points_earned', referenceActivityId = null, suppressActivityLog = false) {
        ...
        setLoadingState('stats', true); // Indicate that stats might change

        const currentPoints = currentProfile.points || 0;
        const newPoints = currentPoints + pointsValue;

        try {
            // 1. Update points in the profiles table
            ...
            currentProfile.points = newPoints; // Update local cache
            ...
            const { error: transactionError } = await supabase
                ...
                .insert({
                    ...
                    amount: pointsValue,
                    description: reason,
                    balance_after_transaction: newPoints, // Store current balance
                    reference_activity_id: referenceActivityId // Optional link to specific activity
                });

            if (transactionError) {
                console.error("[Points] Error logging credit transaction:", transactionError);
                showToast('Varování', 'Kredity připsány, ale záznam transakce selhal.', 'warning');
            } else {
                console.log(`[Points] Credit transaction logged: ${pointsValue} for ${reason}`);
            }
            ...
            if (pointsValue > 0) {
                ...
                showToast('Kredity Utraceny!', `${pointsValue} kreditů za: ${reason}`, 'info', 2500);
            }

            // Update stats cards display
            userStatsData = await fetchUserStats(currentUser.id, currentProfile); // Re-fetch or update existing
            updateStatsCards(userStatsData);

            // Optionally, refresh credit history list if DashboardLists module is used
            if (typeof DashboardLists !== 'undefined' && typeof DashboardLists.loadAndRenderCreditHistory === 'function') {
                await DashboardLists.loadAndRenderCreditHistory(currentUser.id, 5); // Load recent 5
            }

            // 3. Log activity (unless suppressed)
             if (pointsValue > 0 && !suppressActivityLog) { // ZMĚNA: Kontrola suppressActivityLog
                await logActivity(
                    currentUser.id,
                    'points_earned', // More specific type
                    `Získáno ${pointsValue} kreditů`,
                    `Důvod: ${reason}`,
                    { points_change: pointsValue, new_total_points: newPoints, source: transactionType }, // Add source of points
                    null, // link_url
                    referenceActivityId, // reference_id
                    'fa-coins' // icon
                );
            }
    ...
    async function logActivity(userId, type, title, description = null, details = null, link_url = null, reference_id = null, icon = null) {
        if (!supabase || !userId) {
            console.error("Cannot log activity: Supabase client or user ID is missing.");
            return;
        }
        console.log(`[Log Activity] Logging: User ${userId}, Type: ${type}, Title: ${title}`);
        try {
            const { error } = await supabase
                ...
                .insert({
                    ...
                    details: details, // JSONB for structured data
                    link_url: link_url,
                    reference_id: reference_id, // UUID or other ID for related entity
                    icon: icon || activityVisuals[type]?.icon || activityVisuals.default.icon // Use mapped icon
                });
            if (error) {
                console.error("Error logging activity:", error);
            } else {
                console.log("Activity logged successfully.");
                // If DashboardLists module is used and this is the dashboard page, refresh lists
                if (typeof DashboardLists !== 'undefined' && typeof DashboardLists.loadAndRenderRecentActivities === 'function' && window.location.pathname.includes('dashboard.html')) {
                    await DashboardLists.loadAndRenderRecentActivities(userId, 5); // Reload recent 5
                }
            }
        } catch (err) {
    ...
    function updateSidebarProfile(profile) {
        ...
        if (profile) {
            ...
            ui.sidebarAvatar.innerHTML = avatarUrl ? `<img src="<span class="math-inline">\{sanitizeHTML\(avatarUrl\)\}?t\=</span>{Date.now()}" alt="${sanitizeHTML(initials)}">` : sanitizeHTML(initials);
            ...
            const endTime = performance.now();
            console.log(`[UI Update] Sidebar aktualizován. Time: ${(endTime - startTime).toFixed(2)}ms`);
        } else {
    ...
    function updateStatsCards(stats) {
        ...
        if (requiredElements.some(el => !el)) {
            ...
            cards.forEach(card => card?.classList.remove('loading')); // Odstranit třídu loading i při chybě
            return;
        }
        cards.forEach(card => card?.classList.remove('loading')); // Odstranit třídu loading

        if (!stats) {
            console.warn("[UI Update Stats] Chybí data statistik, zobrazuji placeholder.");
            Object.values(statElements).forEach(el => {
                if (el.value) el.textContent = '-'; // Předpokládá, že 'value' je pro hlavní hodnotu
                else if (el.id.includes('Footer')) el.innerHTML = `<i class="fas fa-exclamation-circle"></i> Data nenalezena`;
            });
            return;
        }
    ...
    async function loadDashboardData(user, profile) {
        ...
        hideError();
        setLoadingState('stats', true); setLoadingState('notifications', true); setLoadingState('activities', true); setLoadingState('creditHistory', true);

        try {
            await checkAndUpdateLoginStreak(); // Musí proběhnout před fetchUserStats
            updateSidebarProfile(profile); // Aktualizace sidebaru

            console.log("[MAIN] loadDashboardData: Paralelní načítání statistik, notifikací a seznamů.");
            const [statsResult, notificationsResult, dashboardListsResult] = await Promise.allSettled([
            ...
            if (statsResult.status === 'fulfilled' && statsResult.value) { userStatsData = statsResult.value; updateStatsCards(userStatsData); }
            ...
            if (notificationsResult.status === 'fulfilled' && notificationsResult.value) { const { unreadCount, notifications } = notificationsResult.value; renderNotifications(unreadCount, notifications); }
            else { console.error("❌ Chyba při načítání oznámení:", notificationsResult.reason); renderNotifications(0, []); }

            if (dashboardListsResult.status === 'rejected') { console.error("❌ Chyba při DashboardLists.loadAndRenderAll:", dashboardListsResult.reason); setLoadingState('activities', false); setLoadingState('creditHistory', false); }
            const endTime = performance.now();
            console.log(`[MAIN] loadDashboardData: Data načtena a zobrazena. Time: ${(endTime - startTime).toFixed(2)}ms`);
        } catch (error) { console.error('[MAIN] loadDashboardData: Zachycena hlavní chyba:', error); showError('Nepodařilo se kompletně načíst data nástěnky: ' + error.message); updateStatsCards(profile); renderNotifications(0, []); if (typeof DashboardLists !== 'undefined') { if (typeof DashboardLists.renderActivities === 'function') DashboardLists.renderActivities(null); if (typeof DashboardLists.renderCreditHistory === 'function') DashboardLists.renderCreditHistory(null); }
        } finally { setLoadingState('stats', false); setLoadingState('notifications', false); setLoadingState('activities', false); setLoadingState('creditHistory', false); if (typeof initTooltips === 'function') initTooltips(); console.log("[MAIN] loadDashboardData: Blok finally dokončen."); }
    }
    ...
    function setupEventListeners() {
        ...
        const safeAddListener = (element, eventType, handler, key) => {
            if (element) {
                const handlerKey = `_event_${eventType}_${key}`;
                if (element[handlerKey]) { console.warn(`[SETUP] Listener for <span class="math-inline">\{key\} \(</span>{eventType}) already exists. Skipping.`); return; }
                element.addEventListener(eventType, handler);
                element[handlerKey] = true;
        ...
        document.querySelectorAll('.sidebar-link').forEach((link, index) => { safeAddListener(link, 'click', () => { if (window.innerWidth <= 992) closeMenu(); }, `sidebarLink${index}`); });
        ...
        if (ui.mainContent) {
            ui.mainContent.removeEventListener('scroll', initHeaderScrollDetection);
    ...
    async function initializeApp() {
        ...
        console.log("[INIT Dashboard] initializeApp: Start v26.0.8");
        ...
        try {
            ...
            if (session?.user) {
                ...
                initTooltips(); // Call after content is loaded
                ...
                console.log(`✅ [INIT Dashboard] Page fully loaded and initialized. Total Time: ${(totalEndTime - totalStartTime).toFixed(2)}ms`);
            ...
            if (ui.initialLoader && !ui.initialLoader.classList.contains('hidden')) {
                ui.initialLoader.innerHTML = `<p style="color: var(--accent-pink);">CHYBA (${error.message}). OBNOVTE STRÁNKU.</p>`;
            } else {
                showError(`Chyba inicializace: ${error.message}`, true);
            }
            if (ui.mainContent) ui.mainContent.style.display = 'none'; // Skrýt hlavní obsah při kritické chybě
    ...
    // Zbývající pomocné funkce (isSameDate, isYesterday, getCurrentMonthYearString, showModal, hideModal, initMouseFollower, initScrollAnimations, initHeaderScrollDetection, updateCopyrightYear, initTooltips)
    // ponechány pro úplnost, pokud by byly v budoucnu potřeba nebo volány z jiných částí, které nebyly součástí původního požadavku.
    ...
    function getCurrentMonthYearString() { const now = new Date(); const year = now.getFullYear(); const month = String(now.getMonth() + 1).padStart(2, '0'); return `<span class="math-inline">\{year\}\-</span>{month}`; }
    ...
    function showModal(modalId) {
        ...
        if (modal) {
            ...
            // Zde by se měly volat specifické renderovací funkce pro obsah modálu, POUZE pokud je to nutné při každém otevření
            // Například, pokud se obsah mění dynamicky a není součástí hlavního načítání stránky.
            if (modalId === 'monthly-reward-modal' && typeof renderMonthlyCalendar === 'function') {
                renderMonthlyCalendar();
            } else if (modalId === 'streak-milestones-modal' && typeof renderStreakMilestones === 'function') {
    ...
    function hideModal(modalId) { const modal = document.getElementById(modalId); if (modal) { modal.classList.remove('active'); setTimeout(() => { modal.style.display = 'none'; }, 300); } }
