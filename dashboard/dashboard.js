// dashboard.js
// Verze: 26.0.3 - Восстановлены функции для работы с уведомлениями
(function() {
    'use strict';
    ...
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        if (sectionKey === 'all') {
            ...
            console.log(`[SetLoadingState v26.0.2] Section: all, isLoading: ${isLoadingFlag}`);
            return;
        }

        isLoading[sectionKey] = isLoadingFlag;
        console.log(`[SetLoadingState v26.0.2] Section: ${sectionKey}, isLoading: ${isLoadingFlag}`);
        ...
        if (sectionKey === 'stats') {
            ...
            if (typeof DashboardLists !== 'undefined') {
                if (sectionKey === 'activities' && typeof DashboardLists.setActivitiesLoading === 'function') {
                 ...
                 console.warn(`[SetLoadingState] DashboardLists module not found for section ${sectionKey}.`);
    ...
    // --- START: Notification Logic (Restored) ---
    async function fetchNotifications(userId, limit = 5) {
        ...
        try {
            ...
            // Не показываем toast здесь, чтобы не перегружать пользователя при каждом обновлении
    ...
    async function markAllNotificationsRead() {
        ...
        if (isLoading.notifications) return; // Prevent multiple calls if already loading

        setLoadingState('notifications', true); // Set loading state (will also disable button)
        // ui.markAllReadBtn.disabled = true; // Already handled by setLoadingState
        ...
        try {
            ...
            // Re-fetch and re-render notifications after marking all as read
            const { unreadCount, notifications } = await fetchNotifications(currentUser.id, 5);
            renderNotifications(unreadCount, notifications); // This will also update button disable state
            ...
            // Re-enable button only if there was an error AND there are still unread notifications
            ...
            setLoadingState('notifications', false); // Always reset loading state
        }
    }
    ...
    async function loadDashboardData(user, profile) {
        ...
        if (typeof DashboardLists !== 'undefined') {
            ...
            // Fallback or set global loading states if module isn't ready
        ...
        try {
            ...
            // isLoading для activities и creditHistory теперь управляется из dashboard-lists.js
            // но на всякий случай, если DashboardLists не был загружен:
            if (typeof DashboardLists === 'undefined') {
    ...
    async function initializeApp() {
        console.log("[INIT Dashboard] initializeApp: Start v26.0.3 - Restored notification functions");
        cacheDOMElements();
