// dashboard.js
// Verze: 25.3 - Исправлена синтаксическая ошибка в setLoadingState, улучшена логика отображения.
(function() {
    'use strict';
    ...
    function cacheDOMElements() {
        ...
        ui = {
            ...
            statsCards: document.querySelector('.stat-cards'), 
    ...
    // --- Обновленная функция setLoadingState v25.3 ---
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        if (sectionKey === 'all') {
            Object.keys(isLoading).forEach(key => {
                if (key !== 'all') setLoadingState(key, isLoadingFlag); // Рекурсивный вызов для каждой секции
            });
        ...
        switch (sectionKey) {
            case 'activities':
                ...
                break;
            case 'creditHistory':
                targetListContainer = ui.creditHistoryListContainer;
        ...
        if (isLoadingFlag) {
            ...
            // Видимость contentList, emptyState, errorState управляется функциями render
        }
    }
    ...
    // Обновленная renderActivities v25.3
    function renderActivities(activities) {
        if (!ui.activityList || !ui.activityListContainer || !ui.activityListEmptyState || !ui.activityListErrorState || !ui.activityListLoadingPlaceholder) {
            console.error("[Render Activities] Essential UI elements missing. Rendering cancelled.");
            setLoadingState('activities', false);
            return;
        }
        console.log("[Render Activities v25.3] START, count:", activities?.length);

        ui.activityList.innerHTML = '';
        if (ui.activityListLoadingPlaceholder) ui.activityListLoadingPlaceholder.style.display = 'none';
        ...
        if (activities === null) {
            if (ui.activityListErrorState) ui.activityListErrorState.style.display = 'block';
            console.warn("[Render Activities] Received null data (error state).");
        } else if (!activities || activities.length === 0) {
            if (ui.activityListEmptyState) ui.activityListEmptyState.style.display = 'block';
            console.log("[Render Activities] No activities to display.");
        } else {
            ...
            console.log(`[Render Activities] SUCCESS: Rendered ${activities.length} activities.`);
        }
        if (ui.activityListContainer) ui.activityListContainer.classList.remove('loading');
        setLoadingState('activities', false);
        console.log("[Render Activities v25.3] Rendering finished.");
    }
    ...
    function renderActivitySkeletons(count = 5) {
        ...
        // Display is handled by setLoadingState
    }

    // Обновленная renderCreditHistory v25.3
    function renderCreditHistory(transactions) {
        ...
        console.log("[RenderCreditHistory v25.3] Rendering credit history, transaction count:", transactions?.length);

        ui.creditHistoryList.innerHTML = '';
        if (ui.creditHistoryLoadingPlaceholder) ui.creditHistoryLoadingPlaceholder.style.display = 'none';
        ...
        if (transactions === null) {
            if (ui.creditHistoryErrorState) ui.creditHistoryErrorState.style.display = 'block';
            console.warn("[RenderCreditHistory] Received null data (error state).");
        } else if (!transactions || transactions.length === 0) {
            if (ui.creditHistoryEmptyState) ui.creditHistoryEmptyState.style.display = 'block';
            console.log("[RenderCreditHistory] No transactions to display.");
        } else {
            const fragment = document.createDocumentFragment();
            ...
            console.log(`[RenderCreditHistory] Rendered ${transactions.length} transactions.`);
        }
        if (ui.creditHistoryListContainer) ui.creditHistoryListContainer.classList.remove('loading');
        setLoadingState('creditHistory', false);
        console.log("[RenderCreditHistory v25.3] Rendering finished.");
    }
    ...
    async function loadDashboardData(user, profile) {
        ...
        try {
            ...
            if (activitiesResult.status === 'fulfilled') {
                renderActivities(activitiesResult.value); // Передаем activitiesResult.value напрямую
            } else {
            ...
            if (creditHistoryResult.status === 'fulfilled') {
                renderCreditHistory(creditHistoryResult.value); // Передаем creditHistoryResult.value напрямую
            } else {
    ...
    function hideModal(modalId) {
        ...
        if (modal) {
            ...
            }, 300); // Время должно совпадать с transition-duration в CSS
        }
    }
