// dashboard.js
// Версия: 24.1 - Oprava SyntaxError na řádku ~636
(function() {
    'use strict';
    ...
    async function checkAndUpdateLoginStreak() {
        ...
        if (!lastLogin || !isSameDate(today, lastLogin)) {
            ...
            updateData.streak_days = currentStreak; updateData.last_login = today.toISOString();
        ...
        } else { console.log(`[StreakCheck] Monthly claims for ${currentMonth} already exist.`); }

        if (needsDbUpdate) {
            console.log("[StreakCheck] Updating profile in DB with:", updateData);
            try { const { error: updateError } = await supabase.from('profiles').update(updateData).eq('id', currentUser.id); if (updateError) throw updateError;
    ...
    async function claimMilestoneReward(milestoneDay, buttonElement, rewardType, rewardValue, rewardName) {
        ...
        if (lastClaimed >= milestoneDay) { console.warn(`[ClaimMilestone] Milestone ${milestoneDay} already claimed (last claimed: ${lastClaimed}).`); showToast('Info', 'Milník již byl dosažen.', 'info'); setLoadingState('streakMilestones', false); renderStreakMilestones(); return; }
    ...
    async function initializeApp() {
        console.log("[INIT Dashboard] initializeApp: Start v24.1 - Fix SyntaxError");
        ...
        try {
            ...
            if (session?.user) {
                ...
                initTooltips();
                 ...
                 }); // <<< CORRECTED: Removed extra '}' before this semicolon
