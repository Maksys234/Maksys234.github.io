...
(function() {
    ...
    let allTitles = [];
    let userStatsData = null; // Globální proměnná
    ...
    function setLoadingState(section, isLoadingFlag) { const sections = section === 'all' ? Object.keys(isLoading) : [section]; sections.forEach(sec => { if (!ui || Object.keys(ui).length === 0) { console.warn("UI cache not ready for setLoadingState"); return; } if (isLoading[sec] === isLoadingFlag && section !== 'all') return; isLoading[sec] = isLoadingFlag; console.log(`[setLoadingState] Section: ${sec}, isLoading: ${isLoadingFlag}`); const loaderOverlay = sec === 'monthlyRewards' ? ui.monthlyRewardModal?.querySelector('.loading-overlay') : sec === 'streakMilestones' ? ui.streakMilestonesModal?.querySelector('.loading-overlay') : null; const contentContainer = sec === 'monthlyRewards' ? ui.modalMonthlyCalendarGrid : sec === 'streakMilestones' ? ui.modalMilestonesGrid : sec === 'activities' ? ui.activityList : sec === 'notifications' ? ui.notificationsList : null; const emptyStateContainer = sec === 'monthlyRewards' ? ui.modalMonthlyCalendarEmpty : sec === 'streakMilestones' ? ui.modalMilestonesEmpty : sec === 'activities' ? ui.activityListEmptyState : sec === 'notifications' ? ui.noNotificationsMsg : null; const parentSection = sec === 'monthlyRewards' ? ui.monthlyRewardModal?.querySelector('.modal-body') : sec === 'streakMilestones' ? ui.streakMilestonesModal?.querySelector('.modal-body') : sec === 'activities' ? ui.activityListContainer : sec === 'notifications' ? ui.notificationsDropdown : null; const statCards = sec === 'stats' ? [ui.progressCard, ui.pointsCard, ui.streakCard] : []; if (statCards.length > 0) { statCards.forEach(card => card?.classList.toggle('loading', isLoadingFlag)); } else if (loaderOverlay || parentSection) { parentSection?.classList.toggle('loading', isLoadingFlag); if (loaderOverlay) loaderOverlay.classList.toggle('hidden', !isLoadingFlag); if (isLoadingFlag) { if (contentContainer) contentContainer.innerHTML = ''; if (emptyStateContainer) emptyStateContainer.style.display = 'none'; if (sec === 'activities') renderActivitySkeletons(5); else if (sec === 'monthlyRewards') renderMonthlyCalendarSkeletons(); else if (sec === 'streakMilestones') renderMilestoneSkeletons(); else if (sec === 'notifications') renderNotificationSkeletons(2); } else { if (contentContainer && !contentContainer.hasChildNodes() && emptyStateContainer) { emptyStateContainer.style.display = 'block'; } else if (emptyStateContainer) { emptyStateContainer.style.display = 'none'; } } } else if (sec === 'notifications' && ui.notificationBell) { ui.notificationBell.style.opacity = isLoadingFlag ? 0.5 : 1; if(ui.markAllReadBtn) { const currentUnreadCount = parseInt(ui.notificationCount?.textContent?.replace('+', '') || '0'); ui.markAllReadBtn.disabled = isLoadingFlag || currentUnreadCount === 0; } if (isLoadingFlag && ui.notificationsList && typeof renderNotificationSkeletons === 'function') { renderNotificationSkeletons(2); } } }); }
