// dashboard.js
// Verze: 27.0.4 - Oprava zobrazení kreditů po odměně; Milníky série dle longest_streak_days.
// EDIT LOG: Developer Action -> Fixing credit display after milestone reward and changing milestone unlock logic to use longest_streak_days.
// EDIT LOG: Stage -> Modifying dashboard.js functions updateStatsCards, renderStreakMilestones, and handleStreakMilestoneClaim.
(function() {
    'use strict';
    ...
    let allTitles = [];
    let userStatsData = null; // Used to store the result of fetchUserStats

    const AUTH_TIMEOUT = 30000; // 30 sekund
    const DATA_FETCH_TIMEOUT = 20000; // 20 sekund
    ...
    const MONTHLY_REWARD_DAYS = 31; // Assuming May for now, adjust if dynamic
    ...
    async function updateLastMilestoneClaimedInDB(milestoneDay, rewardKey, rewardName) {
        ...
        try {
            ...
            if (insertError && insertError.code !== '23505') { // '23505' is unique_violation
                ...
                console.warn(`[DB Update Milestones] Milník ${rewardKey} pro den ${milestoneDay} již byl zaznamenán.`);
            } else {
                console.log(`[DB Update Milestones] Milník ${rewardKey} pro den ${milestoneDay} úspěšně zaznamenán.`);
            }
    ...
    function updateStatsCards(stats) {
        ...
        // MODIFIED: Logic for pointsFooter display
        const latestTxData = window.lastCreditTransaction; // Use the global variable updated by fetchAndDisplayLatestCreditChange
        statElements.pointsFooter.classList.remove('positive', 'negative');
        if (latestTxData && latestTxData.description && latestTxData.amount !== undefined) {
            const amount = latestTxData.amount;
            const description = latestTxData.description || `Transakce`;
            ...
            statElements.pointsFooter.innerHTML = `<i class="fas fa-history"></i> <span title="${sanitizeHTML(description)}: ${sign}${amount}">${sanitizeHTML(displayDesc)}: <span style="font-weight:bold;" class="${amountColorClass}">${sign}${amount}</span></span>`;
            if (amount > 0) statElements.pointsFooter.classList.add('positive');
            if (amount < 0) statElements.pointsFooter.classList.add('negative');
        } else {
            const weeklyPoints = stats.points_weekly ?? 0;
             if (weeklyPoints !== 0 && weeklyPoints != null) {
                 ...
                 statElements.pointsFooter.innerHTML = `<i class="fas fa-minus"></i> Žádné nedávné kredity`;
             }
        }
        // END MODIFIED
    ...
    // --- START: Rewards Logic ---
    // ... (handleMonthlyRewardClaim is assumed to be the same)
    // ... (renderMonthlyCalendar is assumed to be the same)
    ...
    function renderStreakMilestones() {
        ...
        sortedMilestoneDays.forEach(dayCount => {
            ...
            // MODIFIED: Check against the claimed_streak_milestones array using reward_key
            ...
            // MODIFIED: Unlock based on longest_streak_days
            const canClaim = currentProfile.longest_streak_days >= dayCount && !isClaimed;
            ...
            if (isClaimed) {
                ...
                // MODIFIED: Message for locked based on longest_streak_days
                buttonOrStatusHTML = `<div class="milestone-status locked"><i class="fas fa-lock"></i> Potřebujete nejdelší sérii ${dayCount} dní</div>`;
            }
    ...
    async function handleStreakMilestoneClaim(milestoneDay, rewardKey) {
        console.log(`[Streak Milestone] Pokus o vyzvednutí: ${milestoneDay} dní, klíč: ${rewardKey}`);
        ...
        // MODIFIED: Check against longest_streak_days for unlocking
        if (currentProfile.longest_streak_days < milestoneDay) {
            showToast('Nedostatečná Nejdelší Série', `Vaše nejdelší série musí být alespoň ${milestoneDay} dní.`, 'warning');
            return;
        }
        ...
        try {
            ...
            for (const reward of milestoneConfig.rewards) {
                ...
                if (reward.type === 'credits' && reward.value) {
                    ...
                    console.log(`[Streak Milestone] Placeholder pro udělení odznaku: ${reward.key} - ${reward.name}`);
            ...
            if (allRewardsAwardedSuccessfully) {
                ...
                if (dbRecordSuccess) {
                    ...
                    // Ensure client-side update reflects the structure expected by renderStreakMilestones
                    ...
                    if (!alreadyLocallyTracked) {
                        currentProfile.claimed_streak_milestones.push({
                            milestone_day: milestoneDay, // Stored as number
                            reward_key: rewardKey,      // Stored as string
                            claimed_at: new Date().toISOString(),
                            reward_name: milestoneConfig.name
                        });
                    }

                    if (milestoneDay > (currentProfile.last_milestone_claimed || 0)) {
            ...
            if (allRewardsAwardedSuccessfully) {
                userStatsData = await fetchUserStats(currentUser.id, currentProfile); // Re-fetch stats
                window.lastCreditTransaction = await fetchAndDisplayLatestCreditChange(currentUser.id); // Store and update latest transaction
                updateStatsCards(userStatsData); // Update cards with new stats and potentially new latest transaction
            }
        }
    }
    // --- END: Rewards Logic ---
    ...
    async function initializeApp() {
        ...
        console.log("[INIT Dashboard] initializeApp: Start v27.0.4"); // Updated version
    ...
    window.DashboardApp = {
        showToast, // Expose showToast if needed by other modules
    };

// EDIT LOG: Developer Action -> Fixing credit display after milestone reward and changing milestone unlock logic to use longest_streak_days.
// EDIT LOG: Stage -> Modified dashboard.js functions updateStatsCards, renderStreakMilestones, and handleStreakMilestoneClaim.
// EDIT LOG: Stage -> Ensured fetchClaimedStreakMilestones is called and data stored in loadDashboardData.
// EDIT LOG: Stage -> Modified `updateStatsCards` to prioritize latest transaction display.
// EDIT LOG: Stage -> Modified `renderStreakMilestones` and `handleStreakMilestoneClaim` to use `longest_streak_days` for unlocking.
})();