...
(function() {
    ...
    // *** FIXED showError function ***
    function showError(message, isGlobal = false) {
         console.error("Došlo k chybě:", message);
         if (isGlobal && ui.globalError) {
             ui.globalError.innerHTML = `<div class="error-message"><i class="fas fa-exclamation-triangle"></i><div>${sanitizeHTML(message)}</div><button class="retry-button btn" id="global-retry-btn">Obnovit Stránku</button></div>`; // Changed button text
             ui.globalError.style.display = 'block';
             const retryBtn = document.getElementById('global-retry-btn');
             if (retryBtn) {
                 retryBtn.addEventListener('click', () => {
                     location.reload(); // Simple page reload on global error retry
                 });
             }
         } else {
    ...
    async function fetchUserProfile(userId) {
        ...
        try {
            ...
            return null; // Error handled in initializeApp
        }
    }
    ...
    async function createDefaultProfile(userId, userEmail) {
        ...
        try {
            const defaultData = {
                 ...
                 consecutive_login_days: 0, last_daily_login: null, // Initialize new fields
    ...
    async function fetchTitles() {
        ...
        try {
            ...
            return []; // Return empty on error, don't show toast here
        } finally {
    ...
    async function loadDashboardData(user, profile) {
         ...
         try {
             ...
             if (results[0].status === 'fulfilled') {
                  updateStatsCards(results[0].value || profile);
    ...
    async function initializeApp() {
        ...
        setupUIEventListeners(); // Setup basic listeners first
        ...
        try {
            ...
            if (session?.user) {
                ...
                updateSidebarProfile(currentProfile); // Update sidebar including title
            ...
            if (ui.mainContent) ui.mainContent.style.display = 'none'; // Keep content hidden on critical error
        }
    }
