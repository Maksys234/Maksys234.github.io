// dashboard.js
// Verze: 27.0.0 - Přidáno načítání posledního kreditu, vylepšení struktury a chyb.
// Refaktoring pro lepší čitelnost, správu stavu a robustnější zpracování dat.
...
(function() {
    ...
    const AUTH_TIMEOUT = 30000; // ms
    const DATA_FETCH_TIMEOUT = 25000; // ms
    const SIDEBAR_STATE_KEY = 'sidebarCollapsedState';
    const NOTIFICATION_FETCH_LIMIT = 5;
    const ACTIVITY_FETCH_LIMIT = 5; // Pro recent activities a credit history
    const MONTHLY_REWARD_DAYS = 31; // Max days in a month for calendar

    // Конфигурация наград за серию (Streak Milestones)
    ...
    // Конфигурация майских наград (Měsíční odměny)
    const mayRewards = { // Ключи - дни месяца (1-31)
    ...
    // Сопоставление типов активностей с иконками и классами
    const activityVisuals = {
        ...
        reward_streak_milestone: { name: 'Odměna za Sérii', icon: 'fa-star-half-alt', class: 'reward_streak' }, // Specific for streak rewards
        reward_monthly_calendar: { name: 'Měsíční Bonus', icon: 'fa-calendar-check', class: 'reward_monthly' }, // Specific for monthly rewards
    ...
    let allTitles = []; // Для хранения названий титулов
    let userStatsData = null; // Для хранения общей статистики
    let isUserSessionInitialized = false; // Флаг инициализации сессии
    ...
    let isLoading = {
        ...
        session: false, // Для отслеживания загрузки сессии
        all: false // Общий флаг для одновременной загрузки
    };
    // --- END: State Variables ---

    // --- START: UI Elements Cache ---
    const ui = {}; // Будет заполнено в cacheDOMElements

    function cacheDOMElements() {
        const startTime = performance.now();
        console.log("[CACHE DOM dashboard.js] Caching elements...");
        const elementDefinitions = [
            { key: 'initialLoader', id: 'initial-loader', critical: true },
            { key: 'sidebar', id: 'sidebar', critical: true },
            { key: 'mainContent', id: 'main-content', critical: true }, // Это основной <main>
            { key: 'mainContentAreaPlaceholder', id: 'main-content-area-placeholder', critical: true }, // Placeholder для контента
            ...
            { key: 'statsCardsContainer', id: 'stats-cards-container', critical: true }, // Родитель для карточек статистики
            ...
            { key: 'latestCreditChange', id: 'latest-credit-change', critical: false }, // Новый элемент для кредитов
            // ... (остальные элементы как в вашем dashboard.js)
            ...
            { key: 'markAllReadBtn', id: 'mark-all-read', critical: false }, // Предполагается, что кнопка имеет id "mark-all-read"
            ...
            { key: 'statsCardsSkeletonContainer', id: 'stats-cards-skeleton-container', critical: false }, // Parent for stats skeletons
            { key: 'shortcutGridReal', id: 'shortcut-grid-real', critical: false },
            { key: 'shortcutGridSkeletonContainer', id: 'shortcut-grid-skeleton-container', critical: false }, // Parent for shortcut skeletons
        ...
        elementDefinitions.forEach(def => {
            ...
            if (element) {
                ...
                ui[def.key] = null; // Explicitly set to null if not found
        ...
        // Specific lists inside containers, if they exist (used by dashboard-lists.js)
        ...
        if (notFoundCritical.length > 0) {
            console.error(`[CACHE DOM] KRITICKÉ elementy nenalezeny (${notFoundCritical.length}):`, notFoundCritical.join(', '));
            throw new Error(`Chyba načítání stránky: Kritické komponenty chybí (${notFoundCritical.join(', ')}).`);
        } else { console.log("[CACHE DOM] Všechny kritické elementy nalezeny."); }
    ...
    async function fetchLatestCreditChange(userId) {
        ...
        try {
            const { data, error } = await supabase
                ...
                .select('amount, description') // Selecting amount and description
                .eq('user_id', userId)
    ...
    async function awardPoints(pointsValue, reason = "Nespecifikováno", transactionType = 'points_earned', referenceActivityId = null, suppressActivityLog = false) {
        ...
        setLoadingState('stats', true); // Keep this for immediate UI feedback
        ...
        try {
            ...
            else { console.log(`[Points] Credit transaction logged: ${pointsValue} for ${reason}`); }
    ...
    // (updateSidebarProfile, updateStatsCards, renderMonthlyCalendar, renderStreakMilestones)
    // ... (Все функции остаются без изменений, как в dashboard.js) ...
    ...
    function renderMonthlyCalendar() { /* ... (без изменений, как в dashboard.js) ... */ if (!ui.modalMonthlyCalendarGrid || !ui.modalCurrentMonthYearSpan || !ui.modalMonthlyCalendarEmpty) { console.error("Monthly calendar MODAL UI elements missing. Cannot render."); setLoadingState('monthlyRewards', false); return; } console.log("[RenderMonthly] Rendering calendar..."); setLoadingState('monthlyRewards', true); const gridContainer = ui.modalMonthlyCalendarGrid; const modalTitleSpan = ui.modalCurrentMonthYearSpan; const emptyState = ui.modalMonthlyCalendarEmpty; const now = new Date(); const year = now.getFullYear(); const month = now.getMonth(); const today = now.getDate(); const daysInMonth = new Date(year, month + 1, 0).getDate(); const monthString = getCurrentMonthYearString(); const monthName = now.toLocaleString('cs-CZ', { month: 'long', year: 'numeric' }); console.log(`[RenderMonthly] currentProfile PŘED renderováním:`, JSON.parse(JSON.stringify(currentProfile))); const claimedDaysThisMonth = currentProfile?.monthly_claims?.[monthString] || []; console.log(`[RenderMonthly] Claimed days for ${monthString} (z currentProfile):`, JSON.parse(JSON.stringify(claimedDaysThisMonth))); modalTitleSpan.textContent = monthName; gridContainer.innerHTML = ''; emptyState.style.display = 'none'; gridContainer.style.display = 'grid'; const fragment = document.createDocumentFragment(); let daysRendered = 0; for (let day = 1; day <= daysInMonth; day++) { const dayElement = document.createElement('div'); dayElement.classList.add('calendar-day'); dayElement.dataset.day = day; const isClaimed = claimedDaysThisMonth.includes(day); const isClaimable = (day <= today && !isClaimed); const isUpcoming = day > today; let rewardIcon = 'fa-gift'; let rewardText = `Den ${day}`; let specificReward = null; let rewardType = 'default'; let rewardKey = `day_${day}`; let rewardValue = 1; if (month === 4 && mayRewards[day]) { specificReward = mayRewards[day]; rewardIcon = specificReward.icon || 'fa-gift'; rewardText = specificReward.name; rewardType = specificReward.type; rewardKey = specificReward.key || `may_reward_${day}`; rewardValue = specificReward.value !== undefined ? specificReward.value : 1; } else { if (day % 7 === 0) { rewardIcon = 'fa-coins'; rewardText = '10 Kreditů'; rewardType = 'credits'; rewardValue = 10; rewardKey = `week_bonus_day_${day}`; } else if (day % 5 === 0) { rewardIcon = 'fa-star'; rewardText = '5 EXP'; rewardType = 'xp'; rewardValue = 5; rewardKey = `exp_bonus_day_${day}`; } } dayElement.dataset.rewardType = rewardType; dayElement.dataset.rewardKey = rewardKey; dayElement.dataset.rewardValue = rewardValue; dayElement.innerHTML = `<span class="day-number">${day}</span><div class="reward-icon"><i class="fas ${rewardIcon}"></i></div><span class="reward-text">${rewardText}</span><span class="reward-status"></span><button class="claim-button btn btn-sm" style="display: none;"><i class="fas fa-check"></i> Vyzvednout</button>`; const statusSpan = dayElement.querySelector('.reward-status'); const claimButton = dayElement.querySelector('.claim-button'); if (isClaimed) { dayElement.classList.add('claimed'); if (statusSpan) statusSpan.textContent = 'Získáno'; } else if (isClaimable) { dayElement.classList.add('available'); if (statusSpan) statusSpan.textContent = 'Dostupné'; if (claimButton) { claimButton.style.display = 'block'; claimButton.addEventListener('click', (e) => { e.stopPropagation(); claimMonthlyReward(day, claimButton, rewardType, rewardValue, rewardText); }); } } else if (isUpcoming) { dayElement.classList.add('upcoming'); if (statusSpan) statusSpan.textContent = 'Připravuje se'; } else { dayElement.classList.add('missed'); if (statusSpan) statusSpan.textContent = 'Zmeškáno'; } if (day === today) { dayElement.classList.add('today'); } fragment.appendChild(dayElement); daysRendered++; } if (daysRendered > 0) { gridContainer.appendChild(fragment); } else { emptyState.style.display = 'block'; gridContainer.style.display = 'none'; } ui.monthlyRewardModal?.querySelector('.modal-body')?.classList.remove('loading'); console.log("[RenderMonthly] Modal calendar rendered."); setLoadingState('monthlyRewards', false); initTooltips(); }
    function renderStreakMilestones() { /* ... (без изменений, как в dashboard.js) ... */ if (!ui.modalMilestonesGrid || !ui.modalMilestonesEmpty || !ui.modalCurrentStreakValue || !ui.modalLongestStreakValue) { console.error("Streak milestones MODAL UI elements missing."); setLoadingState('streakMilestones', false); return; } console.log("[RenderMilestones] Rendering streak milestones..."); setLoadingState('streakMilestones', true); const gridContainer = ui.modalMilestonesGrid; const emptyState = ui.modalMilestonesEmpty; const currentStreakSpan = ui.modalCurrentStreakValue; const longestStreakSpan = ui.modalLongestStreakValue; const currentStreak = currentProfile?.streak_days || 0; const longestStreakOverall = currentProfile?.longest_streak_days || currentStreak; currentStreakSpan.textContent = currentStreak; longestStreakSpan.textContent = longestStreakOverall; gridContainer.innerHTML = ''; emptyState.style.display = 'none'; gridContainer.style.display = 'grid'; console.log(`[RenderMilestones] DEBUG - currentProfile PŘED DB dotazem na claimed_streak_milestones:`, JSON.parse(JSON.stringify(currentProfile))); supabase.from('claimed_streak_milestones').select('milestone_day, reward_key').eq('user_id', currentUser.id).then(({ data: claimedData, error: fetchClaimedError }) => { if (fetchClaimedError) { console.error("Error fetching claimed streak milestones:", fetchClaimedError); showToast('Chyba', 'Nepodařilo se načíst vyzvednuté milníky série.', 'error'); setLoadingState('streakMilestones', false); return; } const claimedMilestoneEntries = new Set((claimedData || []).map(c => `${c.milestone_day}_${c.reward_key}`)); console.log("[RenderMilestones] Claimed milestones loaded from DB:", claimedMilestoneEntries); const fragment = document.createDocumentFragment(); let milestonesToShow = 0; sortedMilestoneDays.forEach(milestoneDay => { const config = STREAK_MILESTONES_CONFIG[milestoneDay]; if (!config) return; const milestoneElement = document.createElement('div'); milestoneElement.classList.add('milestone-card'); milestoneElement.dataset.milestone = milestoneDay; const uniqueClaimKey = `${milestoneDay}_${config.reward_key}`; const isClaimed = claimedMilestoneEntries.has(uniqueClaimKey); const isVisuallyAchieved = longestStreakOverall >= milestoneDay; const canActuallyClaim = currentStreak >= milestoneDay && !isClaimed; let statusHTML = ''; let buttonHTML = ''; if (isClaimed) { milestoneElement.classList.add('claimed'); statusHTML = `<span class="reward-status claimed">Získáno</span>`; } else if (isVisuallyAchieved) { if (canActuallyClaim) { milestoneElement.classList.add('available'); statusHTML = `<span class="reward-status available">Dostupné</span>`; buttonHTML = `<button class="claim-button btn btn-sm btn-success"><i class="fas fa-check"></i> Vyzvednout</button>`; } else { milestoneElement.classList.add('achieved-not-claimable'); statusHTML = `<span class="reward-status achieved">Dosaženo (Rekord: ${longestStreakOverall})</span>`; } } else { milestoneElement.classList.add('locked'); const daysNeeded = milestoneDay - longestStreakOverall; statusHTML = `<span class="reward-status locked">Ještě ${daysNeeded} ${daysNeeded === 1 ? 'den' : (daysNeeded < 5 ? 'dny' : 'dní')} k rekordu</span>`; } milestoneElement.innerHTML = `<span class="day-number">Série ${milestoneDay}</span><div class="reward-icon"><i class="fas ${config.icon || 'fa-award'}"></i></div><span class="reward-text" title="${sanitizeHTML(config.description)}">${sanitizeHTML(config.name)}</span><div class="milestone-desc">${sanitizeHTML(config.description || '')}</div>${statusHTML}${buttonHTML}`; const claimButton = milestoneElement.querySelector('.claim-button'); if (claimButton) { claimButton.addEventListener('click', (e) => { e.stopPropagation(); claimMilestoneReward(milestoneDay, claimButton, config.reward_type, config.reward_value, config.name, config.reward_key); }); } fragment.appendChild(milestoneElement); milestonesToShow++; }); if (milestonesToShow > 0) { gridContainer.appendChild(fragment); } else { emptyState.innerHTML = '<i class="fas fa-road"></i><p>Pro tuto sérii nejsou definovány žádné milníky.</p>'; emptyState.style.display = 'block'; gridContainer.style.display = 'none'; } setLoadingState('streakMilestones', false); initTooltips(); }).catch(error => { console.error("Error in renderStreakMilestones fetching claimed data:", error); showToast('Chyba', 'Nepodařilo se načíst data o milnících.', 'error'); emptyState.innerHTML = '<i class="fas fa-exclamation-triangle"></i><p>Chyba načítání milníků.</p>'; emptyState.style.display = 'block'; gridContainer.style.display = 'none'; setLoadingState('streakMilestones', false); }); console.log("[RenderMilestones] Milestones rendering initiated in modal."); }

    // --- NEW: Function to update only the credit change display ---
    async function updateLatestCreditUIDisplay(userId) {
        if (!ui.latestCreditChange) {
            console.warn("[UpdateCreditUI] Element for latest credit change not found.");
            return;
        }
        try {
            const { text, class: changeClass, description } = await fetchLatestCreditChange(userId);
            ui.latestCreditChange.textContent = text;
            ui.latestCreditChange.className = `credit-change-value ${changeClass}`; // Reset classes and add new one
            ui.latestCreditChange.title = description || ''; // Add description as tooltip
            console.log(`[UpdateCreditUI] Display updated to: ${text}, Class: ${changeClass}`);
        } catch (error) {
            console.error("[UpdateCreditUI] Failed to update latest credit change display:", error);
            ui.latestCreditChange.textContent = 'ERR';
            ui.latestCreditChange.className = 'credit-change-value error';
            ui.latestCreditChange.title = 'Chyba načítání poslední transakce';
        }
    }
    // --- END: UI Update Functions ---


    // --- START: Modals Interaction ---
    function showModal(modalId) { /* ... (без изменений, как в dashboard.js) ... */ const modal = document.getElementById(modalId); if (modal) { console.log(`[Modal] Opening modal: ${modalId}`); modal.style.display = 'flex'; if (modalId === 'monthly-reward-modal') { renderMonthlyCalendar(); } else if (modalId === 'streak-milestones-modal') { renderStreakMilestones(); } requestAnimationFrame(() => { modal.classList.add('active'); }); } else { console.error(`Modal element not found: #${modalId}`); } }
    function hideModal(modalId) { /* ... (без изменений, как в dashboard.js) ... */ const modal = document.getElementById(modalId); if (modal) { console.log(`[Modal] Closing modal: ${modalId}`); modal.classList.remove('active'); setTimeout(() => { modal.style.display = 'none'; }, 300); } }
    // --- END: Modals Interaction ---

    // --- START: Skeleton Rendering (Minor adjustments if needed) ---
    // (renderSkeleton, toggleSkeletonUI - в основном остаются как в dashboard.js,
    // но toggleSkeletonUI теперь управляется глобально)
    function toggleSkeletonUI(sectionKey, showSkeleton) { /* ... (без изменений, как в dashboard.js) ... */ console.log(`[Skeleton Toggle] Section: ${sectionKey}, Show Skeleton: ${showSkeleton}`); let skeletonContainer, realContainer; switch (sectionKey) { case 'welcomeBanner': skeletonContainer = ui.welcomeBannerSkeleton; realContainer = ui.welcomeBannerReal; break; case 'stats': skeletonContainer = ui.statsCardsSkeletonContainer; realContainer = ui.statsCardsContainer; break; case 'shortcuts': skeletonContainer = ui.shortcutGridSkeletonContainer; realContainer = ui.shortcutGridReal; break; case 'activities': skeletonContainer = ui.activityListSkeletonContainer; realContainer = ui.activityListContainer; break; case 'creditHistory': skeletonContainer = ui.creditHistorySkeletonContainer; realContainer = ui.creditHistoryListContainer; break; default: console.warn(`[Skeleton Toggle] Unknown sectionKey: ${sectionKey}`); return; } const sectionParent = skeletonContainer?.closest('section') || realContainer?.closest('section'); if (skeletonContainer) { skeletonContainer.style.display = showSkeleton ? (skeletonContainer.classList.contains('stat-cards') || skeletonContainer.classList.contains('shortcut-grid') ? 'grid' : 'block') : 'none'; if(showSkeleton && sectionParent) sectionParent.classList.add('skeleton-active'); } if (realContainer) { realContainer.style.display = showSkeleton ? 'none' : (realContainer.classList.contains('stat-cards') || realContainer.classList.contains('shortcut-grid') ? 'grid' : 'block'); if(!showSkeleton && sectionParent) sectionParent.classList.remove('skeleton-active'); } }
    ...
    async function loadDashboardData(user, profile) {
        ...
        // Lists loading will be managed by DashboardLists module

        try {
            await checkAndUpdateLoginStreak(); // Check streak first
            updateSidebarProfile(currentProfile); // Update sidebar with potentially new streak/claims info

            console.log("[MAIN] loadDashboardData: Paralelní načítání statistik, notifikací a seznamů.");
            const [statsResult, notificationsResult, latestCreditResult, dashboardListsResult] = await Promise.allSettled([
                fetchUserStats(user.id, currentProfile),
                fetchNotifications(user.id, NOTIFICATION_FETCH_LIMIT),
                fetchLatestCreditChange(user.id), // Fetch latest credit change
                (typeof DashboardLists !== 'undefined' && typeof DashboardLists.loadAndRenderAll === 'function')
                    ? DashboardLists.loadAndRenderAll(user.id, ACTIVITY_FETCH_LIMIT)
                    : Promise.resolve({ status: 'fulfilled', value: console.warn("DashboardLists.loadAndRenderAll not found or module not ready for parallel load.") })
            ]);
            console.log("[MAIN] loadDashboardData: Načítání statistik, notifikací a seznamů dokončeno.");
            ...
            if (statsResult.status === 'fulfilled' && statsResult.value) {
                ...
                updateStatsCards(currentProfile); // Fallback to profile data if stats fetch fails
            }
            ...
            // Process and render latest credit change
            if (latestCreditResult.status === 'fulfilled' && latestCreditResult.value) {
                const { text, class: changeClass, description } = latestCreditResult.value;
                if (ui.latestCreditChange) {
                    ui.latestCreditChange.textContent = text;
                    ui.latestCreditChange.className = `credit-change-value ${changeClass}`;
                    ui.latestCreditChange.title = description || '';
                }
            } else {
                console.error("❌ Chyba při načítání poslední změny kreditů:", latestCreditResult.reason);
                if (ui.latestCreditChange) {
                    ui.latestCreditChange.textContent = 'ERR';
                    ui.latestCreditChange.className = 'credit-change-value error';
                    ui.latestCreditChange.title = 'Chyba načítání';
                }
            }
            ...
            if (dashboardListsResult.status === 'rejected') {
                console.error("❌ Chyba při DashboardLists.loadAndRenderAll:", dashboardListsResult.reason);
                 // DashboardLists handles its own internal loading states, so no global setLoadingState here
            }
            ...
            updateStatsCards(currentProfile); // Fallback to profile
            renderNotifications(0, []);
            if (typeof DashboardLists !== 'undefined') { // Ensure lists module fallback
            ...
            setLoadingState('stats', false);
            setLoadingState('notifications', false);
            // Loading states for activities and creditHistory are managed by DashboardLists module
            if (typeof initTooltips === 'function') initTooltips();
            console.log("[MAIN] loadDashboardData: Blok finally dokončen.");
        }
    }
    // --- END: Main Data Loading Orchestration ---

    // --- START: Event Listeners Setup ---
    function setupEventListeners() { /* ... (без изменений, как в dashboard.js) ... */ const startTime = performance.now(); console.log("[SETUP] setupUIEventListeners: Start"); if (!ui || Object.keys(ui).length === 0) { console.error("[SETUP] UI cache is empty! Cannot setup listeners."); return; } const listenersAdded = new Set(); const safeAddListener = (element, eventType, handler, key) => { if (element) { element.removeEventListener(eventType, handler); element.addEventListener(eventType, handler); listenersAdded.add(key); } else { console.warn(`[SETUP] Element not found for listener: ${key}`); } }; safeAddListener(ui.mainMobileMenuToggle, 'click', openMenu, 'mainMobileMenuToggle'); safeAddListener(ui.sidebarCloseToggle, 'click', closeMenu, 'sidebarCloseToggle'); safeAddListener(ui.sidebarOverlay, 'click', closeMenu, 'sidebarOverlay'); safeAddListener(ui.sidebarToggleBtn, 'click', toggleSidebar, 'sidebarToggleBtn'); document.querySelectorAll('.sidebar-link').forEach(link => { link.addEventListener('click', () => { if (window.innerWidth <= 992) closeMenu(); }); }); safeAddListener(ui.startPracticeBtn, 'click', () => { window.location.href = '/dashboard/procvicovani/main.html'; }, 'startPracticeBtn'); safeAddListener(ui.openMonthlyModalBtn, 'click', () => showModal('monthly-reward-modal'), 'openMonthlyModalBtn'); safeAddListener(ui.openStreakModalBtn, 'click', () => showModal('streak-milestones-modal'), 'openStreakModalBtn'); safeAddListener(ui.refreshDataBtn, 'click', async () => { if (!currentUser || !currentProfile) { showToast("Chyba", "Pro obnovení je nutné se přihlásit.", "error"); return; } if (Object.values(isLoading).some(state => state)) { showToast("PROBÍHÁ SYNCHRONIZACE", "Data se již načítají.", "info"); return; } const icon = ui.refreshDataBtn.querySelector('i'); const text = ui.refreshDataBtn.querySelector('.refresh-text'); if (icon) icon.classList.add('fa-spin'); if (text) text.textContent = 'RELOADING...'; ui.refreshDataBtn.disabled = true; await loadDashboardData(currentUser, currentProfile); if (icon) icon.classList.remove('fa-spin'); if (text) text.textContent = 'RELOAD'; ui.refreshDataBtn.disabled = false; }, 'refreshDataBtn'); safeAddListener(ui.notificationBell, 'click', (event) => { event.stopPropagation(); ui.notificationsDropdown?.classList.toggle('active'); }, 'notificationBell'); safeAddListener(ui.markAllReadBtn, 'click', markAllNotificationsRead, 'markAllReadBtn'); safeAddListener(ui.notificationsList, 'click', async (event) => { const item = event.target.closest('.notification-item'); if (item) { const notificationId = item.dataset.id; const link = item.dataset.link; const isRead = item.classList.contains('is-read'); if (!isRead && notificationId) { const success = await markNotificationRead(notificationId); if (success) { item.classList.add('is-read'); item.querySelector('.unread-dot')?.remove(); const currentCountText = ui.notificationCount?.textContent?.replace('+', '') || '0'; const currentCount = parseInt(currentCountText) || 0; const newCount = Math.max(0, currentCount - 1); if(ui.notificationCount) { ui.notificationCount.textContent = newCount > 9 ? '9+' : (newCount > 0 ? String(newCount) : ''); ui.notificationCount.classList.toggle('visible', newCount > 0); } if(ui.markAllReadBtn) ui.markAllReadBtn.disabled = newCount === 0; } } if (link) window.location.href = link; } }, 'notificationsList'); document.addEventListener('click', (event) => { if (ui.notificationsDropdown?.classList.contains('active') && !ui.notificationsDropdown.contains(event.target) && !ui.notificationBell?.contains(event.target)) { ui.notificationsDropdown?.classList.remove('active'); } }); safeAddListener(ui.closeMonthlyModalBtn, 'click', () => hideModal('monthly-reward-modal'), 'closeMonthlyModalBtn'); safeAddListener(ui.monthlyRewardModal, 'click', (event) => { if (event.target === ui.monthlyRewardModal) { hideModal('monthly-reward-modal'); } }, 'monthlyRewardModal'); safeAddListener(ui.closeStreakModalBtn, 'click', () => hideModal('streak-milestones-modal'), 'closeStreakModalBtn'); safeAddListener(ui.streakMilestonesModal, 'click', (event) => { if (event.target === ui.streakMilestonesModal) { hideModal('streak-milestones-modal'); } }, 'streakMilestonesModal'); window.addEventListener('online', updateOnlineStatus); window.addEventListener('offline', updateOnlineStatus); if (ui.mainContent) ui.mainContent.addEventListener('scroll', initHeaderScrollDetection, { passive: true }); const endTime = performance.now(); console.log(`[SETUP] Event listeners set up. Added: ${[...listenersAdded].length}. Time: ${(endTime - startTime).toFixed(2)}ms`); }
    // --- END: Event Listeners Setup ---

    // --- START: App Initialization ---
    async function initializeApp() { /* ... (общая логика без изменений, как в dashboard.js, но теперь включает updateLatestCreditUIDisplay) ... */ const totalStartTime = performance.now(); console.log("[INIT Dashboard] initializeApp: Start v26.0.13"); let stepStartTime = performance.now(); const initialLoaderElement = document.getElementById('initial-loader'); const mainContentElement = document.getElementById('main-content'); const sidebarElement = document.getElementById('sidebar'); const headerElement = document.querySelector('.dashboard-header'); try { if (sidebarElement) sidebarElement.style.display = 'flex'; if (headerElement) headerElement.style.display = 'flex'; if (mainContentElement) mainContentElement.style.display = 'block'; cacheDOMElements(); console.log(`[INIT Dashboard] cacheDOMElements Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); if (ui.mainContentAreaPlaceholder) { ui.mainContentAreaPlaceholder.innerHTML = '<div class="loading-spinner" style="margin:auto;"></div><p>Načítání palubní desky...</p>'; ui.mainContentAreaPlaceholder.style.display = 'flex'; toggleSkeletonUI('welcomeBanner', true); toggleSkeletonUI('stats', true); toggleSkeletonUI('shortcuts', true); if (ui.activityListContainerWrapper) ui.activityListContainerWrapper.classList.add('loading-section'); if (ui.creditHistoryContainerWrapper) ui.creditHistoryContainerWrapper.classList.add('loading-section'); } else { console.warn("[INIT Dashboard] mainContentAreaPlaceholder NOT FOUND in DOM after cache. Layout might be affected."); } if (ui.welcomeBannerReal) ui.welcomeBannerReal.style.display = 'none'; if (ui.statsCardsContainer) ui.statsCardsContainer.style.display = 'none'; if (ui.shortcutGridReal) ui.shortcutGridReal.style.display = 'none'; if (ui.activityListContainer) ui.activityListContainer.style.display = 'none'; if (ui.creditHistoryListContainer) ui.creditHistoryListContainer.style.display = 'none'; if (initialLoaderElement) { initialLoaderElement.classList.add('hidden'); setTimeout(() => { if (initialLoaderElement) initialLoaderElement.style.display = 'none'; }, 50); } console.log(`[INIT Dashboard] Basic UI visible, initialLoader hidden. Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); const waitForSupabase = new Promise((resolve, reject) => { const maxAttempts = 10; let attempts = 0; const intervalId = setInterval(() => { attempts++; if (typeof window.supabase !== 'undefined' && typeof window.supabase.createClient === 'function') { console.log(`[INIT Dashboard] Supabase library found after ${attempts} attempts.`); clearInterval(intervalId); resolve(); } else if (attempts >= maxAttempts) { console.error("[INIT Dashboard] Supabase library not found after waiting. Aborting."); clearInterval(intervalId); reject(new Error("Knihovna Supabase nebyla nalezena včas.")); } else { console.log(`[INIT Dashboard] Waiting for Supabase library... (Attempt ${attempts}/${maxAttempts})`); } }, 200); }); await waitForSupabase; console.log(`[INIT Dashboard] waitForSupabase Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); if (!initializeSupabase()) { console.error("[INIT Dashboard] Supabase init function failed. Aborting."); return; } console.log(`[INIT Dashboard] initializeSupabase Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); applyInitialSidebarState(); console.log(`[INIT Dashboard] applyInitialSidebarState Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); setupEventListeners(); console.log(`[INIT Dashboard] setupEventListeners Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); setLoadingState('session', true); console.log("[INIT Dashboard] Checking auth session (async)..."); try { const { data: { session }, error: sessionError } = await withTimeout(supabase.auth.getSession(), AUTH_TIMEOUT, new Error('Ověření sezení vypršelo.')); console.log(`[INIT Dashboard] getSession Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); setLoadingState('session', false); if (sessionError) { if (sessionError.message.toLowerCase().includes('networkerror')) { throw new Error('Chyba sítě při ověřování sezení. Zkontrolujte své připojení k internetu.'); } throw new Error(`Nepodařilo se ověřit sezení: ${sessionError.message}`); } if (session?.user) { currentUser = session.user; console.log(`[INIT Dashboard] User authenticated (ID: ${currentUser.id}). Loading profile and titles...`); if (ui.mainContentAreaPlaceholder) ui.mainContentAreaPlaceholder.style.display = 'none'; const [profileResult, titlesResult] = await Promise.allSettled([ fetchUserProfile(currentUser.id), fetchTitles() ]); console.log(`[INIT Dashboard] fetchProfile & fetchTitles (parallel) Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); stepStartTime = performance.now(); if (profileResult.status === 'fulfilled' && profileResult.value) { currentProfile = profileResult.value; console.log("[INIT Dashboard] Profile loaded:", currentProfile); } else { const profileErrorReason = profileResult.reason || 'Neznámá chyba při načítání profilu.'; console.warn(`[INIT Dashboard] Profile not found or fetch failed (${profileErrorReason}), attempting to create default...`); currentProfile = await createDefaultProfile(currentUser.id, currentUser.email); if (!currentProfile) throw new Error(`Nepodařilo se vytvořit/načíst profil uživatele. Důvod: ${profileErrorReason}`); console.log("[INIT Dashboard] Default profile created/retrieved."); } if (titlesResult.status === 'fulfilled') { allTitles = titlesResult.value || []; console.log("[INIT Dashboard] Titles loaded:", allTitles.length); } else { console.warn("[INIT Dashboard] Failed to load titles:", titlesResult.reason); allTitles = []; showToast('Varování', 'Nepodařilo se načíst seznam titulů.', 'warning'); } updateSidebarProfile(currentProfile); updateCopyrightYear(); updateOnlineStatus(); if (typeof DashboardLists !== 'undefined' && typeof DashboardLists.initialize === 'function') { DashboardLists.initialize({ supabaseClient: supabase, currentUser: currentUser, activityVisuals: activityVisuals, formatRelativeTime: formatRelativeTime, sanitizeHTML: sanitizeHTML, }); } else { console.error("Modul DashboardLists není definován nebo nemá funkci initialize!"); setLoadingState('activities', false); setLoadingState('creditHistory', false); } toggleSkeletonUI('welcomeBanner', false); toggleSkeletonUI('stats', false); toggleSkeletonUI('shortcuts', false); await loadDashboardData(currentUser, currentProfile); console.log(`[INIT Dashboard] loadDashboardData Time: ${(performance.now() - stepStartTime).toFixed(2)}ms`); requestAnimationFrame(() => { if (ui.mainContent) ui.mainContent.classList.add('loaded'); initScrollAnimations(); }); initMouseFollower(); initHeaderScrollDetection(); initTooltips(); const readyEvent = new CustomEvent('dashboardReady', { detail: { user: currentUser, profile: currentProfile, client: supabase, titles: allTitles } }); document.dispatchEvent(readyEvent); console.log("[INIT Dashboard] Dispatching 'dashboardReady' event."); } else { console.log('[INIT Dashboard] V sezení není uživatel, přesměrování.'); if (ui.mainContentAreaPlaceholder) ui.mainContentAreaPlaceholder.style.display = 'none'; showError("Nejste přihlášeni. Přesměrovávám na přihlašovací stránku...", false, ui.mainContentAreaPlaceholder); setTimeout(() => { window.location.href = '/auth/index.html'; }, 3000); } } catch (authError) { console.error("❌ [INIT Dashboard] Auth/Session Check Error:", authError); setLoadingState('session', false); let userFriendlyMessage = `Chyba ověření: ${authError.message}`; if (authError.message.toLowerCase().includes('ověření sezení vypršelo')) { userFriendlyMessage = "Ověření sezení vypršelo. Zkuste prosím obnovit stránku, nebo zkontrolujte své internetové připojení."; } else if (authError.message.toLowerCase().includes('networkerror')) { userFriendlyMessage = "Chyba sítě. Zkontrolujte své internetové připojení a zkuste to znovu."; } showError(userFriendlyMessage, false, ui.mainContentAreaPlaceholder); if (ui.mainContent) ui.mainContent.style.display = 'block'; } const totalEndTime = performance.now(); console.log(`✅ [INIT Dashboard] App initializeApp function finished. Total Time: ${(totalEndTime - totalStartTime).toFixed(2)}ms`); } catch (error) { console.error("❌ [INIT Dashboard] Kritická chyba PŘED ověřením sezení:", error); const endTime = performance.now(); console.log(`[INIT Dashboard] Initialization failed. Time: ${(endTime - totalStartTime).toFixed(2)}ms`); const initialLoaderElementForError = document.getElementById('initial-loader'); if (initialLoaderElementForError && !initialLoaderElementForError.classList.contains('hidden')) { initialLoaderElementForError.innerHTML = `<p style="color: var(--accent-pink);">CHYBA (${error.message}). OBNOVTE STRÁNKU.</p>`; } else { const globalErrorElement = document.getElementById('global-error'); if (globalErrorElement) { globalErrorElement.innerHTML = `<div class="error-message"><i class="fas fa-exclamation-triangle"></i><div>Kritická chyba inicializace: ${sanitizeHTML(error.message)}</div><button class="retry-button btn" onclick="location.reload()">Obnovit Stránku</button></div>`; globalErrorElement.style.display = 'block'; } else { alert(`Kritická chyba inicializace: ${error.message}`); } } const mainContentForError = document.getElementById('main-content'); if (mainContentForError) mainContentForError.style.display = 'none'; if (typeof setLoadingState === 'function') setLoadingState('all', false); } }
