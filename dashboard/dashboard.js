// dashboard.js
// Verze: 26.0.13 - Oprava logiky pro Streak Milestones, Monthly Rewards a perzistenci odměn.
...
(function() {
    ...
    const MONTHLY_REWARD_DAYS = 31; // Počet dní v aktuálním měsíci pro odměny (nastavitelné)

    // Konfigurace milníků studijní série
    const STREAK_MILESTONES_CONFIG = {
        ...
        14: { name: "Dvou Týdenní Hrdina", description: "Již 14 dní studia! Neuvěřitelné!", icon: "fa-star", reward_type: "badge", reward_value: "badge_streak_14", reward_key: "streak_14_days_badge_perseverance" }, // reward_value by měl být klíč odznaku
        30: { name: "Měsíční Maratonec", description: "Celý měsíc konzistence! Jsi legenda!", icon: "fa-trophy", reward_type: "title", reward_value: "title_monthly_marathon", reward_key: "streak_30_days_title_marathon" }, // reward_value by měl být klíč titulu
        // Můžete přidat další milníky
    };
    const sortedMilestoneDays = Object.keys(STREAK_MILESTONES_CONFIG).map(Number).sort((a, b) => a - b);

    // Příklad odměn pro Květen (může být konfigurováno dynamicky)
    const mayRewards = { // Květen je 5. měsíc (index 4)
        1: { type: 'title', key: 'majovy_poutnik', name: 'Titul: Májový Poutník', icon: 'fa-hiking', value: null }, // value je klíč titulu
    ...
    function cacheDOMElements() {
        ...
        const elementDefinitions = [
            ...
            { key: 'modalLongestStreakValue', id: 'modal-longest-streak-value', critical: false}, // Přidáno
    ...
    function showError(message, isGlobal = false, targetElement = ui.globalError) {
        ...
        if (errorDisplayElement) {
            if (targetElement === ui.mainContentAreaPlaceholder && !isGlobal) {
                 ...
                 if (retryBtn) {
                     retryBtn.onclick = () => { // Změna na onclick pro jednorázové přiřazení
    ...
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        if (skeletonMap[sectionKey]) {
            ...
            const loaderOverlay = modalBody?.querySelector('.loading-overlay'); // Předpokládáme, že .loading-overlay je uvnitř .modal-body
            if (loaderOverlay) loaderOverlay.classList.toggle('hidden', !isLoadingFlag);
            if (modalBody) modalBody.classList.toggle('loading', isLoadingFlag); // Můžeme přidat třídu na .modal-body pro stylování
            if (isLoadingFlag) {
            ...
            // Jen logujeme, UI se neaktualizuje přímo zde pro tituly
    ...
    async function updateLastMilestoneClaimedInDB(milestoneDay, rewardKey, rewardName) {
        ...
        try {
            // ZÁPIS DO claimed_streak_milestones
            const { error: insertError } = await supabase
                .from('claimed_streak_milestones')
                .insert({
                    user_id: currentUser.id,
                    milestone_day: milestoneDay,
                    reward_key: rewardKey, // Ujistěte se, že tento sloupec existuje
                    reward_name: rewardName
                });

            if (insertError && insertError.code !== '23505') { // 23505 = unique_violation
                console.error(`[DB Update] Error inserting into claimed_streak_milestones for milestone ${milestoneDay} (key: ${rewardKey}):`, insertError);
                throw insertError;
            } else if (insertError && insertError.code === '23505') {
                console.warn(`[DB Update] Milestone ${milestoneDay} (key: ${rewardKey}) already claimed by user ${currentUser.id}. No new record inserted into claimed_streak_milestones.`);
                // Není třeba aktualizovat last_milestone_claimed v profiles, pokud už byl záznam
                return true; // Považujeme za úspěch, protože záznam existuje
            }
            console.log(`[DB Update] Milestone ${milestoneDay} (key: ${rewardKey}) successfully recorded in claimed_streak_milestones.`);

            // Aktualizace last_milestone_claimed v profiles POUZE pokud je nový milník větší
            if (currentProfile && milestoneDay > (currentProfile.last_milestone_claimed || 0)) {
                ...
                if (profileUpdateError) {
                    ...
                    // Chyba zde by neměla blokovat úspěch, pokud byl záznam do claimed_streak_milestones úspěšný
                } else {
    ...
    function renderStreakMilestones() {
        ...
        const longestStreakOverall = currentProfile?.longest_streak_days || currentStreak; // Použijeme nejdelší sérii z profilu
        currentStreakSpan.textContent = currentStreak;
        longestStreakSpan.textContent = longestStreakOverall; // Zobrazíme nejdelší sérii

        gridContainer.innerHTML = ''; emptyState.style.display = 'none'; gridContainer.style.display = 'grid';
        // Načítání již vyzvednutých milníků
        ...
        .then(({ data: claimedData, error: fetchClaimedError }) => {
            ...
            sortedMilestoneDays.forEach(milestoneDay => {
                ...
                // Změna: Logika pro "isClaimable" nyní bere v úvahu longest_streak_days pro zobrazení, ale vyzvednutí se stále řídí currentStreak
                const isVisuallyAchieved = longestStreakOverall >= milestoneDay; // Zda milník byl někdy dosažen
                const canActuallyClaim = currentStreak >= milestoneDay && !isClaimed; // Zda lze aktuálně vyzvednout
                ...
                } else if (isVisuallyAchieved) { // Milník byl dosažen, ale možná nelze aktuálně vyzvednout
                    if (canActuallyClaim) {
                        ...
                        milestoneElement.classList.add('achieved-not-claimable'); // Nový stav
                        statusHTML = `<span class="reward-status achieved">Dosaženo (Rekord: ${longestStreakOverall})</span>`;
                        // Žádné tlačítko, pokud aktuální série nestačí
                    }
                } else { // Milník ještě nebyl nikdy dosažen
                    milestoneElement.classList.add('locked');
                    const daysNeeded = milestoneDay - longestStreakOverall; // Kolik dní chybí k rekordu
    ...
    async function claimMilestoneReward(milestoneDay, buttonElement, rewardType, rewardValue, rewardName, rewardKey) {
        ...
        // Ověření, zda uživatel aktuálně splňuje podmínku série
        const currentStreak = currentProfile?.streak_days || 0;
        ...
        // Ověření, zda již byla odměna vyzvednuta (znovu pro jistotu, i když UI by to mělo reflektovat)
        const { data: existingClaimCheck, error: clientCheckError } = await supabase
            .from('claimed_streak_milestones')
            .select('id')
            .eq('user_id', currentUser.id)
            .eq('milestone_day', milestoneDay)
            .eq('reward_key', rewardKey) // Přidána kontrola reward_key
            .maybeSingle();

        if (clientCheckError) {
            console.error("Error checking claim (client-side):", clientCheckError);
            showToast('Chyba', 'Ověření milníku selhalo.', 'error');
            return;
        }
        if (existingClaimCheck) {
            console.warn(`Milestone ${milestoneDay} (key: ${rewardKey}) already claimed (client check).`);
            showToast('Již Vyzvednuto', 'Tento milník byl již vyzvednut.', 'info');
            renderStreakMilestones(); // Znovu vykreslíme pro aktuální stav
            return;
        }
        ...
        if (dbSuccess) {
            ...
            switch (rewardType) {
                case 'points':
                    await awardPoints(rewardValue, `Odměna za sérii ${milestoneDay} dní`, 'reward_streak_milestone', null, true);
                    activityDescription += ` Získáno ${rewardValue} kreditů.`;
                    mainLogMessage += ` (+${rewardValue} kreditů)`;
                    break;
                case 'badge':
                    const badgeIdentifier = rewardValue; // Předpokládáme, že reward_value je title_key odznaku
                    console.log(`[ClaimMilestone] Attempting to award badge with identifier: ${badgeIdentifier}`);
                    const { data: badgeData, error: badgeError } = await supabase
                        .from('badges')
                        .select('id, title')
                        .or(`title_key.eq.${badgeIdentifier},id.eq.${typeof badgeIdentifier === 'number' ? badgeIdentifier : -1}`) // Hledání podle klíče nebo ID
                        .single();

                    if (badgeError || !badgeData) {
                        console.error(`[ClaimMilestone] Error fetching badge data for identifier ${badgeIdentifier}:`, badgeError);
                        showToast('Chyba', `Odznak "${rewardName}" nenalezen nebo chyba databáze.`, 'error');
                    } else {
                        const { error: insertBadgeError } = await supabase
                            .from('user_badges')
                            .insert({ user_id: currentUser.id, badge_id: badgeData.id });

                        if (insertBadgeError && insertBadgeError.code !== '23505') { // 23505 = unique_violation
                            ...
                            else currentProfile.badges_count = newBadgeCount; // Aktualizace lokálního profilu
                            showToast('Odznak Získán!', `Získali jste odznak: ${badgeData.title}`, 'success');
                            activityDescription += ` Získán odznak: ${badgeData.title}.`;
                            mainLogMessage += ` (Odznak: ${badgeData.title})`;
                        }
                    }
                    break;
                case 'title':
                    const titleKeyToAward = rewardValue; // Předpokládáme, že reward_value je title_key titulu
            ...
            showToast('Chyba', 'Nepodařilo se uložit vyzvednutí milníkové odměny.', 'error');
            // Návrat tlačítka do původního stavu, pokud DB operace selhala
            if (buttonElement) {
        ...
        setLoadingState('streakMilestones', false);
        renderStreakMilestones(); // Vždy znovu vykreslit pro aktualizaci stavu
    }
