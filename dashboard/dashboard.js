// dashboard.js
// Verze: 27.0.4 - Oprava zobrazení poslední kreditové transakce a změna logiky milníků série na longest_streak_days.
// EDIT LOG: Developer Action -> Fixing credit transaction display and changing streak milestone logic to longest_streak_days.
...
(function() {
    ...
    async function updateLastMilestoneClaimedInDB(milestoneDay, rewardKey, rewardName) {
        ...
        try {
            // Insert into claimed_streak_milestones
            ...
            if (insertError && insertError.code !== '23505') { // '23505' is unique_violation
                throw insertError; // Propagate other errors
                ...
                // No need to throw, but good to know. The UI should ideally prevent this.
            } else {
                console.log(`[DB Update Milestones] Milník ${rewardKey} pro den ${milestoneDay} úspěšně zaznamenán do claimed_streak_milestones.`);
            }

            // Update profiles.last_milestone_claimed only if this milestone_day is greater
            if (currentProfile && milestoneDay > (currentProfile.last_milestone_claimed || 0)) {
                ...
                if (profileUpdateError) {
                    ...
                    // Don't throw here, as the primary record in claimed_streak_milestones might have succeeded.
                } else {
    ...
    function updateStatsCards(stats) {
        ...
        // Clear previous latest credit change if it exists
        if (ui.latestCreditChange) {
            ui.latestCreditChange.innerHTML = '';
            ui.latestCreditChange.style.display = 'none'; // Ensure it's hidden initially
        }
        ...
        if (!stats) {
            ...
            if (statElements.pointsFooter) statElements.pointsFooter.innerHTML = `<i class="fas fa-minus"></i> Načítání...`; // Default for points footer
        ...
        if (ui.totalPointsValue.firstChild && ui.totalPointsValue.firstChild.nodeType === Node.TEXT_NODE) {
            ...
            // Ensure any old #latest-credit-change span is removed before adding new content or new span
            const existingSpan = ui.totalPointsValue.querySelector('#latest-credit-change');
            if (existingSpan) existingSpan.remove();
            ui.totalPointsValue.textContent = `${pointsValue} `; // Set the main points value
        }

        // MODIFIED: Logic for totalPointsFooter to always use latestTxData if available
        const latestTx = (typeof fetchAndDisplayLatestCreditChange.latestTxData !== 'undefined') ? fetchAndDisplayLatestCreditChange.latestTxData : null;

        if (latestTx && latestTx.amount !== undefined) {
            const amount = latestTx.amount;
            const description = latestTx.description || `Transakce`;
            const sign = amount > 0 ? '+' : (amount < 0 ? '' : ''); // Only show + for positive
            const amountColorClass = amount > 0 ? 'positive' : (amount < 0 ? 'negative' : '');
            let displayDescription = description;
            const maxDescLengthFooter = 20; // Max length for description in footer
            if (displayDescription.length > maxDescLengthFooter) {
                displayDescription = displayDescription.substring(0, maxDescLengthFooter - 3) + "...";
            }
            // Construct the HTML, ensuring the icon is appropriate for the transaction type
            let iconClassFooter = 'fa-history'; // Default
            ...
            // Fallback if no recent transaction or if points_weekly is preferred and available
    ...
    // --- END: Rewards Logic ---
    // --- START: Streak Milestones Logic (MODIFIED) ---
    function renderStreakMilestones() {
        ...
        sortedMilestoneDays.forEach(dayCount => {
            ...
            const canClaim = (currentProfile.longest_streak_days || 0) >= dayCount && !isClaimed;
            ...
            if (isClaimed) {
                ...
                // MODIFIED: Text for locked state if longest_streak_days is used for unlocking
    ...
    async function handleStreakMilestoneClaim(milestoneDay, rewardKey) {
        ...
        // MODIFIED: Unlock condition now uses longest_streak_days
        if ((currentProfile.longest_streak_days || 0) < milestoneDay) {
            showToast('Nedostatečná Nejdelší Série', `Potřebujete mít nejdelší sérii alespoň ${milestoneDay} dní. Vaše nejdelší série je ${currentProfile.longest_streak_days || 0} dní.`, 'warning');
            return;
        }
        ...
        try {
            ...
            if (allRewardsAwardedSuccessfully) {
                ...
                if (dbRecordSuccess) {
                    ...
                    await logActivity(currentUser.id, 'streak_milestone_claimed', `Vyzvednut milník série: ${milestoneConfig.name}`, `Dosaženo ${milestoneDay} dní (rekord).`, { milestone_day: milestoneDay, reward_key: rewardKey });
            ...
            setLoadingState('streakMilestones', false);
            // if (claimButton) claimButton.disabled = false; // Button will be re-rendered
            ...
            if (allRewardsAwardedSuccessfully) {
                ...
                await fetchAndDisplayLatestCreditChange(currentUser.id); // Ensure this is called to get the latest transaction
                updateStatsCards(userStatsData); // This will now use the fetched latest transaction
            }
        }
    }
    // --- END: Streak Milestones Logic ---
...
// EDIT LOG: Stage -> Ensured fetchAndDisplayLatestCreditChange is used in updateStatsCards. Changed milestone unlock to use longest_streak_days.
})();