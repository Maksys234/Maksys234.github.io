...
(function() {
    ...
    let userBadges = [];
    let allBadges = [];
    let allTitles = []; // <<< NEW: Store available titles from title_shop
    let leaderboardData = [];
    ...
    let isLoading = {
        ...
        recentBadges: false,
        notifications: false,
        titleShop: false // <<< NEW: Loading state for title shop
    };
    ...
    const ui = {
        ...
        sidebarRole: document.querySelector('.sidebar .user-role'), // <<< NEW: Sidebar role element
        ...
        titleShopContainer: document.getElementById('title-shop-container'), // <<< NEW
        shopUserCredits: document.getElementById('shop-user-credits'),       // <<< NEW
        titleShopLoading: document.getElementById('title-shop-loading'),     // <<< NEW
        titleShopGrid: document.getElementById('title-shop-grid'),           // <<< NEW
        titleShopEmpty: document.getElementById('title-shop-empty')          // <<< NEW
    };
    ...
    function setLoadingState(section, isLoadingFlag) {
        // <<< UPDATED: Added 'titleShop' mapping >>>
        const sectionsMap = {
        ...
        sectionsToUpdate.forEach(secKey => {
            ...
            if (config.container) {
                // General container loading class
                config.container.classList.toggle('loading', isLoadingFlag);

                // Specific handling for different sections
                if (config.childrenSelector) { // For stats cards
                ...
                if (secKey === 'leaderboard') {
                    ...
                    if (config.emptyEl) config.emptyEl.style.display = 'none'; // Initially hide empty state during load
                    ...
                    if (config.loadingEl) config.loadingEl.style.display = isLoadingFlag ? 'flex' : 'none'; // Show/hide dedicated loader
                    if (config.contentEl) config.contentEl.style.display = isLoadingFlag ? 'none' : 'grid'; // Show grid when loaded
                ...
                } else { // For userBadges, availableBadges, recentBadges (no dedicated loader)
                    if (isLoadingFlag) {
                        if (config.contentEl) config.contentEl.style.display = 'none';
                        if (config.emptyEl) config.emptyEl.style.display = 'none';
                    }
                    // Note: After loading, render functions will handle displaying content or empty state
                }

            } else if (secKey === 'notifications' && ui.notificationBell) { // Special handling for notification bell itself
    ...
    async function initializeApp() { console.log("🚀 [Init Oceneni - Kyber v6] Initializing Awards Page..."); if (!initializeSupabase()) return; setupUIEventListeners(); if (ui.initialLoader) { ui.initialLoader.classList.remove('hidden'); ui.initialLoader.style.display = 'flex'; } if (ui.mainContent) ui.mainContent.style.display = 'none'; try { console.log("[Init Oceneni - Kyber] Checking auth session..."); const { data: { session }, error: sessionError } = await supabase.auth.getSession(); if (sessionError) throw new Error(`Nepodařilo se ověřit přihlášení: ${sessionError.message}`); if (session?.user) { currentUser = session.user; console.log(`[Init Oceneni - Kyber] User authenticated (ID: ${currentUser.id}). Loading data...`); currentProfile = await fetchUserProfile(currentUser.id); // <<< UPDATED: Fetch includes title fields
                if (!currentProfile) throw new Error("Nepodařilo se načíst profil uživatele.");
                // Fetch titles definition needed for sidebar update
                allTitles = await fetchTitleShopData();
                updateSidebarProfile(currentProfile, allTitles); // <<< UPDATED: Pass titles for role lookup
    ...
    // <<< NEW: Fetch Title Shop Data >>>
    async function fetchTitleShopData() { console.log("[TitleShop] Fetching available titles..."); if (!supabase) return []; try { const { data, error } = await supabase .from('title_shop') .select('*') .eq('is_available', true) // Fetch only available titles .order('cost', { ascending: true }); // Order by price, cheapest first
             if (error) throw error; console.log(`[TitleShop] Fetched ${data?.length || 0} available titles.`); return data || []; } catch (error) { console.error("[TitleShop] Error fetching available titles:", error); showError("Nepodařilo se načíst nabídku titulů v obchodě."); return []; } }
    // <<< UPDATED: Load All Award Data includes Title Shop >>>
    async function loadAllAwardData() {
        ...
        try {
            // <<< UPDATED: Added fetchTitleShopData >>>
            const results = await Promise.allSettled([
                ...
                fetchTitleShopData() // <<< Fetch titles here too (redundant if fetched in init, but ensures fresh data on retry)
            ]);
            ...
            const [
                ...
                statsResult,
                notificationsResult,
                titlesResult // <<< Get titles result
            ] = results;
            ...
            allTitles = titlesResult.status === 'fulfilled' ? titlesResult.value : []; // <<< Store fetched titles
            ...
            if (titlesResult.status === 'rejected') { console.error("❌ Error fetching titles:", titlesResult.reason); showError("Nepodařilo se načíst obchod s tituly."); } // <<< Show error for titles

            // Update UI sections and manage loading states
            setLoadingState('userBadges', false);
            setLoadingState('recentBadges', false);
            setLoadingState('leaderboard', false);
            setLoadingState('stats', false);
            setLoadingState('notifications', false);
            setLoadingState('titleShop', false); // <<< Set title shop loading to false
    ...
    function updateSidebarProfile(profile, titlesData = allTitles) {
        ...
        if (profile) {
            ...
            ui.sidebarRole.textContent = 'Pilot'; // Reset role on error/no profile
        }
    }
    ...
    function renderTitleShop(titles, userProfile) {
        ...
        setLoadingState('titleShop', false); // Ensure loading state is off
        ...
        ui.titleShopGrid.style.display = 'grid'; // Make sure grid is visible
        ...
        titles.forEach((title, index) => {
            ...
            itemElement.className = 'title-item card'; // Use card style
            itemElement.setAttribute('data-title-key', title.title_key);
            itemElement.setAttribute('data-title-cost', title.cost);
            itemElement.setAttribute('data-animate', ''); // Add animation attribute
            itemElement.style.setProperty('--animation-order', index); // Set animation order
        ...
        requestAnimationFrame(initScrollAnimations); // Re-initialize animations for new items
    }
    ...
    function setupUIEventListeners() {
        ...
        // <<< NEW: Title Shop Event Listener (using delegation) >>>
        if (ui.titleShopGrid) {
    ...
    async function handleBuyTitle(titleKey, cost, buttonElement) {
        ...
        try {
            // Ensure purchased_titles is an array
            const currentPurchased = Array.isArray(currentProfile.purchased_titles) ? currentProfile.purchased_titles : [];

            // Prevent duplicates (though UI should hide button already)
            if (currentPurchased.includes(titleKey)) {
                 showToast('Již Vlastněno', 'Tento titul již máte zakoupený.', 'info');
                 buttonElement.innerHTML = '<i class="fas fa-shopping-cart"></i> Koupit'; // Reset button text
                 // Re-render shop to fix state if needed
            ...
            if (ui.pointsCount) ui.pointsCount.textContent = newCredits; // Update stats card too
            renderTitleShop(allTitles, currentProfile); // Re-render the shop section to update button states
            ...
            // Re-enable button on error, reset text
            buttonElement.disabled = false;
            buttonElement.innerHTML = '<i class="fas fa-shopping-cart"></i> Koupit';
             // Check if user still has enough credits (might have changed in another tab)
             if(currentProfile.points < cost) buttonElement.disabled = true;

        }
    }

    // <<< NEW: Handle Equip Title >>>
    async function handleEquipTitle(titleKey, buttonElement) {
        console.log(`[Title Shop] Attempting to equip title: ${titleKey}.`);
         ...
         // Ensure the title is actually purchased
         ...
         try {
            ...
            // Update UI
            renderTitleShop(allTitles, currentProfile); // Re-render shop to update buttons
            updateSidebarProfile(currentProfile, allTitles); // Update the sidebar role display
