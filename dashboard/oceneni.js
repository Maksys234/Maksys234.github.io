...
(function() {
    ...
    function initializeSupabase() {
         try {
             if (typeof window.supabase === 'undefined' || typeof window.supabase.createClient !== 'function') { throw new Error("Knihovna Supabase nebyla správně načtena."); }
    ...
    async function fetchAllBadgesDefinition() {
         console.log("[Badges] Fetching all badge definitions...");
         if (!supabase) return [];
         try {
             const { data, error } = await supabase.from('badges').select('*').order('id');
             if (error) throw error;
             console.log(`[Badges] Fetched ${data?.length || 0} badge definitions.`);
             return data || [];
         } catch (error) {
             console.error("[Badges] Error fetching definitions:", error);
             showError("Nepodařilo se načíst definice odznaků.");
             return [];
         }
     }
    async function fetchUserEarnedBadges(userId) {
         console.log(`[UserBadges] Fetching earned badges for user ${userId}...`);
         ...
         try {
             const { data, error } = await supabase
                 ...
                 `) // Select needed fields from badges
                 .eq('user_id', userId)
                 .order('earned_at', { ascending: false }); // Sort by newest first

             if (error) throw error;
             console.log(`[UserBadges] Fetched ${data?.length || 0} earned badges.`);
             return data || [];
         } catch (error) {
             console.error("[UserBadges] Error fetching earned badges:", error);
             showError("Nepodařilo se načíst získané odznaky.");
             return [];
         }
     }
    ...
    function renderLeaderboard(data) {
         ...
         setLoadingState('leaderboard', false); // Stop loading animation

         ui.leaderboardBody.innerHTML = ''; // Clear previous rows
         const filterMap = { points: 'Kredity', badges: 'Odznaky', streak: 'Série' };
         ui.scoreHeader.textContent = filterMap[currentLeaderboardFilter] || 'Skóre'; // Update header text

         if (!data || data.length === 0) {
             ui.leaderboardEmpty.style.display = 'block'; // Show empty state message
             ui.leaderboardBody.style.display = 'none'; // Hide table body
             console.log("[Render] Leaderboard empty.");
             return;
         }
         ui.leaderboardEmpty.style.display = 'none';
         ui.leaderboardBody.style.display = ''; // Show table body

         const fragment = document.createDocumentFragment();
         data.forEach((entry) => {
             // Ensure profile data exists for the entry
             const userProfile = entry.profile;
             if (!userProfile) {
                 console.warn(`Missing profile data for user_id: ${entry.user_id} in leaderboard entry. Skipping.`);
                 return; // Skip rendering this entry if profile is missing
             }

             const rank = entry.calculated_rank || entry.rank || '?'; // Use calculated or DB rank
             const isCurrentUser = entry.user_id === currentUser?.id;
             // Construct display name: try full name, then username, then fallback
             ...
             const initials = getInitials(userProfile); // Use helper function with profile data
             ...
             switch (currentLeaderboardFilter) {
                 ...
                 case 'streak': scoreValue = userProfile.streak_days ?? 0; break; // Get streak from profile
                 default: scoreValue = entry.points ?? 0;
             }
             const badgesCount = entry.badges_count ?? 0; // Always show badge count
