// oceneni.js
// Версия: 23.13 - Oprava syntaktické chyby (chybějící catch/finally)
(function() { // <<<< START IIFE
    ...
    function cacheDOMElements() {
        ...
        const ids = [
            ...
            // Note: sidebar-user-role is handled by checking for sidebar-user-title first below
        ];
        const notFound = [];
        ids.forEach(id => {
        ...
        // Handle potential alternative ID for sidebar role/title
        if (ui.sidebarUserTitle) { ui.sidebarRole = ui.sidebarUserTitle; }
        else {
            const roleEl = document.getElementById('sidebar-user-role');
            if (roleEl) { ui.sidebarRole = roleEl; }
	...
	function setLoadingState(section, isLoadingFlag) {
		...
		sectionsToUpdate.forEach(secKey => {
			...
			if (config.childrenSelector && config.container) { config.container.querySelectorAll(config.childrenSelector).forEach(child => { child?.classList.toggle('loading', isLoadingFlag); }); }
			...
			if (isLoadingFlag) {
				...
				// Call appropriate skeleton rendering function
				if (secKey === 'userBadges' && typeof renderBadgeSkeletons === 'function') renderBadgeSkeletons(contentEl);
				else if (secKey === 'availableBadges' && typeof renderAvailableBadgeSkeletons === 'function') renderAvailableBadgeSkeletons(contentEl);
				else if (secKey === 'titleShop' && typeof renderTitleShopSkeleton === 'function') renderTitleShopSkeleton();
				else if (secKey === 'avatarDecorations' && typeof renderAvatarDecorationsSkeleton === 'function') renderAvatarDecorationsSkeleton();
				else if (secKey === 'notifications' && typeof renderNotificationSkeletons === 'function') renderNotificationSkeletons(2);
			} else {
				const isContentEmpty = !contentEl || !contentEl.hasChildNodes();
				if (isContentEmpty && emptyEl) { emptyEl.style.display = 'block'; if (contentEl) contentEl.style.display = 'none'; }
				else { if (emptyEl) emptyEl.style.display = 'none'; if (contentEl) contentEl.style.display = 'grid'; } // Use grid by default
			...
			if (secKey === 'notifications' && ui.notificationBell) {
				...
				if (ui.markAllReadBtn) { const count = parseInt(ui.notificationCount?.textContent?.replace('+', '') || '0'); ui.markAllReadBtn.disabled = isLoadingFlag || count === 0; }
			}
		});
	}
	...
	async function initializeApp() {
        ...
        try { // <<<< START try block
            ...
            // Fetch user profile initially (needed for most operations)
            ...
            // Setup basic UI elements and listeners that DON'T depend on full data load
            ...
            // Load all other data needed for display
            await loadAllAwardData(); // This fetches stats, badges, titles, decor, leaderboard, notifications and renders them
            ...
            initTooltips(); // Initialize tooltips after dynamic content is rendered

             // Dispatch a custom event signifying the dashboard framework is ready (including profile)
             // This allows other scripts (like main.js) to react AFTER the core dashboard is set up.
             const readyEvent = new CustomEvent('dashboardReady', {
                 detail: {
                     user: currentUser,
                     profile: currentProfile,
                     client: supabase, // Pass the client too
                     titles: allTitles // Pass fetched titles
                 }
             });
             document.dispatchEvent(readyEvent);
             console.log("[Init Oceneni] Dispatching 'dashboardReady' event.");


            console.log("✅ [Init Oceneni] Page fully initialized.");

        // <<<< START FIX: Added catch and finally blocks >>>>
        } catch (error) {
             ...
             if (ui.initialLoader && !ui.initialLoader.classList.contains('hidden')) {
                 ...
                 showError(`Chyba inicializace: ${error.message}`, true);
                  ...
                  alert(`Chyba inicializace: ${error.message}`);
             }
             if (ui.mainContent) ui.mainContent.style.display = 'block'; // Show content to display error if possible
              // Ensure loading states are reset on error
             if (typeof setLoadingState === 'function') setLoadingState('all', false);
        } finally {
             // Code here will run regardless of whether there was an error or not
             if (ui.initialLoader && !ui.initialLoader.classList.contains('hidden')) {
                  ui.initialLoader.classList.add('hidden');
                  setTimeout(() => { if (ui.initialLoader) ui.initialLoader.style.display = 'none'; }, 500);
             }
             // Any other cleanup or final actions can go here
             console.log("🏁 [Init Oceneni] Finální část inicializace (finally block).");
        }
        // <<<< END FIX >>>>>>
	}

	// --- Initialize Supabase (moved function definition here) ---
	function initializeSupabase() {
		try {
			...
			if (!supabase) throw new Error("Vytvoření klienta Supabase selhalo.");
	...
	// --- Event Listeners Setup (moved function definition here) ---
	function setupUIEventListeners() {
		...
		const safeAddListener = (element, eventType, handler, key) => {
			if (element) {
				// Remove previous listener if exists (simple approach)
		...
		safeAddListener(ui.refreshDataBtn, 'click', handleGlobalRetry, 'refreshDataBtn');
