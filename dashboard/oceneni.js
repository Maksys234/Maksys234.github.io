...
// Version: 23.19 - Added filter for is_purchasable in fetchTitleShopData.
(function() { // IIFE for scope isolation
    ...
    let allBadges = [];
    let allTitles = [];
    let allDecorations = []; // Kept for sidebar styling logic
    let leaderboardData = [];
    ...
    function setLoadingState(sectionKey, isLoadingFlag) {
        ...
        const updateSingleSection = (key, loading) => {
            ...
            if (loading) {
                ...
                // After loading, rendering functions will handle showing content or empty state.
                // We don't need to guess here, the render function knows best.
                // If a section has no content *after* loading, its render function should show the empty state.
            }
    ...
    // <<< MODIFIED: fetchTitleShopData includes is_purchasable filter >>>
    async function fetchTitleShopData() {
        ...
        try {
            const { data, error } = await supabase
                ...
                .eq('is_purchasable', true) // <<< Only fetch purchasable titles
                .order('cost', { ascending: true });
            if (error) throw error;
            console.log("[Titles] Fetched purchasable titles:", data);
            return data || [];
        } catch (error) {
    ...
    async function fetchAvatarDecorationsData() { console.warn("Avatar decorations fetching skipped: Table 'avatar_decorations_shop' does not exist or feature is disabled."); return []; } // Return empty array
    ...
    async function loadAllAwardData() {
        ...
        try {
            ...
            renderAvailableBadges(allBadges, userBadges); // Fixed variable name here
            ...
            updateSidebarProfile(currentProfile, allTitles); // Update sidebar after all data potentially used by it is fetched
            ...
            // Attempt to render with potentially partial data or defaults
    ...
    async function fetchTitles() { if (!supabase) return []; console.log("[Titles] Fetching available titles..."); setLoadingState('titles', true); try { const { data, error } = await supabase.from('title_shop').select('title_key, name').eq('is_available', true).eq('is_purchasable', true); if (error) throw error; console.log("[Titles] Fetched purchasable titles:", data); return data || []; } catch (error) { console.error("[Titles] Error fetching purchasable titles:", error); return []; } finally { setLoadingState('titles', false); } }
    async function initializeApp() {
        ...
        try {
            ...
            allTitles = await fetchTitles(); // Fetch purchasable titles

            updateSidebarProfile(currentProfile, allTitles); // Update sidebar
            setupEventListeners();
            initMouseFollower();
            initHeaderScrollDetection();
            updateCopyrightYear();
            updateOnlineStatus();
            await loadAllAwardData(); // Load the rest
